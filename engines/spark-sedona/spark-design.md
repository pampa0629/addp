...
**关键洞察**：
- Spark SQL Thrift Server 提供 JDBC 接口（端口 10000）
- 与 Doris 的使用方式完全一致
- 无需区分"是否存储数据"，关键是"是否提供 SQL 接口"

### 2. Sedona 作为 Spark SQL 的空间函数扩展
...
**GeoPandas vs Spark + Sedona**：

| 特性 | GeoPandas Engine | Spark SQL + Sedona |
|------|-----------------|-------------------|
...
```json
{
  ...
  "subtype": "spark_sql",
  "name": "Spark SQL Engine with Sedona",
...
- **数据源名称**：Spark SQL Engine with Sedona
- **连接类型**：Spark SQL (Hive2)
...
### Spark SQL vs Doris vs PostgreSQL

| 特性 | PostgreSQL + PostGIS | Doris | Spark SQL + Sedona |
|------|---------------------|-------|-------------------|
...
### Spark SQL + Sedona vs GeoPandas Engine

| 特性 | GeoPandas Engine | Spark SQL + Sedona |
|------|-----------------|-------------------|
| **架构类型** | Python 库封装 + Flask API | SQL 引擎（原生支持） |
| **部署位置** | 独立容器（geopandas-engine） | Spark 集群（business） |
...
1. **配置数据源连接**
   - 在 Develop 模块中添加 Spark SQL 数据源支持（如需要）
...
### 2. 可视化工作流支持（可选）
如果未来用户需要类似 GeoPandas Engine 的拖拽式工作流：
...
| **Spark SQL** | **业务数据计算（SQL）** | **10000** | **business-network** |
...
**不实施的内容**：
- ❌ `spark-sql-engine/`（独立 Python Flask 服务）
