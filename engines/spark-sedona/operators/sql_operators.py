...
SQL_METADATA = OperatorMetadata(
    ...
    params=[
        ...
        OperatorParam(
            ...
            required=True,
            description="SQL查询语句",
            notes="必须是合法的 Spark SQL 语法,表需要提前注册为临时视图"
        )
    ],
    use_cases=[
...
CREATE_TEMP_VIEW_METADATA = OperatorMetadata(
    ...
    overview="create_temp_view 将 DataFrame 注册为 Spark SQL 临时视图,使其可以在 sql 算子中通过表名引用。视图在 Spark Session 生命周期内有效,支持替换已存在的同名视图。",
    params=[
        OperatorParam(
            name="input_df",
            type="dataframe",
