# Python Workflow Engine 容器管理指南

本文档说明如何在 ADDP 平台的 local 和 production 部署模式下管理 python-workflow-engine 容器。
...
3. **服务列表** (593 行):
   - 已添加到标准服务列表: `"python-workflow-engine:engines/geopandas"`
...
1. **镜像检查** (71 行):
   ...
   ```
   - 启动前验证 python-workflow-engine 镜像是否存在
   - 缺失时提示用户先构建镜像
...
**使用方式**:
```bash
# 启动所有服务 (包括 python-workflow-engine)
bash scripts/local/start.sh

# 服务会自动通过 docker-compose.yml 启动 python-workflow-engine
```
...
**说明**:
- 无需修改，使用 `docker compose down` 会自动停止所有服务包括 python-workflow-engine

**使用方式**:
```bash
# 停止应用层 (包括 python-workflow-engine)
...
- python-workflow-engine 会随其他服务一起重启

**使用方式**:
```bash
# 重启应用层 (包括 python-workflow-engine)
...
2. **健康检查列表** (76 行):
   ...
   ```
   - 启动后自动检查 python-workflow-engine 健康状态 (最长 30 秒)
...
# 3. 业务后端 + python-workflow-engine + gateway
# 4. 前端服务 + nginx
```
...
**使用方式**:
```bash
# 检查所有服务健康状态 (包括 python-workflow-engine)
...
```bash
# 1. 构建 python-workflow-engine 镜像 (首次或代码修改后)
bash scripts/build/build-images.sh --services python-workflow-engine

# 2. 启动开发环境 (包括 python-workflow-engine)
bash scripts/local/start.sh
...
# 4. 查看日志
docker compose -f docker-compose.yml logs -f python-workflow-engine
...
如果需要单独管理 python-workflow-engine 容器，也可以直接使用 docker-compose:
...
# 重启
docker-compose restart python-workflow-engine

# 停止
docker-compose stop python-workflow-engine

# 查看日志
docker-compose logs -f python-workflow-engine

# 查看状态
docker-compose ps python-workflow-engine

# 进入容器
docker-compose exec python-workflow-engine bash

# 重新构建并启动
docker-compose up -d --build python-workflow-engine
```
...
1. **镜像构建**:
   - 首次使用前必须先构建镜像: `bash scripts/build/build-images.sh --services python-workflow-engine`
   - 修改代码后需要重新构建

2. **依赖服务**:
   - python-workflow-engine 依赖 System Backend，确保 System Backend 先启动
...
4. **健康检查**:
   ...
   - 健康检查失败时查看日志: `docker-compose logs python-workflow-engine`
...
### scripts/build/build-images.sh
- ✅ 添加 python-workflow-engine 智能缓存检查
- ✅ 添加 python-workflow-engine 构建逻辑
- ✅ 添加到服务列表
...
目前所有容器管理脚本已完全支持 python-workflow-engine，可以直接使用。

如需进一步优化，可以考虑:
- [ ] 添加独立的 python-workflow-engine 启动/停止脚本
- [ ] 支持多 python-workflow-engine 实例的负载均衡
- [ ] 添加性能监控和指标收集
- [ ] 集成到 CI/CD 流程
