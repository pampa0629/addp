...
LOAD_METADATA = OperatorMetadata(
    ...
    params=[
        ...
        OperatorParam(
            ...
            type="str",
            required=False,
            description="数据库schema名称(table类型)",
            notes="PostgreSQL必需,MySQL可选"
        ),
        OperatorParam(
            name="table",
            type="str",
        ...
        OperatorParam(
            ...
            type="str",
            required=False,
            description="文件路径(file类型)",
            notes="支持 s3a://、hdfs://、file:// 协议"
        ),
        OperatorParam(
            name="format",
            type="str",
    ...
    use_cases=[
        "POI 数据分析: 从 PostgreSQL 加载 100万条 POI 点位数据,进行空间聚类分析",
...
SAVE_METADATA = OperatorMetadata(
    ...
    use_cases=[
        ...
        "中间结果缓存: 将空间连接的中间结果保存为 Parquet,供下游工作流复用"
    ],
    notes=[
...
PREVIEW_METADATA = OperatorMetadata(
    ...
    use_cases=[
        ...
        "工作流调试: 在复杂工作流中插入 preview 节点,快速验证中间结果是否符合预期",
...
CACHE_METADATA = OperatorMetadata(
    ...
    use_cases=[
        ...
        "交互式查询: Jupyter Notebook 中缓存清洗后的数据,支持快速试错和探索"
    ],
    notes=[
...
PERSIST_METADATA = OperatorMetadata(
    ...
    use_cases=[
        ...
        "纯磁盘缓存: 临时结果表使用 DISK_ONLY,避免占用宝贵内存资源",
        "高频访问数据: 省市边界等基础数据使用 MEMORY_ONLY,保证最快访问速度"
    ],
    notes=[
