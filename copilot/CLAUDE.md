...
6. **生成与校验候选**：SQL/Cypher 先生成结构化 Query Plan，再基于已验证事实生成并校验只读候选。MQL 使用唯一的强类型语义计划路径：模型只选择已验证 collection、字段、谓词、排序、投影和统计语义，不直接生成 MQL；Copilot 确定性校验字段类型、数组元素类型、操作符、参数、统计对象和歧义后，由 MQL 编译器生成单个 JSON command object。任何未经用户确认的同义词扩展、值翻译或非空 `assumptions` 都必须返回澄清，不能进入编译。MQL 不保留模型自由生成候选的旧路径
