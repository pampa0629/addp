...
6. **生成与校验候选**：SQL/Cypher 先生成结构化 Query Plan，再基于已验证事实生成并校验只读候选。MQL 使用唯一的强类型语义计划路径：模型只选择已验证 collection、字段、谓词、排序、投影、统计或集合比较语义，不直接生成 MQL；集合比较只允许两个实体、一个或多个数组元素身份字段，以及 `intersection_count`、`jaccard`、`overlap_coefficient` 三种明确口径，编译器按身份字段去重后确定性生成交并集管道。只说“重叠度”等未定义口径时必须返回澄清，不能默认 Jaccard。Copilot 确定性校验字段类型、数组元素类型、操作符、参数、统计对象和歧义后，由 MQL 编译器生成单个 JSON command object。任何未经用户确认的同义词扩展、值翻译或非空 `assumptions` 都必须返回澄清，不能进入编译。MQL 不保留模型自由生成候选的旧路径
