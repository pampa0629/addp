...
| `standard_document_extraction` | `/standard-documents/extract` | 从 Standard 已分段并标注绝对行号的 Markdown 修订中提炼可追溯候选，不持久化和发布标准。 |
...
- Standard 调用文档候选提炼时使用 `addp-standard` Tenant Service Access Token；Copilot 只校验证据定位和结构化输出，批次、候选、证据及人工裁决由 Standard 保存。
