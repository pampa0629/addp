...
class KGBuildPipeline:
    ...
    async def run(self, request: KGExtractRequest) -> KGExtractResponse:
        ...
        print(f"\n[KGBuildPipeline] 开始处理 chunk，文本长度: {len(request.text)} 字符")
        print(f"[KGBuildPipeline] 本体: {len(request.ontology.entity_types)} 个实体类型，"
              f"{len(request.ontology.relation_types)} 个关系类型")
        ...
        print(f"[KGBuildPipeline] ✅ 完成，实体: {len(entities)}，关系: {len(relations)}，"
