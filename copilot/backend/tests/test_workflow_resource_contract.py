...
from models.workflow_models import Workflow, Task
...
class FakeOperatorCatalog:
    def __init__(self):
        self.seen_workflow_engine_ids = []

    async def get_operator(self, operator_name: str, workflow_engine_id: int, tenant_id: int = 0):
...
def test_service_stops_before_generation_when_spatial_resource_has_no_crs():
    ...
    service.operator_selection_chain = MustNotSelect()
...
async def _assert_validation_requires_workflow_engine_id():
    validator = WorkflowValidationChain(FakeOperatorCatalog())
    workflow = Workflow(tasks=[
        Task(
            id="task1",
            operator="load",
