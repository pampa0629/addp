...
def test_build_data_source_candidates_maps_schema_to_namespace_contract():
    candidates = build_data_source_candidates(
        metadata_results=[{
            ...
            "schema": "public",
            "score": 0.76,
            "locator": "addp://engine/12/path/public/roads?type=table&item_id=91",
            "target_parent_locator": "addp://engine/12/path/public?type=schema&node_id=7",
        }],
        engines=[{
            "id": 12,
            "name": "城市 PostgreSQL",
...
def test_build_data_source_candidates_maps_object_storage_location():
    candidates = build_data_source_candidates(
        metadata_results=[{
            ...
            "type": "object",
            "engine_id": 8,
            "locator": "addp://engine/8/path/gis/roads/roads.parquet?type=object&item_id=92",
            "target_parent_locator": "addp://engine/8/path/gis/roads?type=prefix&node_id=8",
    ...
    assert payload["resource_name"] == "roads.parquet"
...
def test_build_data_source_candidates_skips_results_without_fact_locators():
    candidates = build_data_source_candidates(
        metadata_results=[{
            "name": "roads",
            "type": "table",
