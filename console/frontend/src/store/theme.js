...
export const useThemeStore = defineStore('theme', () => {
  // 使用共享的 useTheme composable（不监听 Console，因为自己就是 Console）
  ...
  watch(theme.mode, (newMode) => {
    ...
    console.log(`[Console Theme] 广播主题变化到 ${iframes.length} 个 iframe`, {
      mode: newMode,
    ...
    iframes.forEach((iframe, index) => {
      try {
        ...
        console.log(`[Console Theme] 已发送消息到 iframe #${index} (${targetOrigin})`)
      } catch (e) {
        console.warn(`[Console Theme] 发送消息到 iframe #${index} 失败:`, e)
      }
    })
