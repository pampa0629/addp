...
describe('Console navigation bridge', () => {
  ...
  it('groups IAM navigation into five business-category pages', () => {
    ...
    for (const page of ['organization', 'accounts', 'roles', 'application-access', 'security']) {
    ...
    expect(zhCn.console.menus.system.iamOrganization).toBe('组织管理')
    ...
    ], 'tenant').map(item => item.route)).toContain('/system/iam/roles')
  ...
  it('exposes the consolidated Security information architecture', () => {
    ...
    expect(configSource).not.toContain("index: '/security/protection-baselines'")
    ...
    expect(searchSource).not.toContain("route: '/security/protection-baselines'")
