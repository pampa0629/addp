...
文本逻辑类型统一为 `string`，不另设 `text`；PostgreSQL 物理映射为有最大长度时 `VARCHAR(n)`、未限定长度时 `TEXT`。已有 `text` 属性和逻辑字段迁移为无长度约束的 `string`，保持原来的物理文本语义，并推进受影响聚合版本。Model 的 `decimal` 逻辑类型统一表示 DECIMAL(38,18)，PostgreSQL 物理映射为 `NUMERIC(38,18)`；目标表的原生精度必须与逻辑语义一致，不能以无精度 `numeric` 冒充。历史执行与指标依赖快照不改写；版本或依赖发生漂移时按现有流程重新审批或发布。
