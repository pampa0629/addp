...
// ListDimensionRelations 查询事实表的维度关联或维度表的被引用关系。
func (s *TableRelationService) ListDimensionRelations(factTableID, tenantID int64) ([]models.TableRelationDetail, error) {
	...
	if table.TableType != "fact" && table.TableType != "dimension" {
		return nil, apperrors.Validation("dimensional_table_required", i18n.MsgValidationFailed)
	}
	return s.repo.ListDetailsByTable(factTableID, tenantID)
}

// AddDimensionRelation 为事实表添加维度表关联
func (s *TableRelationService) AddDimensionRelation(factTableID, tenantID int64, req *models.SaveTableRelationRequest) (*models.TableRelationMutationResponse, error) {
	return s.saveDimensionRelation(0, factTableID, tenantID, req)
}

// UpdateDimensionRelation 原位更新事实表管理的维度关联。
func (s *TableRelationService) UpdateDimensionRelation(relationID, factTableID, tenantID int64, req *models.SaveTableRelationRequest) (*models.TableRelationMutationResponse, error) {
	return s.saveDimensionRelation(relationID, factTableID, tenantID, req)
}

func (s *TableRelationService) saveDimensionRelation(relationID, factTableID, tenantID int64, req *models.SaveTableRelationRequest) (*models.TableRelationMutationResponse, error) {
	if err := validateSaveTableRelationRequest(req); err != nil {
		return nil, err
	}
	...
	err := s.repo.DB().Transaction(func(tx *gorm.DB) error {
		...
		if factTable.Status != "draft" {
			return apperrors.Conflict("table_relation_state_conflict", i18n.MsgTableRelationSourceStateConflict)
		}
		...
		if relationID != 0 {
			if _, err := txRepo.GetByID(relationID, factTableID, tenantID); err != nil {
				return apperrors.NotFound("table_relation_not_found", i18n.MsgInvalidRelationID)
			}
		}
		exists, err := txRepo.Exists(factTableID, req.SourceField, req.TargetTable, req.TargetField, tenantID, relationID)
		if err != nil {
			return err
		}
		if exists {
		...
		if relationID == 0 {
			err = txRepo.Create(&response.Relation)
		} else {
			err = txRepo.Update(&response.Relation)
		}
		if err != nil {
...
func (s *TableRelationService) RemoveDimensionRelation(relationID, factTableID, tenantID, version int64) (*models.VersionResponse, error) {
	...
	err := s.repo.DB().Transaction(func(tx *gorm.DB) error {
		...
		if tables[factTableID].Status != "draft" {
			return apperrors.Conflict("table_relation_state_conflict", i18n.MsgTableRelationSourceStateConflict)
		}
