...
func TestModelListHandlersRejectInvalidQueryParameters(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Mermaid export domain type", path: "/entities/export-mermaid?domain_id=abc", handler: NewEntityHandler(nil).ExportMermaid},
		{name: "Mermaid export domain zero", path: "/entities/export-mermaid?domain_id=0", handler: NewEntityHandler(nil).ExportMermaid},
		{name: "Mermaid export repeated domain", path: "/entities/export-mermaid?domain_id=1&domain_id=2", handler: NewEntityHandler(nil).ExportMermaid},
