...
按请求选择步骤：解释已有 definition 时只检查相关定义和契约，缺少影响解释的事实时再查询；跟踪已有 execution 时调用 `execution.get`。以下完整流程用于生成或修改 definition。执行已有 definition 时复核资源事实、正式校验和审批要求，不重新生成它。

1. 明确任务是只设计、需要校验，还是需要实际执行。不要把“看看方案”理解成执行授权。
2. 调用 `engine.list` 确认可用的工作流运行时。用户指定实例时验证其存在且可用；未指定且候选不唯一时要求澄清。
3. 调用 `data.search` 对每个输入数据项做粗筛召回。若原始业务词和首轮跨语言技术名均未召回满足目标数据类型或能力的数据项，或结果只有文档内容命中，则把已尝试检索词和零召回事实反馈给大模型，只为缺失的数据角色补充新的常见单词型英文名、单复数、复合词或数据库标识符形式，再执行一次受限补充搜索；按 locator 合并去重。转换后的词只用于检索，不能当作资源事实。
4. 对粗筛候选调用 `resource.ancestors.get` 校验 locator，再调用 `data.preview` 获取受限字段、类型、CRS 和样本事实。候选较多时可以让大模型基于业务意图和这些已验证事实排序并标记推荐项，但不得删除仍然合理的候选。大模型不能生成资源身份，也不能替代用户对最终输入的确认；同一输入角色存在多个已验证候选时，必须完整展示并由用户选择一个。
