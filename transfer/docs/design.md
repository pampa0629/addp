...
| Planner | `backend/internal/planner` | 解析新 endpoint JSON，解析 System engine，生成 table transfer plan |
| Executor | `backend/internal/executor` | 按计划执行 table reader / transform / writer |
...
- `data_type` 第一阶段稳定支持 `table`。
- `representation` 支持 `native` 和 `encoded`。
- `format` 只用于 encoded endpoint。
- `policy.write_mode` 支持 `overwrite` 和 `append`。
