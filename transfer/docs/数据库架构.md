...
| resumable | PostgreSQL/MySQL source 的 watermark incremental 已通过 `transfer.sync_states` 支持 execution 间 resume，目标由幂等 `TableUpsertProvider` 能力选择，当前包括 PostgreSQL、MySQL 与非空间 OceanBase；snapshot checkpoint 仍只可观测。 |
