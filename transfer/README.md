...
`target.policy.apply_mode` 支持 snapshot 的 `replace` / `append`、watermark 和业务 Kafka 的幂等 `upsert`，以及数据库 CDC 的 `upsert_delete`。watermark 目标由 `table_upsert` capability 选择，当前支持 PostgreSQL、MySQL 与非空间 OceanBase。旧 `write_mode` 不兼容。snapshot replace 失败执行可按 restartable 从头 retry；watermark 和未阻塞的 continuous/CDC 从 `transfer.sync_states` committed position resume；append retry 被拒绝。数据库 CDC schema drift 会把任务置为 `status=blocked` 且不推进当前 offset。PostgreSQL/MySQL 中只有当前消息新增的 nullable 非 geometry 字段可由用户逐字段审批，平台复用目标 Provider 幂等加列并把任务置为 paused；Oracle 第一期的任何 schema drift 和其他不可审批变化均只能 Stop 后创建新任务和新目标表。
