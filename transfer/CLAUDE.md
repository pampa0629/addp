...
- bounded watermark source 必须消费声明 `bounded_watermark_read` 的 common engine Provider；PostgreSQL、MySQL 与 MySQL 模式 OceanBase 都在源数据库一致性快照内冻结复合上界，OceanBase 当前只开放非空间 InnoDB 基表。目标只按幂等 `TableUpsertProvider` 能力选择，当前 PostgreSQL、MySQL 与 MySQL 模式 OceanBase 可作为非空间目标；Transfer 不按源或目标引擎类型建立专用执行分支。
