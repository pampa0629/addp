...
- bounded watermark source 必须消费声明 `bounded_watermark_read` 的 common engine Provider；PostgreSQL、MySQL、MySQL 模式 OceanBase 与 openGauss 都在源数据库一致性快照内冻结游标上界。单字段游标仅同步新增且必须自身是非空唯一键，复合游标同步新增和更新且 tie breaker 必须是非空唯一键；OceanBase 限定非空间 InnoDB 基表，openGauss 当前只开放非空间表。目标只按幂等 `TableUpsertProvider` 能力选择，当前 PostgreSQL、MySQL、MySQL 模式 OceanBase 与 openGauss 可作为目标，其中 OceanBase 与 openGauss 只支持非空间表；Transfer 不按源或目标引擎类型建立专用执行分支。
