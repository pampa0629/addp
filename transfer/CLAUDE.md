...
- bounded watermark source 必须消费声明 `bounded_watermark_read` 的 common engine Provider；PostgreSQL、MySQL 与 MySQL 模式 OceanBase 都在源数据库一致性快照内冻结游标上界。单字段游标仅同步新增且必须自身是非空唯一键，复合游标同步新增和更新且 tie breaker 必须是非空唯一键；OceanBase 当前只开放非空间 InnoDB 基表。目标只按幂等 `TableUpsertProvider` 能力选择，当前 PostgreSQL、MySQL 与 MySQL 模式 OceanBase 可作为非空间目标；Transfer 不按源或目标引擎类型建立专用执行分支。
