  ...
  <el-dialog v-model="visible" title="定时调度配置" width="700px" @close="handleClose">
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="自定义" name="custom">
        <el-form label-width="100px" label-position="left">
          <el-form-item label="分钟">
            <el-radio-group v-model="customCron.minuteType" @change="updateCronExpression">
              <el-radio label="every">每分钟</el-radio>
              <el-radio label="interval">
                每
                <el-input-number v-model="customCron.minuteInterval" :min="1" :max="59" size="small"
                  @change="updateCronExpression" />
                分钟
              </el-radio>
              <el-radio label="specific">
                指定
                <el-select v-model="customCron.minuteValues" multiple placeholder="选择分钟" size="small"
                  style="width: 200px" @change="updateCronExpression">
          ...
          <el-form-item label="小时">
            <el-radio-group v-model="customCron.hourType" @change="updateCronExpression">
              <el-radio label="every">每小时</el-radio>
              <el-radio label="interval">
                每
                <el-input-number v-model="customCron.hourInterval" :min="1" :max="23" size="small"
                  @change="updateCronExpression" />
                小时
              </el-radio>
              <el-radio label="specific">
                指定
                <el-select v-model="customCron.hourValues" multiple placeholder="选择小时" size="small"
                  style="width: 200px" @change="updateCronExpression">
          ...
          <el-form-item label="日期">
            <el-radio-group v-model="customCron.dayType" @change="updateCronExpression">
              <el-radio label="every">每天</el-radio>
              <el-radio label="interval">
                每
                <el-input-number v-model="customCron.dayInterval" :min="1" :max="31" size="small"
                  @change="updateCronExpression" />
                天
              </el-radio>
              <el-radio label="specific">
                指定
                <el-select v-model="customCron.dayValues" multiple placeholder="选择日期" size="small"
                  style="width: 200px" @change="updateCronExpression">
          ...
          <el-form-item label="月份">
            <el-radio-group v-model="customCron.monthType" @change="updateCronExpression">
              <el-radio label="every">每月</el-radio>
              <el-radio label="specific">
                指定
                <el-select v-model="customCron.monthValues" multiple placeholder="选择月份" size="small"
                  style="width: 200px" @change="updateCronExpression">
          ...
          <el-form-item label="星期">
            <el-radio-group v-model="customCron.weekType" @change="updateCronExpression">
              <el-radio label="any">不限</el-radio>
              <el-radio label="specific">
                指定
                <el-select v-model="customCron.weekValues" multiple placeholder="选择星期" size="small"
                  style="width: 200px" @change="updateCronExpression">
      ...
      <el-tab-pane label="手动输入" name="manual">
        <el-input v-model="manualCronExpression" placeholder="输入 Cron 表达式，如：0 0 * * *" />
        <div class="hint">
    ...
    <div class="preview-section">
      <el-divider />
      <div class="preview-label">生成的 Cron 表达式：</div>
...
const manualCronExpression = ref('')
...
const currentExpression = computed(() => {
  if (activeTab.value === 'preset') {
    ...
    return manualCronExpression.value
  } else {
...
const expressionDescription = computed(() => {
  ...
  return describeCronExpression(currentExpression.value)
})
...
const buildCronExpression = () => {
  ...
  // 分钟
  if (customCron.value.minuteType === 'every') {
    parts.push('*')
  } else if (customCron.value.minuteType === 'interval') {
    parts.push(`*/${customCron.value.minuteInterval}`)
  } else {
    parts.push(customCron.value.minuteValues.join(',') || '*')
  }

  // 小时
  if (customCron.value.hourType === 'every') {
    parts.push('*')
  } else if (customCron.value.hourType === 'interval') {
    parts.push(`*/${customCron.value.hourInterval}`)
  } else {
    parts.push(customCron.value.hourValues.join(',') || '*')
  }

  // 日期
  if (customCron.value.dayType === 'every') {
    parts.push('*')
  } else if (customCron.value.dayType === 'interval') {
    parts.push(`*/${customCron.value.dayInterval}`)
  } else {
    parts.push(customCron.value.dayValues.join(',') || '*')
  }

  // 月份
  if (customCron.value.monthType === 'every') {
    parts.push('*')
  } else {
    parts.push(customCron.value.monthValues.join(',') || '*')
  }

  // 星期
  if (customCron.value.weekType === 'any') {
    parts.push('*')
  } else {
    parts.push(customCron.value.weekValues.join(',') || '*')
  }
...
// 描述 Cron 表达式
const describeCronExpression = (expr) => {
  try {
