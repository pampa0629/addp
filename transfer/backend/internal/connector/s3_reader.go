...
func (r *S3Reader) Open(ctx context.Context, config pipeline.ConnectorConfig) error {
	...
	r.bucket = getStringConfig(config, "bucket", "")
	r.prefix = getStringConfig(config, "prefix", "")
	r.pattern = getStringConfig(config, "pattern", "*")
	r.fileType = getStringConfig(config, "file_type", "json")
