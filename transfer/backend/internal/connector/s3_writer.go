...
func (w *S3Writer) Open(ctx context.Context, config pipeline.ConnectorConfig) error {
	...
	w.fileType = strings.ToLower(getStringConfig(config, "file_type", "json"))
	region := getStringConfig(config, "region", "us-east-1")
	useSSL := getBoolConfig(config, "use_ssl", false)
	...
	// 解析空间字段配置
	w.geometryFields = getStringSliceConfig(config, "geometry_fields")
	geometryField := strings.TrimSpace(getStringConfig(config, "geometry_field", ""))
	if geometryField != "" {
	...
	w.spatialFormat = strings.ToLower(getStringConfig(config, "spatial_format", ""))
...
func (w *S3Writer) openGeoJSONWriter(ctx context.Context, config pipeline.ConnectorConfig) error {
	...
	fileConfig := pipeline.ConnectorConfig{
		Config: map[string]interface{}{
			...
			"pretty":         getBoolConfig(config, "pretty", false),
		},
	}
