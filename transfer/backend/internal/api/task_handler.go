...
// @Description 创建 bounded、业务 Kafka continuous 或数据库 CDC 任务。业务 Kafka continuous 必须显式使用 runtime.record_failure.mode=block|dead_letter；dead_letter 只处理确定性记录级数据错误。数据库 CDC 支持 PostgreSQL/MySQL/Oracle 单表 initial_snapshot、block 和 PostgreSQL/MySQL 新目标表 upsert_delete；Oracle 第一期不支持 Spatial/LOB。旧 mode/write_mode 字段会被拒绝。| Create a bounded, business Kafka continuous, or database CDC task. Business Kafka continuous tasks must explicitly use runtime.record_failure.mode=block|dead_letter; dead_letter only handles deterministic record-level data errors. Database CDC supports a single PostgreSQL, MySQL, or Oracle source table with initial_snapshot, block policy, and a new PostgreSQL or MySQL upsert_delete target; Oracle Spatial and LOB are excluded from v1. Legacy mode/write_mode fields are rejected.
...
func (h *TaskHandler) StartTask(c *gin.Context) {
	...
	if err != nil {
		logger.L().Error("transfer task start failed", "task_id", id, "tenant_id", tenantID, "error", err)
		respondTaskServiceError(c, err)
		return
	}
