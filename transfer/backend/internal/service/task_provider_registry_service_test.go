...
func TestTaskProviderRegistryRegistersStandardTransferContract(t *testing.T) {
	...
	registry := NewTaskProviderRegistryService(server.URL, "internal-key", "http://transfer.internal")
	...
	if sync.DefinitionSchema["type"] != "object" || sync.ExecutionSchema["type"] != "object" {
		t.Fatalf("sync schemas must be object schemas: %#v", sync)
	}
}
