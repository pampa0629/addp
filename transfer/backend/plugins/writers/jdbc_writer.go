...
func (w *JDBCWriter) flushBuffer(ctx context.Context) error {
	...
	stmt, err := tx.PrepareContext(ctx, w.buildInsertSQL())
	if err != nil {
...
func (w *JDBCWriter) buildPostgresInsert() string {
	...
	for i, col := range w.columns {
		...
		if meta, ok := w.geometryColumns[col]; ok {
			if meta.SRID > 0 {
				placeholders[i] = fmt.Sprintf("CASE WHEN %s IS NULL THEN NULL ELSE ST_GeomFromWKB(%s, %d) END", placeholder, placeholder, meta.SRID)
			} else {
				placeholders[i] = fmt.Sprintf("CASE WHEN %s IS NULL THEN NULL ELSE ST_GeomFromWKB(%s) END", placeholder, placeholder)
			}
		} else {
