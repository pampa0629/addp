...
import (
	...
	commonShapefile "github.com/addp/common/format/plugins/shapefile"
...
func NewShapefileReader(config pipeline.ConnectorConfig) (pipeline.Reader, error) {
	...
	if err := utils.MapToStruct(config.Config, &shpConfig); err != nil {
...
func ensureRingClosed(points []shp.Point) []shp.Point {
	...
	if commonShapefile.AlmostEqual(first.X, last.X) && commonShapefile.AlmostEqual(first.Y, last.Y) {
		return points
	}
