...
func (r *FileReader) Open(ctx context.Context, config pipeline.ConnectorConfig) error {
	// 解析配置
	r.filePath = utils.GetStringConfig(config, "file_path", "")
	r.fileType = utils.GetStringConfig(config, "file_type", "csv")
	r.batchSize = utils.GetIntConfig(config, "batch_size", 1000)
	r.delimiter = rune(utils.GetStringConfig(config, "delimiter", ",")[0])
