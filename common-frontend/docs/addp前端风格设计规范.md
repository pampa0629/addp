...
业务域选择统一使用 `common-frontend/basic` 的 `BusinessDomainSelect`。下拉列表默认展开，按父域在前、子域紧随的顺序展示，每深入一级缩进 16px；父域与子域均可选。名称本身不拼接空格或树形符号。

搜索结果显示完整业务域名称路径，支持名称、编码和路径搜索；选中后输入框仅显示名称，不保留缩进，悬停显示完整路径。缩进不改变筛选语义，不自动把父域选择解释为包含所有子域。

Standard 是层级事实源。完整树由共享工具转换为带 `depth`、`path` 的选项；Catalog 分页候选及导航分面由后端返回权威路径，前端不得根据名称猜测层级。模块负责权限、加载和业务特殊选项，共享组件负责统一渲染与交互。
