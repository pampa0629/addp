...
for arg in "$@"; do
    ...
    case $arg in
        ...
        -h|--help)
            echo "使用方法: bash scripts/stop.sh [-postgres|-supermap-postgresql|-minio|-clickhouse|-mongodb|-doris|-spark|-neo4j|-mysql|-redpanda|-nfs|-all]"
            exit 0
            ;;
        -nfs)
        ...
        -*)
            case "$key" in
                postgres|supermap-postgresql|minio|clickhouse|mongodb|neo4j|mysql)
                    SERVICES+=("$key")
                    ;;
                redpanda)
