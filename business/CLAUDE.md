...
- KingbaseES V9R1C10 `V009R001C010B0004`：只在 owner-managed Linux x86_64 主机使用固定 SHA-256 的官方 Docker tar 和 owner 正规 License，以独立 `engine_type=kingbase` 注册；`business/scripts/kingbase.sh` 拥有无卷 disposable 容器与幂等普通关系样例，不进入日常 `start.sh -all`。首版只声明 PG 模式下经真实门禁验证的非空间目录、查询、读取、COPY 写会话、bounded watermark 与 `ON CONFLICT` upsert，不包含 PostGIS、CDC、PostgreSQL 扩展或 openGauss `MERGE`。
