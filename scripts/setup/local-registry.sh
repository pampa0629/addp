...
echo ""
echo "  2. 构建并推送镜像:"
echo "     ./scripts/deploy/1-build-images-multiarch.sh --registry $LOCAL_IP:$REGISTRY_PORT"
echo ""
echo "  3. 在服务器上拉取并启动:"
echo "     ./scripts/deploy/deploy-all.sh --server user@host --registry $LOCAL_IP:$REGISTRY_PORT --skip-build"
echo ""
echo "管理命令："
