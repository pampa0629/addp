...
# 2. 检查容器是否在运行
STATUS=$(docker compose -f docker-compose.yml ps system-backend --format json 2>/dev/null | jq -r '.State' 2>/dev/null || echo "unknown")
...
docker compose -f docker-compose.yml logs --tail=50 system-backend
echo ""

# 5. 检查错误日志
echo -e "${YELLOW}[4/8] 查找错误日志...${NC}"
docker compose -f docker-compose.yml logs system-backend | grep -i "error\|fatal\|panic" | tail -20 || echo "未发现明显错误"
echo ""

# 6. 测试内部健康检查
echo -e "${YELLOW}[5/8] 测试内部健康检查端点...${NC}"
HEALTH_RESPONSE=$(docker compose -f docker-compose.yml exec -T system-backend wget -qO- http://localhost:8080/health 2>&1 || echo "failed")
...
docker compose -f docker-compose.yml exec -T system-backend nc -zv postgres 5432 2>&1 || echo "无法连接到 PostgreSQL"
echo ""
echo "从 System Backend 到 Redis:"
docker compose -f docker-compose.yml exec -T system-backend nc -zv redis 6379 2>&1 || echo "无法连接到 Redis"
echo ""
...
echo "   docker compose -f docker-compose.yml restart system-backend"
echo ""
echo "2. 重启依赖服务:"
echo "   docker compose -f docker-compose.yml restart postgres redis"
echo "   sleep 30"
echo "   docker compose -f docker-compose.yml restart system-backend"
echo ""
echo "3. 查看实时日志:"
echo "   docker compose -f docker-compose.yml logs -f system-backend"
echo ""
echo "4. 完全重启:"
