...
# 查看系统库（PostgreSQL、Redis、MinIO、Elasticsearch）状态并做快速健康检查
...
docker compose ps postgres redis minio elasticsearch || true
...
ES_HTTP_PORT=$(get_port elasticsearch 9200 || echo 9200)

# Postgres
printf "- PostgreSQL (%s):  " "localhost:${PG_PORT}"
...
# Redis
printf "- Redis (%s):       " "localhost:${REDIS_PORT}"
...
# MinIO
printf "- MinIO (%s/%s):    " "API:${MINIO_API_PORT}" "Console:${MINIO_CONSOLE_PORT}"
...
# Elasticsearch
printf "- Elasticsearch (%s):" "localhost:${ES_HTTP_PORT}"
if curl -fs "http://localhost:${ES_HTTP_PORT}/_cluster/health?wait_for_status=yellow&timeout=2s" >/dev/null 2>&1; then
  echo -e " ${GREEN}Healthy${NC}"
else
  echo -e " ${RED}Unhealthy${NC}"
fi
