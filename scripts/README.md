...
├── relational-consumer-flow-online.py # OceanBase/openGauss 共用的关系引擎跨模块断言
...
专用部署默认只允许由 `.github/workflows/online-t4-gates.yml` 的手工 `workflow_dispatch` 触发；在正式 Suite 注册表声明 `nightly=True` 的已毕业 suite 还接受每日夜间 `schedule`。除 `opengauss-consumer-flow` 外，所有 suite 在带 `self-hosted`、`macOS`、`addp-online` 标签的 Runner 上调用 `online-host-gate.sh`，并显式排除定时事件；预检在不启停任何服务的前提下验证专用主机、仓库外环境文件与证据目录、显式 Tenant、suite profile、必要命令和干净工作区。`opengauss-consumer-flow` 在 GitHub Hosted Linux x86_64 Runner 上调用 `online-hosted-opengauss-gate.sh`，预检 Actions/OS/CPU 标记、仓库外 artifact/secret 分区、无根 `.env`、无容器冲突和干净 checkout，然后只使用当次 disposable 默认值构建整套环境。两种 profile 都产出不含密钥的 `readiness.txt`，共用唯一 `make test-online` 分发入口，并在退出时停止本次应用。self-hosted 只从 `ADDP_ONLINE_ENV_FILE` 加载专用外部环境；Hosted 必须额外销毁当次 openGauss、Infra 和凭据目录。`scripts/ci/check-online-ci-registration.py` 要求 suite、部署 profile、Runner 预检、workflow choices 与夜间登记完全一致，拒绝未毕业的定时 suite、非每日 cron、未固定 suite 输入或可能被定时事件误触发的其他 Job。
...
`scripts/ci/check-t2-ci-registration.py` 是 GitHub Hosted Runner 上 disposable T2 门禁的登记完整性检查。它从 `ADDP_T2_SERVICES`、`ADDP_T2_OWNED_SERVICES` 与 `ADDP_T2_HOSTED_ONLY` 元数据自动发现门禁，要求每条门禁同时具有根 `Makefile` 标准入口、对应的串行聚合入口、`release-and-t2-gates.yml` 调用和共享模块变更选择器。Workflow service 与 owner-managed Compose 中的镜像都必须按显式 tag 与 sha256 digest 固定；owner-managed gate 还必须声明唯一 Compose 文件、拥有 startup/cleanup，并清除 volumes、orphans 和本次 project 容器。名称含 `disposable` 的 PostgreSQL database 必须在 Service、健康检查和测试 DSN 中保持一致。Hosted-only 门禁必须由 owner 脚本独占创建和删除 disposable Docker 容器，且不得进入 macOS `test-integration`。ArcGIS 开放格式等需要专用样本的 T5 不属于该 Hosted Runner 契约。
