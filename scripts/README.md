...
`oceanbase-consumer-flow` 要求全量 System、Gateway、Meta、Transfer、Develop 和 Service，并使用 `business/scripts/online-oceanbase-consumer-fixture.sh` 管理独立 OceanBase CE。专用环境预置永久 `engine_type=oceanbase` Engine Instance；Fixture 固定维护 5 行 InnoDB watermark 源表和同构空目标表。suite 经 Gateway 和 Meta 获取正式 ResourceLocator，只创建一条 bounded watermark + upsert Transfer 任务，连续执行并精确断言 5 行首批、2 行增量和 0 行空增量；Develop 与临时 Query Service 读取同一目标表，最终必须得到相同的 6 行 checksum。任务和服务按捕获 ID 删除并确认 404，Fixture 在退出路径恢复源表基线和空目标表。该 suite 仅可从手工 workflow 运行，首次真实通过前不得增加 schedule。
