...
├── kingbase-official-media-release-gate.sh # KingbaseES 官方介质与 owner License 发布认证
├── kingbase-official-media-release-gate_test.py # KingbaseES 介质、License 和敏感暂存清理回归
...
├── online-owner-managed-kingbase-gate.sh # owner-managed Linux x86_64 KingbaseES T4 编排
├── relational-consumer-flow-online.py # OceanBase/TiDB/openGauss/KingbaseES 共用关系引擎断言
...
├── online-transfer-insert-only-fixture_test.py # Transfer PostgreSQL → MySQL 仅新增 Fixture 安全边界测试
├── online-manager-minio-fixture_test.py # Manager lineage 与文档快显专用 Business MinIO Fixture 安全边界测试
...
├── transfer-insert-only-mysql-online.py # 浏览器创建的 PostgreSQL → MySQL 仅新增任务验收
├── transfer-insert-only-mysql-online_test.py # Transfer 仅新增浏览器报告协议测试
...
该入口只启停 `addp-infra` Compose 项目并保留数据卷，不执行 Docker 全局清理。如果发现正在运行的 ADDP Infra，它会拒绝接管。依赖准备完成后，脚本会通过 `model/frontend` 锁定的 Playwright 版本幂等安装 Chromium；浏览器已存在时复用缓存，缺失或版本变化时自动下载。PostgreSQL 阶段会先清除调用方传入的门禁 DSN，再由巡检入口为全部已登记门禁显式注入 `addp_test` 或 `addp_iam_test`，不能依赖仓库根 `.env`。日志、成功 SHA 和运行锁位于 `.git/addp-local-ci/`，不进入工作树。Online 预检与恢复客户端的回环请求固定直连，不读取操作系统 HTTP 代理。该辅助巡检不替代 GitHub Actions，也不运行 T4/T5。
...
单模块交付使用 `make test-module MODULE=<模块>`。`scripts/test/module-gate.py` 从 Git 工作树中的 Go module、`*/frontend/package.json`、`*/pyproject.toml` 和声明了 `# ADDP_T2_SERVICES=<service,...>` 的 `scripts/test/*-gate.sh` 自动生成该模块的 T0-T3 执行计划：先运行 `test-platform`，再串行执行语言测试、前端测试与生产构建、已登记 T2；未声明该元数据的 T4/T5 gate 不会被误纳入模块门禁。必须由调用方提供连接条件的 T2 还要声明 `# ADDP_T2_REQUIRED_ENV=<name[|alternative],...>`，逗号表示同时需要、竖线表示可接受的等价安全条件；正式模块门禁会在启动任何步骤前统一检查，缺失时直接列出目标、变量，并为标准本地 PostgreSQL 门禁给出只指向 `addp_test` 或 `addp_iam_test` 的重试命令。CI 登记检查同步确认对应 Job 已显式提供每组条件中的至少一个变量。Go T1 会统一剔除 `*_POSTGRES_TEST_DSN` 和 `ADDP_*_INTEGRATION`，避免调用者为后续 T2 提供的连接条件提前激活真实基础设施测试；各 T2 仍只由 owner 的正式 gate 脚本显式启用。`--dry-run` 只用于检查计划，不要求真实连接条件；正式入口不跳过或兜底缺少连接条件的 T2。模块发现、环境预检与执行计划的确定性测试已纳入 `make test-platform`。
...
`quality-dynamic-binding` 通过专用 Runner 的 `-all` 部署，需显式配置 `SYSTEM_URL`、`GATEWAY_URL`、`MODEL_URL`、`META_URL`、`QUALITY_URL`、`ORCHESTRATOR_URL` 和非管理员 `ADDP_ONLINE_TEST_USER_ACCESS_TOKEN`。`ADDP_ONLINE_QUALITY_FIXTURES_JSON` 是恰好两个对象的 JSON 数组，每项仅含正整数 `logical_table_id`、规范表 `locator`、正整数 `row_count`。两个 Model 逻辑表必须已审批，绑定不同的已存在 PostgreSQL 物理表，结构相同且数据稳定；Model 必须拥有目标表标记，Meta 结构已扫描，执行身份具有相应 Engine DDL 与读取授权。夹具由专用环境维护，不从个人 Outdoor 业务环境复制凭据或注册信息。

入口为 `make test-online ONLINE_SUITE=quality-dynamic-binding`；在 `Online T4 gates` 手工选择同名 suite，首次真实通过前不加入 schedule。确定性回归及注册一致性检查使用 `make test-online-runner`，并由 `make test-platform` 和 Platform CI 覆盖。脚本使用正式 owner API 创建带 Run ID 的两条规则、两个方案、两个编排；依次验证失败、重复失败、另一目标失败、显式 R2 升级后逐目标恢复，以及默认绑定、工单和质量概览隔离。只读取物理数据，不执行 ETL；Model 任务只校验预置目标结构。
...
`metric-service-revision-lifecycle` 唯一运行于 GitHub Hosted `ubuntu-24.04`，从 `Online T4 gates` 手工选择同名 suite，并用 `metric_engine` 选择 `postgresql` 或 `tidb`；每轮只验收一个业务引擎，无需专用 Mac、预置 Tenant 或仓库 Secret。`online-hosted-metric-gate.sh` 复用 Hosted 生命周期及 System 临时身份入口，启动独立 Infra 与所选只读业务引擎。TiDB 使用固定 digest 的 PD/TiKV/TiDB 临时 Compose 集群；两种引擎均通过真实 API 扫描并建立 Standard 定义、逻辑表、关系和指标修订，复用同一指标契约和确定性结果。仅当次身份凭据与引擎描述写入 `runner.temp` 的 owner-only 文件，不接受永久业务环境提供的 ID 或数据。
...
报告 `metric-service-revision-lifecycle.json` 记录引擎类型、Run ID、实现/修订/服务 ID、校验结果与临时服务清理状态；Hosted `summary.txt` 另记录部署销毁结果。两者及脱敏日志由 Actions Artifact 保留 14 天，不归档凭据、连接密码、原始计划或查询结果正文。强制终止时不得声称清理通过，Runner 的临时 VM 回收承担最终隔离。确定性回归统一由 `make test-online-runner` 与 Platform CI 覆盖；两个引擎变体各自首次真实通过前不加入 schedule。

`workbench-service-consumption` 要求全量 System、Gateway、Service、Workbench 和 Console，并使用 `business/scripts/online-workbench-mysql-fixture.sh` 管理独立 Business MySQL。Fixture 只接受仓库外 `ADDP_ONLINE_WORKBENCH_MYSQL_*` 变量，不读取或创建 `business/.env`；它重建仓库已有确定性样例并把 Engine 使用的账号收敛为仅有 `SELECT` 的读取账号。suite 分离非管理员专用 User Token 与同 Tenant 的专用 Tenant Administrator Token：前者使用永久 MySQL Engine Instance，经 Gateway 检测 Service 输出契约，临时发布主 SQL 服务 `commerce-order-analysis` 和同契约未授权服务 `commerce-order-analysis-denied`，再经 Consumer Descriptor 直接创建含 Table、Chart 两个 Component 的未发布 Data Application；后者只经 System 正式 API 创建仅授权主服务的 API 消费方和一枚进程内凭据。API 层真实验证 cursor、动态筛选、标量类型、有限 CSV、无空间输出、契约指纹变化，以及 API Key 对已授权服务的 200、对未授权服务的 403、对控制面的 401 和删除后 45 秒内收敛失效；浏览器层以非管理员 User 登录 Console，在正式 Component 编辑器中验证两种 renderer 及契约变化后的预览阻断。Host Gate 安装专用 Chromium；完整 API Key 不得进入报告或日志，API 消费方及级联凭据、未发布 Data Application 与两个 Query Service 只按本轮创建 ID 在 `finally` 删除，不使用名称前缀或数据库清理，也不为自动化测试增加已发布应用强删旁路。

`oceanbase-consumer-flow` 要求全量 System、Gateway、Meta、Manager、Transfer、Develop 和 Service，并使用 `business/scripts/online-oceanbase-consumer-fixture.sh` 管理独立 OceanBase CE。专用环境预置永久 `engine_type=oceanbase` Engine Instance；Fixture 固定维护 5 行 InnoDB watermark 源表和同构空目标表。suite 经 Gateway 和 Meta 获取正式 ResourceLocator，只创建一条 bounded watermark + upsert Transfer 任务，连续执行并精确断言 5 行首批、2 行增量和 0 行空增量；Manager 表预览、Develop SQL 与临时 Query Service 通过各自正式数据出口读取同一目标表，最终必须得到相同的 6 行 checksum，Manager 还校验扫描列顺序。任务和服务按捕获 ID 删除并确认 404，Fixture 在退出路径恢复源表基线和空目标表。该 suite 仅可从手工 workflow 运行，首次真实通过前不得增加 schedule。

`opengauss-consumer-flow` 使用同一个 `relational-consumer-flow-online.py` 断言 Manager、Transfer、Develop 和 Service，上层流程只通过通用 Engine Plugin/Provider 和 SQLDialect 工作。由于 openGauss 6.0.6 官方容器的 NUMA 限制，该 suite 使用 GitHub Hosted `ubuntu-24.04` x86_64 profile：`online-hosted-opengauss-gate.sh` 通过正式 Infra 生命周期在镜像缺失时从 `scripts/infra/Dockerfile.postgres` 构建 x86_64 PostgreSQL 镜像，再从固定 SHA-256 官方介质创建独占 openGauss，启动 disposable `addp_online` Infra 及 Manager/Transfer/Develop/Service 参与链路；Business Fixture 只输出 owner-only Engine 描述，System IAM owner helper 创建非默认 Tenant、仅含租户可定制业务 Permission 的最小消费 User，并把一次性 Engine Provisioner 绑定到正式内置 `tenant.infrastructure_administrator` Role。通用 `online-engine-registration.py` 使用 Provisioner Token 通过正式 System API 注册并验证当次 Engine Instance，随后立即从业务断言环境清除该 Token 与数据库凭据；consumer 不访问 System Engine 控制面，只验证 Manager、Transfer、Develop 和 Service 的消费路径，证据另写入 artifact 目录。成功、失败和中断均停止应用、删除 openGauss 容器并销毁 Infra 数据卷。它不读取根 `.env`、Business `.env`、GitHub Environment 或外部实例。2026-09-10 的 [Online T4 run 34469670386](https://github.com/pampa0629/addp/actions/runs/34469670386) 已确认构建身份、5/2/0 watermark、三出口同 checksum、清理和零残留全部通过，因此当前同时保留手工 `workflow_dispatch`，并在每日 `20:30 UTC` 只调度该 Hosted profile。

`tidb-consumer-flow` 与 OceanBase/openGauss 复用同一个 `relational-consumer-flow-online.py`，只新增 `engine_type=tidb` 的 database catalog profile，不在 Manager、Transfer、Develop 或 Service 增加数据库分支。macOS 专用 Runner 预置永久 TiDB Engine Instance，并在仓库外环境文件提供 `ADDP_ONLINE_TIDB_ENGINE_ID`、`ADDP_ONLINE_TIDB_PORT`、`ADDP_ONLINE_TIDB_DATABASE`、`ADDP_ONLINE_TIDB_USER` 和可为空的 `ADDP_ONLINE_TIDB_PASSWORD`；`online-tidb-consumer-fixture.sh` 复用 T2 的固定 digest、无卷 PD/TiKV/TiDB Compose，重建 5 行源和空目标，推进 2 行 watermark 后由同一 Transfer 任务得到 5/2/0 读取计数，并由 Manager、Develop、Service 比较 6 行同 checksum。任务、服务和三组件在退出路径清理并验证零残留。首次真实通过前仅允许手工 `workflow_dispatch`，不得加入 schedule。

`transfer-insert-only-mysql` 要求全量 System、Gateway、Meta、Transfer 和 Console，并使用 `business/scripts/online-transfer-insert-only-fixture.sh` 同时管理专用 PostgreSQL 源表和可写 MySQL 目标库。suite 以真实 User 登录 Console，经 Transfer 向导选择两端资源；字段映射阶段必须调用正式推荐接口，把无声明精度的 `numeric` 当前值分析为 `DECIMAL(6,2)`，配置阶段再选择仅新增并自动派生 `id` 水位和目标键。页面创建后首轮必须写入 6 行；Fixture 修改旧 `id=1` 并新增 `id=7` 后，从任务详情再次执行只能写入 1 行，最终目标共 7 行、旧值不变、新值存在且无重复键。任务按捕获 ID 删除并确认 404，复合 Fixture 在所有退出路径删除固定表并停止两个容器。该 suite 仅可从手工 workflow 运行，首次真实通过前不得增加 schedule。

`manager-internal-artifact-lineage` 要求全量 System、Gateway、Meta、Manager、Monitor、Console、PointCloud Runtime 和 Document Workflow Runtime，并使用 `business/scripts/online-manager-minio-fixture.sh` 管理独立 Business MinIO。专用环境长期预置指向该 MinIO 的 Engine Instance；Fixture 把仓库已有小型 LAS 与确定性的 3 页 PPTX 幂等写入配置的两个 object。suite 以真实 User 经 Gateway 执行一次 Meta scan，使用扫描事实创建并运行 `point_cloud_copc_generation`，同时通过 Quick View Capability 声明的 `generate_pptx_pdf` action 触发 `pptx_pdf_generation`；两类执行均验证 Manager 与 Monitor 所见 `addp.lineage-facts/v1` 完全一致、infra 输出支持 Range 读取。PPTX 首次 action 必须返回受理状态，完成后再读取 Capability 必须复用同一任务和结果并返回 3 页 PDF。浏览器先验证 Monitor 血缘，再在 Data Explorer 翻到 PDF 第 2 页，等待一次周期性引擎状态刷新，确认页码保持且已就绪预览不再发起 PPTX 生成 action。两类结果与任务只通过 Manager API 按本轮 ID 删除，并验证内容 404 与零残留；不直接清理数据库或对象存储。
...
专用部署默认只允许由 `.github/workflows/online-t4-gates.yml` 的手工 `workflow_dispatch` 触发；在正式 Suite 注册表声明 `nightly=True` 的已毕业 suite 还接受每日夜间 `schedule`。常规 suite 在带 `self-hosted`、`macOS`、`addp-online` 标签的 Runner 上调用 `online-host-gate.sh`，并显式排除定时事件；`opengauss-consumer-flow` 和 `metric-service-revision-lifecycle` 在 GitHub Hosted Linux x86_64 Runner 上分别调用 `online-hosted-opengauss-gate.sh`、`online-hosted-metric-gate.sh`，共用 `scripts/utils/hosted-online.sh` 生命周期；`kingbase-consumer-flow` 只在带 `self-hosted`、`Linux`、`X64`、`addp-kingbase` 标签且受 `addp-kingbase` Environment 审批的 owner-managed Runner 上调用 `online-owner-managed-kingbase-gate.sh`。三种 profile 都在生命周期操作前验证仓库外 artifact/secret 分区、无根 `.env`、无容器冲突和干净 checkout，产出不含密钥内容的 `readiness.txt`，共用唯一 `make test-online` 分发入口。常规 self-hosted 只从 `ADDP_ONLINE_ENV_FILE` 加载专用外部环境；KingbaseES 只从 owner-only `ADDP_KINGBASE_GATE_ENV_FILE` 加载 License 路径、License SHA-256 等受控事实；Hosted 与 KingbaseES profile 必须额外销毁当次业务引擎、Infra 和凭据目录。`scripts/ci/check-online-ci-registration.py` 要求 suite、部署 profile、Runner 预检、workflow choices 与夜间登记完全一致，拒绝未毕业的定时 suite、非每日 cron、未固定 suite 输入或可能被定时事件误触发的其他 Job。

Runner 管理员应以根 `.env.example` 为字段清单，在仓库外创建独立环境文件并替换全部 Secret；不能把该文件复制为仓库根 `.env`。除专用 Infra 连接和所有内置 `*_SERVICE_CLIENT_SECRET` 外，至少显式配置 `ADDP_ONLINE_TEST=1`、`ADDP_ONLINE_TEST_TENANT_ID`、`POSTGRES_DB=addp_online`、`SYSTEM_URL`、`GATEWAY_URL`、`MANAGER_URL`、`META_URL`、`SECURITY_URL`、`TRANSFER_URL`、`CATALOG_URL`、`ASSET_URL`、`PORTAL_URL`、`DEVELOP_URL`、`SERVICE_URL`、`WORKBENCH_URL`、`CONSOLE_URL`、`STANDARD_URL`、`MODEL_URL`。`module-registry-recovery` 使用其中的 `MANAGER_SERVICE_CLIENT_SECRET`；`standard-model-reference-deletion` 使用 `ADDP_ONLINE_TEST_USER_ACCESS_TOKEN`；`consumer-engine-recovery` 另外要求同一专用 User 的 `ADDP_ONLINE_TEST_USER_USERNAME`、`ADDP_ONLINE_TEST_USER_PASSWORD`，以及稳定 `ADDP_ONLINE_TEST_ENGINE_ID`、名称、端口、用户、密码和数据库；`enterprise-catalog-publishing` 同样使用该专用 User 的用户名和密码，并需预置永久 Domain 和 Department ID；`workbench-service-consumption` 另需同 Tenant 专用 `ADDP_ONLINE_TEST_TENANT_ADMIN_ACCESS_TOKEN`、永久 `ADDP_ONLINE_WORKBENCH_MYSQL_ENGINE_ID`，以及端口、数据库、只读用户、只读密码和 root fixture 密码，其浏览器阶段仍使用非管理员专用 User 的用户名和密码；`transfer-insert-only-mysql` 复用永久 PostgreSQL Engine Instance，另需永久 `ADDP_ONLINE_TRANSFER_MYSQL_ENGINE_ID`、名称、端口、数据库、可写用户、可写密码和仅供 Fixture 使用的 root 密码，浏览器使用同一专用 User 的用户名和密码；`oceanbase-consumer-flow` 另需永久 `ADDP_ONLINE_OCEANBASE_ENGINE_ID`，以及该实例对应的端口、database、租户限定用户和密码，专用 User 只需具备 Meta、Manager、Transfer、Develop 和 Service 所需最小权限，其中包括 `manager.data_item.read`；Engine 控制面状态由部署 profile 负责验证，业务 consumer 不需要 `system.engine.read`；`manager-internal-artifact-lineage` 另需永久 `ADDP_ONLINE_MANAGER_MINIO_ENGINE_ID` 以及独立 MinIO 端口、凭据、bucket、LAS object 和 PPTX object，并复用同一专用 User 完成 API 与浏览器验收；`security-transfer-protection` 另需永久 `ADDP_ONLINE_SECURITY_MONGODB_ENGINE_ID`、MongoDB 端口、数据库、只读账号，以及仅供 Fixture 使用的 root 账号；`security-plaintext-access` 复用永久 PostgreSQL Engine Fixture，并要求 `ADDP_ONLINE_TEST_USER_ACCESS_TOKEN` 对应的申请用户具备 Manager 预览及原值访问申请权限、`ADDP_ONLINE_TEST_APPROVER_ACCESS_TOKEN` 对应的另一名用户具备审批和撤销权限；`security-mysql-owner-protection` 复用永久 MySQL 和 PostgreSQL Engine Instance，需要敏感定义创建/删除、邮箱治理读取、Finding 复核与四个 Owner 的最小读取/执行权限。GitHub Environment `addp-online` 的 Repository Variable `ADDP_ONLINE_ENV_FILE` 只保存该仓库外文件的绝对路径，不保存文件内容。
...
`scripts/ci/check-t2-ci-registration.py` 是 disposable T2 门禁的登记完整性检查。它从 `ADDP_T2_SERVICES`、`ADDP_T2_OWNED_SERVICES`、`ADDP_T2_HOSTED_ONLY` 与 `ADDP_T2_OWNER_MANAGED` 元数据自动发现门禁，要求每条门禁具有根 `Makefile` 标准入口和对应的串行聚合入口；前三类还必须登记 `release-and-t2-gates.yml` 调用和共享模块变更选择器。Workflow service 与 gate-owned Compose 中的镜像都必须按显式 tag 与 sha256 digest 固定；Compose owner 必须声明唯一 Compose 文件、拥有 startup/cleanup，并清除 volumes、orphans 和本次 project 容器。名称含 `disposable` 的 PostgreSQL database 必须在 Service、健康检查和测试 DSN 中保持一致。Hosted-only 门禁不得进入本地 `test-integration`；License 受控的 owner-managed 门禁只进入人工执行的 `test-integration-owner-managed`，登记检查拒绝 workflow 调用其目标或聚合入口。此类门禁仍必须使用外部 owner-only 环境文件、官方介质与 License SHA-256、完整 disposable Docker 生命周期和证据归档。ArcGIS 开放格式等需要专用样本的 T5 不属于该 T2 契约。
...
T5 发布认证的统一入口是 `make test-release RELEASE_SUITE=<suite>`。当前登记 `common-python-cli`、`agent-evaluation`、`opengauss-official-media`、`kingbase-official-media` 和 `dameng-official-media` 五个已有真实 owner 门禁的套件，不以占位项冒充发布认证。分发器把 suite 映射到 owner 的唯一 Make 目标，强制使用仓库外空证据目录，并在成功或失败时生成 `addp.release-gate/v1` 的 `release-report.json` 与 GitHub Step Summary 可直接消费的 `release-summary.md`；报告只记录 suite、owner 目标、结果、耗时与产物相对路径，不保存 Token 或在线证据正文。声明了 `workflow_job` 的 suite 由通用登记检查验证统一入口、证据目录和摘要，不能由 workflow 绕过 owner 分发器；未声明的 suite 不由 GitHub Actions 编排，登记检查拒绝 workflow 调用其 suite 或内部目标。Agent 仍由具备三份在线证据的外部发布流程调用。

发布工作流安全审计统一使用 `make test-workflow-security`：`scripts/test/workflow-security-gate.py` 在仓库外临时虚拟环境安装固定版本 zizmor 1.28.0 的二进制 wheel（首次安装需要网络），只扫描 `.github/workflows/release-and-t2-gates.yml`。本地 `make test-platform` 和 CI 的 System IAM required Job 均调用这一入口，保持 auditor、medium 最低严重级别、关闭在线审计，不加载配置或忽略标记；安装失败、输入缺失、扫描失败或达到阻断级别的发现都会返回非零。临时环境在退出时清理，不安装到业务 Python 运行时，也不启动服务或连接数据库。审计脚本回归和本地/CI 入口唯一性检查纳入 `make test-release-runner`。
...
`kingbase-official-media` 只在持有正规 License 的 owner 受控 Linux x86_64 主机人工运行。KingbaseES 的 T2 Provider 与 T5 官方介质认证暂不接入 GitHub Actions；保留 `make test-common-kingbase`、`make test-integration-owner-managed` 和 `make test-release RELEASE_SUITE=kingbase-official-media` 标准入口及确定性测试。真实认证仍待具备授权环境后执行，脚本测试通过不等于认证通过。owner 脚本固定下载 KingbaseES V9R1C10 `V009R001C010B0004` 官方 Docker tar 并校验 SHA-256，通过仓库外 owner-only `ADDP_KINGBASE_GATE_ENV_FILE` 注入正规 License 路径与 License SHA-256，使用 `docker create` 后注入授权再启动无卷容器。门禁验证 PG 模式下的 `lib/pq`、COPY、`ON CONFLICT`、复合 watermark、目录和取消契约；证据只归档介质与 License 摘要，不归档 License、密码或 tar，退出时删除本次容器、镜像和临时介质。重新接入自动 CI 前必须另行确认 Runner 隔离与生命周期设计，不能用 Environment 审批或忽略审计告警替代。此次调整不修改 Online T4 的已有入口。

`dameng-official-media` 只接受 Docker Server `linux/arm64`，固定下载达梦 `dm8_20260708_HWarm920_kylin10_sp1_64.zip` 并校验 ZIP 与内层 ISO SHA-256，在本机生成不可推送的无卷镜像和 disposable 容器。DM8 服务端、官方 ODBC `libdodbc.so` 与带 build tag 的 Go ODBC bridge 测试都运行在 Linux ARM64 容器边界内，验证版本、试用 License 到期日、参数绑定、事务、复合 watermark、`MERGE` 幂等写入和样例初始化；报告只保存摘要，不归档介质、镜像或官方驱动文件，退出时删除本次容器、镜像、网络和临时介质。独立 Provider T2 使用 `make test-common-dameng` 复用已启动的 Business DM8，从同一镜像提取并校验 SHA-256 为 `96516ecd7c9ec3c405109b81135f2c9d87ed0631632e99cd2de0e06f03b1392e` 的官方 Go 驱动，只注入一次性 Linux ARM64 Docker 测试进程；测试进程、驱动副本和测试表清理后，Business 数据库按开发需要继续运行。该路线是 macOS ARM64 Docker Desktop 可运行的非认证 ABI、试用期技术验证，不登记 Hosted 或 schedule，不证明通用 ARM64、正式麒麟部署、生产认证或长期 License 可用性。
