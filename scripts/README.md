...
`manager-internal-artifact-lineage` 要求全量 System、Gateway、Meta、Manager、Monitor、Console、PointCloud Runtime 和 Document Workflow Runtime，并使用 `business/scripts/online-manager-minio-fixture.sh` 管理独立 Business MinIO。专用环境长期预置指向该 MinIO 的 Engine Instance；Fixture 把仓库已有小型 LAS 与确定性的 3 页 PPTX 幂等写入配置的两个 object。suite 以真实 User 经 Gateway 执行一次 Meta scan，使用扫描事实创建并运行 `point_cloud_copc_generation`，同时通过 `pptx_pdf/preview` 自动触发 `pptx_pdf_generation`；两类执行均验证 Manager 与 Monitor 所见 `addp.lineage-facts/v1` 完全一致、infra 输出支持 Range 读取。PPTX 首次请求必须返回受理状态，完成后的第二次请求必须复用同一任务和结果并返回 3 页 PDF。浏览器先验证 Monitor 血缘，再在 Data Explorer 翻到 PDF 第 2 页，等待一次周期性引擎状态刷新，确认页码保持且浏览器仅发起一次 PPTX preview 请求。两类结果与任务只通过 Manager API 按本轮 ID 删除，并验证内容 404 与零残留；不直接清理数据库或对象存储。
