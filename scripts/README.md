...
`scripts/ci/check-t2-ci-registration.py` 是 disposable T2 门禁的登记完整性检查。它从 `ADDP_T2_SERVICES`、`ADDP_T2_OWNED_SERVICES`、`ADDP_T2_HOSTED_ONLY` 与 `ADDP_T2_OWNER_MANAGED` 元数据自动发现门禁，要求每条门禁同时具有根 `Makefile` 标准入口、对应的串行聚合入口、`release-and-t2-gates.yml` 调用和共享模块变更选择器。Workflow service 与 gate-owned Compose 中的镜像都必须按显式 tag 与 sha256 digest 固定；Compose owner 必须声明唯一 Compose 文件、拥有 startup/cleanup，并清除 volumes、orphans 和本次 project 容器。名称含 `disposable` 的 PostgreSQL database 必须在 Service、健康检查和测试 DSN 中保持一致。Hosted-only 门禁不得进入本地 `test-integration`；License 受控的 owner-managed 门禁只进入 `test-integration-owner-managed`，并必须使用受保护 self-hosted Linux x86_64 Job、外部 owner-only 环境文件、官方介质与 License SHA-256、完整 disposable Docker 生命周期和证据归档。ArcGIS 开放格式等需要专用样本的 T5 不属于该 T2 契约。
...
`kingbase-official-media` 只在带 `self-hosted`、`Linux`、`X64`、`addp-kingbase` 标签并受 `addp-kingbase` Environment 审批的 owner-managed Runner 上手工运行。owner 脚本固定下载 KingbaseES V9R1C10 `V009R001C010B0004` 官方 Docker tar 并校验 SHA-256，通过仓库外 owner-only `ADDP_KINGBASE_GATE_ENV_FILE` 注入正规 License 路径与 License SHA-256，使用 `docker create` 后注入授权再启动无卷容器。门禁验证 PG 模式下的 `lib/pq`、COPY、`ON CONFLICT`、复合 watermark、目录和取消契约；证据只归档介质与 License 摘要，不归档 License、密码或 tar，退出时删除本次容器、镜像和临时介质。首次真实通过前不登记 Tag 或 schedule 自动触发。
