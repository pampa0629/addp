...
`metric-service-revision-lifecycle` 唯一运行于 GitHub Hosted `ubuntu-24.04`，从 `Online T4 gates` 手工选择同名 suite，无需专用 Mac、预置 Tenant 或仓库 Secret。`online-hosted-metric-gate.sh` 复用 Hosted 生命周期及 System 临时身份入口，启动独立 Infra 与只读 PostgreSQL 业务引擎，通过真实 API 扫描并建立 Standard 定义、逻辑表、关系和指标修订。夹具参数与确定性预期结果由套件代码固定；仅当次身份凭据与引擎描述写入 `runner.temp` 的 owner-only 文件，不接受永久业务环境提供的 ID 或数据。
...
报告 `metric-service-revision-lifecycle.json` 记录 Run ID、实现/修订/服务 ID、校验结果与临时服务清理状态；Hosted `summary.txt` 另记录部署销毁结果。两者及脱敏日志由 Actions Artifact 保留 14 天，不归档凭据、连接密码、原始计划或查询结果正文。强制终止时不得声称清理通过，Runner 的临时 VM 回收承担最终隔离。确定性回归统一由 `make test-online-runner` 与 Platform CI 覆盖；首次真实通过前不加入 schedule。
