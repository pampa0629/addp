...
当前唯一正式分发路径是 GitHub Release。推送与包版本一致的 `v<version>` Tag 会在同一次工作流中重新运行上述两项门禁；两项均成功后，发布 Job 只下载 `addp-cli-wheel` artifact，校验便携 SHA-256、包名和 wheel `METADATA` 版本，再创建 Release。发布 Job 不检出源码、不重新构建 wheel。PyPI 或私有包仓库待账号、权限和发布策略明确后另行设计。
