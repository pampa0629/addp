...
专用部署只允许由 `.github/workflows/online-t4-gates.yml` 的手工 `workflow_dispatch` 触发。除 `opengauss-consumer-flow` 外，所有 suite 在带 `self-hosted`、`macOS`、`addp-online` 标签的 Runner 上调用 `online-host-gate.sh`；预检在不启停任何服务的前提下验证专用主机、仓库外环境文件与证据目录、显式 Tenant、suite profile、必要命令和干净工作区。`opengauss-consumer-flow` 在 GitHub Hosted Linux x86_64 Runner 上调用 `online-hosted-opengauss-gate.sh`，预检 Actions/OS/CPU 标记、仓库外 artifact/secret 分区、无根 `.env`、无容器冲突和干净 checkout，然后只使用当次 disposable 默认值构建整套环境。两种 profile 都产出不含密钥的 `readiness.txt`，共用唯一 `make test-online` 分发入口，并在退出时停止本次应用。self-hosted 只从 `ADDP_ONLINE_ENV_FILE` 加载专用外部环境；Hosted 必须额外销毁当次 openGauss、Infra 和凭据目录。`scripts/ci/check-online-ci-registration.py` 要求 suite、部署 profile、Runner 预检和 workflow choices 完全一致，并在首次真实运行通过前禁止增加 `schedule`。
