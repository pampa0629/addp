...
if docker service ls --format '{{.Name}}' | grep -q "addp_transfer-worker"; then
    echo "🔧 Transfer Worker 详情 (高可用):"
    docker service ps addp_transfer-worker --no-trunc
    echo ""

    # 获取副本数
    REPLICAS=$(docker service ls --format '{{.Replicas}}' --filter name=addp_transfer-worker)
    echo "副本状态: $REPLICAS"
    echo ""
fi
...
echo ""
echo "回滚服务:"
echo "  docker service rollback addp_transfer-worker"
echo ""
