...
if ! grep -q "transfer/backend" scripts/dev-start.sh 2>/dev/null; then
    echo -e "${RED}✗ dev-start.sh 未包含 Transfer Backend${NC}"
    ERRORS=$((ERRORS + 1))
else
    echo -e "${GREEN}✓ dev-start.sh 包含 Transfer Backend${NC}"
fi

if ! grep -q "transfer/frontend" scripts/dev-start.sh 2>/dev/null; then
    echo -e "${RED}✗ dev-start.sh 未包含 Transfer Frontend${NC}"
    ERRORS=$((ERRORS + 1))
else
    echo -e "${GREEN}✓ dev-start.sh 包含 Transfer Frontend${NC}"
fi

if ! grep -q "transfer.pid" scripts/dev-stop.sh 2>/dev/null; then
    echo -e "${RED}✗ dev-stop.sh 未包含 Transfer 停止逻辑${NC}"
    ERRORS=$((ERRORS + 1))
else
    echo -e "${GREEN}✓ dev-stop.sh 包含 Transfer 停止逻辑${NC}"
fi
echo ""
