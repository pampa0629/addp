...
HOSTED_FIXTURE_CONTAINERS=(registry addp-online-public-origin-upstreams system-backend gateway console system-frontend meta-backend meta-frontend manager-backend manager-frontend transfer-backend transfer-frontend addp-nginx geopython-workflow-engine business-minio)
...
run_logged make build BUILD_ARGS="--arch amd64 --services system-backend,gateway,meta-backend,manager-backend,transfer-backend"
run_logged make build-images IMAGE_BUILD_ARGS="--verify --services system-backend,gateway,meta-backend,manager-backend,transfer-backend,console,system-frontend,meta-frontend,manager-frontend,transfer-frontend,nginx,geopython-workflow-engine"
...
run_logged docker run -d --name addp-online-public-origin-upstreams \
  ...
  --network-alias orchestrator-frontend \
