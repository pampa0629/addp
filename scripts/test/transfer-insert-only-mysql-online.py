#!/usr/bin/env python3
"""Accept the browser-created PostgreSQL -> MySQL insert-only Transfer flow."""
...
SUPPORT_SPEC = importlib.util.spec_from_file_location("transfer_insert_only_support", SUPPORT_PATH)
...
TASK_PREFIX = "addp_online_insert_only_"
...
def validate_user_identity(client: GatewayClient, tenant_id: int) -> dict[str, object]:
    ...
    if principal.get("type") != "user":
        raise SuiteError("Online Transfer token must belong to a User")
    ...
    if tenant.get("type") != "tenant" or tenant.get("tenant_id") != str(tenant_id):
        raise SuiteError("Online Transfer token must use the configured Tenant Context")
    if token.get("type") not in {"first_party_access_token", "oauth_access_token"}:
        raise SuiteError("Online Transfer token must be a User Access Token")
    ...
    if forbidden:
        raise SuiteError("Online Transfer token must not use administrator roles: " + ", ".join(sorted(forbidden)))
    missing = REQUIRED_PERMISSIONS - permissions
    if missing:
        raise SuiteError("Online Transfer token is missing required permissions: " + ", ".join(sorted(missing)))
    return {
...
def validate_engine(
    ...
    while time.monotonic() < deadline:
        ...
        if engine.get("engine_type") != expected_type:
            raise SuiteError(f"configured Engine Instance {engine_id} must use engine_type={expected_type}")
...
def validate_browser_report(report: object, run_id: str, tenant_id: str, expected_task_name: str) -> dict[str, object]:
    payload = _object(report, "Transfer insert-only browser report")
    expected = {
    ...
    if mismatches:
        raise SuiteError("Transfer insert-only browser report contract mismatch: " + ", ".join(mismatches))
    return payload
...
def run_browser(repository: Path, environment: Mapping[str, str], expected_task_name: str) -> dict[str, object]:
    ...
    report_path = artifact_dir / "transfer-insert-only-mysql-browser.json"
    report_path.unlink(missing_ok=True)
    browser_environment = dict(environment)
    browser_environment.update(
        {
            "ADDP_ONLINE_REPOSITORY": str(repository),
            "ADDP_ONLINE_TRANSFER_TASK_NAME": expected_task_name,
            "ADDP_ONLINE_TRANSFER_SOURCE_TABLE": SOURCE_TABLE,
            "ADDP_ONLINE_TRANSFER_TARGET_TABLE": TARGET_TABLE,
        }
    )
    ...
    if not report_path.is_file():
        raise SuiteError("Playwright did not write transfer-insert-only-mysql-browser.json")
...
def main() -> int:
    ...
    try:
        ...
        source_engine_id = positive_int(required_environment("ADDP_ONLINE_TEST_ENGINE_ID"), "ADDP_ONLINE_TEST_ENGINE_ID")
        target_engine_id = positive_int(required_environment("ADDP_ONLINE_TRANSFER_MYSQL_ENGINE_ID"), "ADDP_ONLINE_TRANSFER_MYSQL_ENGINE_ID")
        ...
        source_engine = validate_engine(
            client,
            source_engine_id,
            "postgresql",
            required_environment("ADDP_ONLINE_TEST_ENGINE_NAME"),
            deadline,
        )
        target_engine = validate_engine(
            client,
            target_engine_id,
            "mysql",
            required_environment("ADDP_ONLINE_TRANSFER_MYSQL_ENGINE_NAME"),
            deadline,
        )
        stale = suite_task_ids(client)
        if stale:
            raise SuiteError("stale Transfer insert-only Online tasks exist before the run")
        source_scan = wait_for_scan(client, source_engine_id, deadline)
        target_scan = wait_for_scan(client, target_engine_id, deadline)
        find_item(client, source_engine_id, f"public.{SOURCE_TABLE}", "table")
        ...
        if residual:
            raise SuiteError("browser left the owned Transfer task behind")
        report = {
            "schema_version": "addp.transfer-insert-only-mysql-online/v1",
            "suite": "transfer-insert-only-mysql",
            "run_id": run_id,
            "result": "passed",
            "identity": identity,
        ...
        if client is not None and owned_name:
            try:
                ...
                print(f"Transfer insert-only Online cleanup failed: {cleanup_error}", file=sys.stderr)
        print(f"Transfer insert-only MySQL Online failed: {error}", file=sys.stderr)
        return 1
