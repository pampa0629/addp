#!/usr/bin/env bash
# Owner-managed Linux lifecycle for the licensed KingbaseES cross-module T4 gate.
...
case "${1:-}" in
  ...
  *) fail "usage: bash scripts/test/online-owner-managed-kingbase-gate.sh [--check-only]" ;;
esac
...
[ "$(uname -s)" = "Linux" ] && [ "$(uname -m)" = "x86_64" ] || fail "KingbaseES Online requires Linux x86_64"
...
[ "${ONLINE_SUITE_INPUT:-$ONLINE_SUITE}" = "$ONLINE_SUITE" ] || fail "this lifecycle only owns $ONLINE_SUITE"
[ ! -e "$ROOT_DIR/.env" ] || fail "owner-managed Online forbids a repository root .env"
[ -z "$(git -C "$ROOT_DIR" status --porcelain)" ] || fail "owner-managed Online requires a clean checkout"
...
[ "$ADDP_ONLINE_ARTIFACT_DIR" != "$ADDP_ONLINE_SECRET_DIR" ] || fail "artifact and secret directories must be different"
...
[ -f "$ROOT_DIR/scripts/infra/Dockerfile.postgres" ] || fail "missing repository Infra PostgreSQL Dockerfile"

for container in addp-postgres addp-redis addp-minio addp-meilisearch addp-redpanda addp-kingbase-online-disposable; do
  docker container inspect "$container" >/dev/null 2>&1 && fail "refusing to reuse existing container: $container"
done
...
GATE_LOG="$ADDP_ONLINE_ARTIFACT_DIR/online-gate.log"
{
  ...
  printf 'suite=%s\n' "$ONLINE_SUITE"
  printf 'runner=owner-managed-linux-x86_64\n'
  printf 'database=addp_online\n'
  printf 'media_sha256=%s\n' "$KINGBASE_OFFICIAL_MEDIA_SHA256"
  printf 'license_sha256=%s\n' "$ADDP_KINGBASE_LICENSE_SHA256"
...
if [ "$MODE" = "check-only" ]; then
  echo "Owner-managed KingbaseES Online readiness check passed"
  exit 0
fi
...
finish() {
  ...
  if [ "$fixture_owned" -eq 1 ]; then
    run_logged bash business/scripts/online-kingbase-consumer-fixture.sh stop || cleanup=failed
  fi
...
export ADDP_ONLINE_OWNER_MANAGED=1
...
export ADDP_KINGBASE_MEDIA_CACHE="$ADDP_ONLINE_SECRET_DIR/kingbase-media-cache"
export ADDP_ONLINE_FIXTURE_ENGINE_DESCRIPTOR_FILE="$ADDP_ONLINE_SECRET_DIR/kingbase-engine.json"
IDENTITY_ENV="$ADDP_ONLINE_SECRET_DIR/identity.env"
ENGINE_RESULT_ENV="$ADDP_ONLINE_SECRET_DIR/engine.env"
unset ADDP_TEST_KINGBASE_DSN ADDP_TEST_KINGBASE_HOST ADDP_TEST_KINGBASE_PORT
unset ADDP_TEST_KINGBASE_DATABASE ADDP_TEST_KINGBASE_USER ADDP_TEST_KINGBASE_PASSWORD
...
fixture_owned=1
run_logged bash business/scripts/online-kingbase-consumer-fixture.sh start
application_owned=1
...
export ADDP_ONLINE_CONSUMER_ENGINE_TYPE=kingbase
