...
func TestDataValidationAuthorizationUsesCurrentLeaseBoundary(t *testing.T) {
	...
	executor.ConfigureDataValidation(nil)
	...
	execution := &commonExecution.TaskExecution{
		...
		TaskType: commonExecution.TaskTypeDataValidation, Status: commonExecution.ExecutionStatusRunning,
	...
	config := &dataValidationExecutionConfig{
		TaskVersion:       1,
		ParentExecutionID: parentExecutionID,
		TableBindings:     []DataValidationTableBinding{{Alias: "participation", Locator: "addp://engine/12/path/public/table_3?type=table"}},
		Assertions: DataValidationAssertionDocument{
			SchemaVersion: dataValidationSchemaVersion,
			Assertions: []DataValidationAssertion{{
	...
	_, _ = executor.doDataValidation(context.Background(), task, execution, lease, config)
