...
func TestValidateDataValidationContractRejectsUnknownFieldsAndDuplicateBindings(t *testing.T) {
	document := json.RawMessage(`{"schema_version":"addp.quality.data-validation/v1","assertions":[{"assertion_key":"f3889a4a-1675-4623-b6e3-773f9125a04d","type":"not_null","severity":"error","params":{"table":"orders","column":"id","sql":"drop table x"}}]}`)
	_, err := validateDataValidationContract([]DataValidationTableBinding{{Alias: "orders", Locator: "addp://engine/12/path/public/table_3?type=table"}}, document)
	...
	_, err = validateDataValidationContract([]DataValidationTableBinding{{Alias: "orders", Locator: "addp://engine/12/path/public/table_3?type=table"}, {Alias: "orders_copy", Locator: "addp://engine/12/path/public/table_3?type=table"}}, valid)
	if err == nil || !strings.Contains(err.Error(), "physical table binding is duplicated") {
		t.Fatalf("duplicate logical table error = %v", err)
	}
}

func TestValidateDataValidationAllowedValues(t *testing.T) {
	binding := []DataValidationTableBinding{{Alias: "orders", Locator: "addp://engine/12/path/public/table_3?type=table"}}
	valid := gateTestDocument(`{"table":"orders","column":"status","values":["enabled","disabled"]}`, "allowed_values")
	if _, err := validateDataValidationContract(binding, valid); err != nil {
	...
	for name, params := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			if _, err := validateDataValidationContract(binding, gateTestDocument(params, "allowed_values")); err == nil || !strings.Contains(err.Error(), "allowed_values params are invalid") {
...
func TestCompileDataValidationUsesOnlyReadContextIdentifiers(t *testing.T) {
	config := &dataValidationExecutionConfig{
		TableBindings: []DataValidationTableBinding{{Alias: "orders", Locator: "addp://engine/12/path/public/table_3?type=table"}, {Alias: "customers", Locator: "addp://engine/12/path/public/table_7?type=table"}},
	...
	if _, _, err := compileDataValidation(config, readContext); err == nil || !strings.Contains(err.Error(), "not present") {
...
func gateTestDocument(params, assertionType string) json.RawMessage {
	return json.RawMessage(`{"schema_version":"addp.quality.data-validation/v1","assertions":[{"assertion_key":"f3889a4a-1675-4623-b6e3-773f9125a04d","type":"` + assertionType + `","severity":"error","params":` + params + `}]}`)
}

func TestDataValidationRejectsCrossEngineAndDuplicatePhysicalIdentity(t *testing.T) {
	...
	for _, other := range []string{"addp://engine/13/path/public/table_3?type=table", "addp://engine/12/path/public/table_3?type=table&item_id=42"} {
		_, err := validateDataValidationContract([]DataValidationTableBinding{{Alias: "orders", Locator: "addp://engine/12/path/public/table_3?type=table"}, {Alias: "other", Locator: other}}, doc)
		if err == nil {
