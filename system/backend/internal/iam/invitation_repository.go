...
func (r *Repository) CreateEnrollmentTicket(ctx context.Context, ticket *EnrollmentTicket) error {
	if ticket == nil {
		return fmt.Errorf("%w: enrollment ticket is required", commonapi.ErrBadRequest)
	}
	return wrapRepositoryError(r.db.WithContext(ctx).Create(ticket).Error)
}

func (r *Repository) LockEnrollmentTicketByHash(
	ctx context.Context,
	tokenHash string,
) (*EnrollmentTicket, error) {
	var ticket EnrollmentTicket
	err := r.db.WithContext(ctx).Clauses(clause.Locking{Strength: "UPDATE"}).
...
func (r *Repository) GetEnrollmentTicketByHash(
	ctx context.Context,
	tokenHash string,
) (*EnrollmentTicket, error) {
	var ticket EnrollmentTicket
	err := r.db.WithContext(ctx).Where("token_hash = ?", tokenHash).Take(&ticket).Error
	if err != nil {
		return nil, wrapRepositoryError(err)
	}
	return &ticket, nil
...
func (r *Repository) ConsumeEnrollmentTicket(ctx context.Context, ticketID int64, consumedAt time.Time) error {
	result := r.db.WithContext(ctx).Model(&EnrollmentTicket{}).
	...
	if result.RowsAffected != 1 {
		return fmt.Errorf("%w: enrollment ticket is already consumed", commonapi.ErrConflict)
	}
	return nil
}
