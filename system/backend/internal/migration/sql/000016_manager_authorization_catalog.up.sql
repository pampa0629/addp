BEGIN;

INSERT INTO system.permissions (
    permission_key,
    owner_module,
    action,
    risk_level,
    delegable,
    allowed_scope_types,
    tenant_customizable,
    name_i18n_key,
    description_i18n_key,
    status
) VALUES (
    'manager.derived_artifact.update',
    'manager',
    'update',
    'medium',
    false,
    ARRAY['tenant', 'department', 'project_group']::text[],
    true,
    'permissions.manager.derived_artifact.update.name',
    'permissions.manager.derived_artifact.update.description',
    'active'
);

INSERT INTO system.role_permissions (
    role_id,
    permission_id,
    source_type,
    created_by_principal_id
)
SELECT role.id, permission.id, 'product', NULL
FROM system.roles AS role
JOIN system.permissions AS permission
  ON permission.permission_key = 'manager.derived_artifact.update'
WHERE role.tenant_id IS NULL
  AND role.role_key = 'tenant.data_steward'
  AND role.role_type = 'tenant_builtin'
  AND role.status = 'active';

COMMIT;
