...
CREATE INDEX idx_notebook_session_authorizations_actor_active
    ON system.notebook_session_authorizations (actor_principal_id, tenant_id, expires_at)
    WHERE revoked_at IS NULL;
CREATE INDEX idx_notebook_session_authorizations_membership_active
    ON system.notebook_session_authorizations (tenant_membership_id, expires_at)
    WHERE revoked_at IS NULL;
CREATE INDEX idx_notebook_session_authorizations_family_active
    ON system.notebook_session_authorizations (token_family_id, expires_at)
    WHERE revoked_at IS NULL;
CREATE INDEX idx_notebook_session_authorizations_expiry_active
    ON system.notebook_session_authorizations (expires_at, id)
    WHERE revoked_at IS NULL;

CREATE FUNCTION system.validate_notebook_session_authorization()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE FUNCTION system.validate_notebook_session_authorization_update()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
ALTER TABLE system.execution_authorizations
    ADD COLUMN source_notebook_session_authorization_id uuid
...
CREATE INDEX idx_execution_authorizations_notebook_session_active
...
CREATE FUNCTION system.revoke_notebook_session_execution_authorizations()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
