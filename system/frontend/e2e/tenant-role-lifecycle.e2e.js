...
test('tenant administrator manages a custom role with localized bulk permission selection', async ({ page }) => {
  ...
  await expect(page.getByText('租户服务账号可在“应用接入”中配置角色，平台运行账号仅可查看。')).toBeVisible()
  ...
  await expect(row).not.toContainText('端到端测试角色')

  await row.getByRole('button', { name: /1 项/ }).click()
  const drawer = page.getByRole('dialog', { name: '角色详情' })
  await expect(drawer).toContainText('端到端测试角色')
  await expect(drawer).toContainText('租户自定义角色')
  await expect(drawer).toContainText('用户账号')
  await expect(drawer).toContainText('部门')
  await expect(drawer).toContainText('权限明细')
  ...
  await expect(row).not.toContainText('可查看智能体任务和数据项')
  await row.getByRole('button', { name: 'E2E 高级研究员', exact: true }).click()
  const updatedDrawer = page.getByRole('dialog', { name: '角色详情' })
  await expect(updatedDrawer).toContainText('可查看智能体任务和数据项')
