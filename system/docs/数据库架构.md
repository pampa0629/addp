...
| 平台配置 | `engines`, `api_consumers`, `api_consumer_service_grants`, `api_consumer_credentials`, `module_definitions`, `module_runtime_instances`, `module_registry_state` | API Consumer 仅授权已发布数据面服务；`engines` 与模块注册表由 System 统一 migration 管理；模块定义带并发 `version`，运行实例由租约推进健康，注册表状态维护路由拓扑 revision |
