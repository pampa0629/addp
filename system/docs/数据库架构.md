...
- Module Definition 保存模块持久身份和管理员 `enabled` 意图；Module Runtime Instance 保存进程角色、端点和短期租约，两者生命周期分离。
...
| 平台配置 | `engines`, `applications`, `api_keys`, `module_definitions`, `module_runtime_instances` | `engines` 与模块注册表由 System 统一 migration 管理；模块定义带并发 `version`，运行实例由租约推进健康 |
...
模块定义是可变聚合根，所有管理员启停操作必须在同一条条件更新中校验并递增 `version`。运行实例不建立独立版本；注册和心跳只维护 owner 声明及租约，不得覆盖管理员 `enabled`。平台管理 API 只读实例投影，不提供手工健康写入。
