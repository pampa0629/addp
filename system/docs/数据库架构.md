...
- **认证中心**:提供 JWT 认证服务,所有模块通过 System 验证用户身份
- **配置中心**:管理平台共享配置(JWT Secret、加密密钥、地图 API Key 等)
- **租户中心**:提供多租户隔离机制
- **审计中心**:记录所有模块的操作日志
...
4. 验证用户名和密码 (bcrypt)
   ↓
5. 生成 JWT Token (包含 user_id、username、tenant_id、user_type)
   ↓
6. 返回 Token 给前端
   ↓
7. 前端存储 Token (localStorage)
   ↓
8. 后续请求携带 Token (Authorization: Bearer <token>)
   ↓
9. 中间件验证 Token (AuthMiddleware)
   ↓
10. 提取用户信息注入 Context
```
...
```
1. Manager 前端请求 (带 JWT Token)
   ↓
2. Manager 后端验证 Token (通过 System 的 AuthMiddleware)
   ↓
3. System 验证 Token 签名和有效期
   ↓
4. 提取 user_id、tenant_id 注入 Context
   ↓
5. Manager 业务逻辑处理 (自动应用租户隔离)
   ↓
6. Manager 返回响应
   ↓
...
| `engines` | `scan_config` | 存储元数据扫描配置(定时调度、扫描深度等) |
| `engines` | `capabilities` | 存储引擎能力声明(支持的操作、数据格式等) |
| `engines` | `extension_api_config` | 存储扩展引擎的 API 配置(端点、超时、映射) |
| `engines` | `health_check_config` | 存储健康检查配置(端点、间隔、超时) |
...
CREATE UNIQUE INDEX idx_engines_identifier ON system.engines(unique_identifier); -- 能力查询
...
- JWT Token 黑名单:`system:jwt:blacklist:{token_id}`,TTL = Token 剩余有效期
...
**Go 内存缓存**:
- JWT Public Key 缓存(避免重复读取文件)
- 加密密钥缓存(避免重复 Base64 解码)
...
- [engines 表](./tables/engines表.md) - 引擎配置表,数据源管理
