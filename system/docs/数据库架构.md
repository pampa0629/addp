...
- **配置中心**:管理允许跨模块共享的配置；`JWT_SECRET` 仅属于 System 私有签名配置
- **租户中心**:提供多租户隔离机制
- **审计中心**:记录所有模块的操作日志
...
- **Token 有效期**:由 `JWT_EXPIRE_MINUTES` 配置，当前默认 180 分钟
- **权威解析**:`GET /api/v1/system/auth/context` 在验证 Token 后回查用户和租户激活状态
- **刷新机制**:当前第一方 Token 刷新将在 OAuth 阶段替换为可轮换、可撤销的 Refresh Token Family
