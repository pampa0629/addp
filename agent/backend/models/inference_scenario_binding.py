"""Agent-owned Inference Scenario Binding."""
...
class InferenceScenarioBinding(Base):
    ...
    __table_args__ = (
        CheckConstraint("scope_type IN ('platform', 'tenant')", name="ck_agent_inference_binding_scope"),
        CheckConstraint(
            "(scope_type = 'platform' AND tenant_id IS NULL) OR "
            "(scope_type = 'tenant' AND tenant_id IS NOT NULL)",
            name="ck_agent_inference_binding_tenant",
        ),
        Index(
            "idx_agent_inference_binding_platform",
            "scenario_code",
            unique=True,
            postgresql_where=text("scope_type = 'platform' AND tenant_id IS NULL"),
        ),
        Index(
            "idx_agent_inference_binding_tenant",
            "scenario_code",
            "tenant_id",
            unique=True,
