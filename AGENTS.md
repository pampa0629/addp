...
This monorepo splits shared assets into `common/` and service-specific code under `system/`, `manager/`, `meta/`, `transfer/`, `gateway/`, and `portal/`. Backends follow the Go layout `cmd/`, `internal/`, and `pkg/`, while Vue frontends live in `system/frontend` and `portal/frontend` with seeding data in `system/data`. Place integration scripts in `scripts/`, reference docs in `docs/`, and keep orchestration artifacts—`Makefile`, `docker-compose.yml`—at the root. Mirror the existing layout when adding services, and colocate tests alongside the code they cover. 门户(`portal/frontend`)仅负责聚合各业务模块界面，诸如元数据扫描等业务页面必须落在对应模块前端（如 `meta/frontend`），避免在门户内直接实现。

## Build, Test, and Development Commands
Run `make init` once after cloning to copy `.env.example` values and prepare data folders. Use `make dev-system` or `cd system/backend && go run cmd/server/main.go` to launch the primary API, and `cd system/frontend && npm install && npm run dev` for the UI. `make up`/`make up-full` boot the Docker stack; `make status` inspects container health. Build binaries with `make build`, produce container images using `make docker-build` or `make docker-build-all`, and rely on `make fmt`, `make lint`, and `make test` to format, lint, and validate Go code across the repo. 对于日常开发，请优先使用`scripts/`目录下的本地脚本来启动或调试服务，避免动用 Docker 以节省时间。门户界面自身不承载业务页面，需通过 iframe 组合各前端模块，meta 模块也要从`meta/frontend`加载，避免再使用内嵌组件。
