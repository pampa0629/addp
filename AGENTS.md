...
ADDP 处于积极开发阶段，当前更重视概念统一、规范遵从、架构优雅和代码简洁，不为兼容旧实现保留额外负担。

一，文档优先，先统一概念
- 优先阅读并遵守仓库规范，尤其是 `docs/spec/` 和 `docs/concepts/`。
- 遇到概念不清、架构不合理、代码与规范冲突或影响长期演进的问题，先停下来与用户讨论确认。
- 涉及概念或规范调整时，先修订对应文档，再基于文档实现。

二，回到本质，保持克制
- 从原始需求和问题本质出发，不套用惯例或模板。
- 动机、目标或设计路径不清晰时，主动追问；路径明显不优时，直接指出并给出更简单的方案。
- 修复问题要追根因，不做临时补丁；架构层面的根因必须主动提出讨论。
...
- 过时的代码、数据和逻辑应直接删除，不保留兼容分支。
- 临时脚本和临时文档放到操作系统临时目录，保持项目树整洁。
- 未经用户明确授权，不创建分支、不提交代码。

四，效率优先
- 每次启动开发工作，特别是再有工作清单的情况下，持续开发，中间不要停，直到完成全部任务或者遇到必须要讨论的地方
