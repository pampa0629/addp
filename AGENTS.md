...
- 对可通过只读检索、现有规范或标准验证查清的事实，先调查；仍存在会改变业务结果、概念、模块边界、权限或操作范围的歧义时，暂停依赖该决定的工作并确认。其余独立工作继续，不猜测、不补丁式兜底、不绕路实现。
...
- 新发现且需要改变既定架构、概念或模块边界的设计与根因必须主动讨论；已确认方案内的实现继续执行。

五，完成标准
- 验收范围和标准入口遵循 `docs/spec/addp测试与验收规范.md`；工作区变更默认运行 `make test-changed`，明确单个 owner 时运行 `make test-module MODULE=<模块>`，共享依赖按规范扩散验证。
- 新逻辑只能有一条主路径
- 旧路径删除完成
- 受影响的必需门禁通过；未运行、失败或跳过项分别说明，不计为通过。
- 涉及 API 契约或文档事实变化时同步 Swagger/文档；不涉及则不要求修改。
- 每次代码、功能、Bug 修复、依赖或环境变更，都必须在实施前识别受影响的测试层级和 CI/CD 门禁；需要适配的实现、测试、根 `Makefile` 标准入口、`.github/workflows/` 编排及相关文档必须同次完成。现有入口和自动发现已覆盖时确认覆盖即可，不为同步而修改文件，不能把 CI/CD 适配留到推送失败后再补。
...
需要运行验收或用户要求启动体验时，使用标准生命周期脚本；仅测试或构建验证不要求重启。不要用 `go run` 或 `go build` 直接运行临时二进制，也不要自行创建 `bin` 目录。需要重启时：
