...
- 编排定义和执行记录是租户资源。HTTP Handler 只能使用 System AuthContext 中的非零 `tenant_id`，Repository 的 Get/Update/Delete 和 Execution 查询必须带租户条件；平台上下文和缺少 `X-Tenant-ID` 的内部调用不得回退到租户 1、query 参数或全租户访问。
