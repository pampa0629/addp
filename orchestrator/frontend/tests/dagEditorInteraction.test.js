...
test('DAG editor uses shared direct connection and viewport controls', async () => {
  ...
  const nodeSource = await readFile(resolve('src/components/orchestrationEditorNode.js'), 'utf8')

  assert.match(source, /createDAGDirectEdgeBehavior\(\{/)
  assert.match(source, /validateDAGConnection\(\{/)
  assert.match(source, /resolveSource: event => orchestrationPort\(event, 'output'\)/)
  assert.match(source, /resolveTarget: event => orchestrationPort\(event, 'input'\)/)
  assert.match(source, /type: 'click-select', selectEdge: true/)
  assert.match(source, /edgeKind: 'control'/)
  assert.match(source, /edgeKind: 'parameter'/)
  assert.match(source, /endArrow: edgeEndArrow\(canvasColors\.warning\)/)
  assert.match(source, /endArrow: edgeEndArrow\(canvasColors\.primary\)/)
  assert.match(source, /path: 'M 0,0 L 9,4 L 9,-4 Z'/)
  assert.match(source, /d: -10/)
  assert.match(source, /parameterBindings\(step\.parameters, target\.inputPorts\)/)
  assert.match(source, /!step\.depends_on\.includes\(edge\.source\)/)
  assert.match(source, /clearCopiedParameterBindings\(item\)/)
  ...
  assert.doesNotMatch(source, /modulesApi\.listTasksByModule/)
  assert.match(source, /ensureTaskRuntimeMetadataForSteps\(steps\)/)
  assert.match(source, /await ensureTaskRuntimeMetadata\(nodeData\.provider, nodeData\.taskType, nodeData\.taskId\)/)
  assert.doesNotMatch(source, /stateStyles: selectedNodeStateStyles/)
  assert.doesNotMatch(source, /'text-shape'/)
  assert.match(nodeSource, /setState\(name, selected, item\)/)
  assert.match(nodeSource, /orchestration-node-card/)
})
