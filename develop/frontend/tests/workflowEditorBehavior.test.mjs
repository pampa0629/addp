...
const workflowCanvas = await readFile(resolve('src/components/workflow/WorkflowDAGCanvas.vue'), 'utf8')
...
assert.equal((editor.match(/class="addp-dialog"/g) || []).length, 6)
...
assert.match(paramsPanel, /const resetBaseline = ref\(\{\}\)/)
assert.match(paramsPanel, /watch\(\[\(\) => props\.operator, \(\) => props\.nodeId\], \(\) => \{/)
assert.match(paramsPanel, /resetBaseline\.value = \{ \.\.\.props\.initialParams \}/)
assert.match(paramsPanel, /formData\.value = \{ \.\.\.resetBaseline\.value \}/)
...
assert.match(editor, /submitWorkflowGeneration\(collectWorkflowResources\(\)\)/)
assert.match(editor, /confirmedResources\([\s\S]*resourceCandidates\.value,[\s\S]*selectedResourceCandidatesByRole\.value/)
assert.match(editor, /resourceConfirmationVisible\.value = false[\s\S]*resourceCandidates\.value = \[\][\s\S]*selectedResourceCandidatesByRole\.value = \{\}/)
assert.match(editor, /v-for="group in resourceCandidateGroups"/)
assert.match(editor, /<el-radio-group[\s\S]*selectedResourceCandidatesByRole\[group\.role\]/)
assert.match(editor, /candidate\.recommended/)
assert.doesNotMatch(editor, /if \(workflowData\.value\.tasks\.length\) resourceConfirmationVisible\.value = false/)
...
assert.match(paramsPanel, /normalizeParamValue\(param, paramValue\)/)
assert.match(paramsPanel, /JSON\.parse\(value\)/)
assert.match(paramsPanel, /dataType === 'array' && !Array\.isArray\(parsed\)/)
assert.match(paramsPanel, /param\.type \|\| param\.data_type/)
assert.match(workflowCanvas, /normalizeWorkflowParams\(/)
assert.match(workflowCanvas, /JSON\.parse\(value\)/)
assert.match(workflowCanvas, /parameter\?\.type \|\| parameter\?\.data_type/)
assert.match(workflowCanvas, /removeStaleInputRefs\(model\.params, model\.publicParameters\)/)
