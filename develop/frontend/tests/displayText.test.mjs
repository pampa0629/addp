...
assert.equal(zhCnMessages.develop.queryResult.exportFull, '导出全部结果')
assert.equal('downloadPreviewCsv' in zhCnMessages.develop.queryResult, false)
assert.match(queryEditor, /<ExportDialog/)
assert.equal(queryEditor.includes('target_parent_locator'), false)
assert.match(queryEditor, /createQueryExport\(executionID, \{/)
assert.match(queryResult, /canExportFullResult/)
assert.match(queryResult, /props\.fullExportSupported/)
assert.match(queryResult, /emit\('export-full'\)/)
assert.equal(queryResult.includes('downloadPreviewCSV'), false)
assert.equal(queryResult.includes("t('develop.queryResult.exportCsv')"), false)
