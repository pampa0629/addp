...
// @Param scope query string false "生命周期视图，current 为保护中，released 为已退出，all 为全部；默认 current | Lifecycle view: current for in-progress protection, released for exited history, all for both; current by default" Enums(current,released,all) default(current)
