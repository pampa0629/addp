...
# MinIO Client (mc)
mc alias set addp-minio http://localhost:9000 minioadmin minioadmin
```
...
**连接信息**:
```
API Endpoint:      http://localhost:9002  (⚠️ 注意：不是 9000)
Console URL:       http://localhost:9003  (⚠️ 注意：不是 9001)
Access Key:        minioadmin
Secret Key:        minioadmin
Default Bucket:    addp-data
```
