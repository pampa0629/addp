...
func coverageResponsibilityPredicate(role string) string {
...
func coveragePredicateFor(dimension string) (governanceCoveragePredicate, bool) {
	...
	case CoverageDimensionBusinessOwner:
		return governanceCoveragePredicate{applicable: coverageAllEntriesPredicate, covered: coverageResponsibilityPredicate(models.ResponsibilityRoleBusinessOwner)}, true
	case CoverageDimensionDataSteward:
...
func (s *EntryService) GetGovernanceCoverage(ctx context.Context, tenantID int64, access EntryAccess) (*GovernanceCoverage, error) {
	...
	accountableDepartment, _ := coveragePredicateFor(CoverageDimensionAccountableDepartment)
