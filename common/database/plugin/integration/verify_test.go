...
func TestPluginInterfaceImplementation(t *testing.T) {
	...
	for dbType, p := range plugins {
		t.Run(dbType, func(t *testing.T) {
			...
			if p.ConnectionCategory() == "" {
				t.Error("ConnectionCategory() should not be empty")
			}
			...
			case "storage":
				...
				if !ok {
					t.Errorf("storage engine should implement StoragePlugin")
					return
				}
			...
			case "compute":
				...
				if !ok {
					t.Errorf("compute engine should implement ComputePlugin")
					return
				}
				t.Logf("%s implements ComputePlugin ✓", dbType)

			default:
				t.Errorf("unknown ConnectionCategory: %s", category)
			}
		})
	}
}
...
func TestRelationalDBPlugins(t *testing.T) {
	...
	for _, dbType := range dbTypes {
		t.Run(dbType, func(t *testing.T) {
			...
			// 验证类别
			if p.ConnectionCategory() != "storage" {
				t.Errorf("expected category 'storage', got '%s'", p.ConnectionCategory())
			}
