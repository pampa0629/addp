...
func (p *ClickHousePlugin) ConnectionCategory() string {
	return "relational_db" // OLAP 分析型数据库
}
...
func (p *ClickHousePlugin) GenerateCapabilities() string {
	return `{"storage":[{"type":"relational_db","engine":"clickhouse","supports_query":true}],"compute":[{"type":"sql_query","description":"OLAP分析查询","dev_modes":["sql"],"features":["columnar","distributed","real_time"]}]}`
}
