...
func TestTableDescribeConversionsKeepDatatypeFields(t *testing.T) {
	...
	result := TableDescribeResultFromTableInfo(tableInfo)
	if len(result.Table.Fields) != 2 {
		t.Fatalf("TableDescribeResultFromTableInfo() fields len = %d, want 2", len(result.Table.Fields))
	}
	...
	roundTrip := TableInfoFromDescribeResult(result)
	if len(roundTrip.Fields) != 2 {
		t.Fatalf("TableInfoFromDescribeResult() fields len = %d, want 2", len(roundTrip.Fields))
	}
