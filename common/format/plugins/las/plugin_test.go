...
func TestDescribePointCloudReadsLASHeader(t *testing.T) {
	result, err := NewPlugin().DescribePointCloud(context.Background(), bytes.NewReader(buildLASHeader()), nil)
	if err != nil {
...
func buildLASHeader() []byte {
	...
	copy(buf[:4], []byte(lasMagic))
	buf[24] = 1
	buf[25] = 4
	copy(buf[26:58], []byte("ADDP"))
	copy(buf[58:90], []byte("unit-test"))
	binary.LittleEndian.PutUint16(buf[94:96], uint16(lasMaxHeaderRead))
