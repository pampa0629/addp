...
func TestKingbaseOfficialMediaCertification(t *testing.T) {
	if os.Getenv("ADDP_KINGBASE_CERTIFICATION") != "1" {
		t.Skip("set ADDP_KINGBASE_CERTIFICATION=1 to run the KingbaseES official-media certification")
	}

	dsn := strings.TrimSpace(os.Getenv("ADDP_TEST_KINGBASE_DSN"))
	if dsn == "" {
		t.Fatal("ADDP_TEST_KINGBASE_DSN is required")
	}
	...
	if !strings.Contains(strings.ToLower(version), "kingbasees v009r001c010") {
		t.Fatalf("version() = %q, want KingbaseES V009R001C010", version)
	}

	var databaseMode string
	if err := db.QueryRowContext(ctx, "SHOW database_mode").Scan(&databaseMode); err != nil {
		t.Fatalf("query database_mode error = %v", err)
	}
	if strings.ToLower(strings.TrimSpace(databaseMode)) != "pg" {
		t.Fatalf("database_mode = %q, want pg", databaseMode)
	}

	const targetTable = "addp_kingbase_certification_target"
	const stagingTable = "addp_kingbase_certification_staging"
	...
	if _, err := db.ExecContext(ctx, `
		CREATE TABLE addp_kingbase_certification_target (
	...
	if _, err := db.ExecContext(ctx, `
		CREATE TABLE addp_kingbase_certification_staging (
			id BIGINT,
			tenant_id BIGINT,
	...
	prepared, err := db.PrepareContext(ctx, `
		INSERT INTO addp_kingbase_certification_target
	...
	baseTime := time.Date(2026, time.September, 12, 8, 0, 0, 123000000, time.UTC)
	if _, err := prepared.ExecContext(ctx, int64(1), int64(7), "KB-001", "首次写入", "12.34", baseTime); err != nil {
		_ = prepared.Close()
	...
	for _, row := range []struct {
		...
		{id: 1, externalID: "KB-001", name: "批量更新", amount: "56.78", updatedAt: baseTime.Add(time.Microsecond)},
		{id: 2, externalID: "KB-002", name: "批量新增", amount: "90.12", updatedAt: baseTime.Add(2 * time.Microsecond)},
	} {
	...
	if _, err := db.ExecContext(ctx, `
		INSERT INTO addp_kingbase_certification_target
			(id, tenant_id, external_id, name, amount, updated_at)
		SELECT id, tenant_id, external_id, name, amount, updated_at
		FROM addp_kingbase_certification_staging
		ON CONFLICT (id) DO UPDATE SET
			name = EXCLUDED.name,
			amount = EXCLUDED.amount,
			updated_at = EXCLUDED.updated_at
	`); err != nil {
		t.Fatalf("ON CONFLICT upsert error = %v", err)
	}

	rows, err := db.QueryContext(ctx, `
		SELECT id, name, amount::text
		FROM addp_kingbase_certification_target
