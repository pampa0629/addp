...
func TestMySQLAnalyticalScanContracts(t *testing.T) {
	...
	conformance.ScanContracts(t, analyticalScanDialect{}, (&MySQLPlugin{}).EngineCatalogModel(), analyticalExpressionDialect{}.QuoteIdentifier, cases)
	if err := (analyticalScanDialect{}).ValidateSchema([]datatype.FieldInfo{{Name: "A", Type: datatype.FieldTypeInt}, {Name: "a", Type: datatype.FieldTypeInt}}); !errors.Is(err, plugin.ErrAnalyticalUnsupported) {
		t.Fatal(err)
	}
}
