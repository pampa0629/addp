...
不为 engine 原生层级新增 `NamespaceInfo`、`DatabaseInfo`、`BucketInfo`、`ObjectInfo`、`FileInfo` 等类型。database、schema、bucket、directory、prefix、file、object 等都先由 `common/engine/plugin` 的 `EngineCatalogEntry` / `EngineCatalogFacts` 表达；只有当内容已经被确认为某个 ADDP data type 后，才进入本包对应的 `TableInfo`、`DocumentInfo`、`MediaInfo`、`ContainerInfo`、`GraphInfo`、`CADInfo`、`Model3DInfo`、`PointCloudInfo` 或 `GaussianSplatInfo`。
