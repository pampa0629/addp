...
func TestRepositoryAuthorizationCatalogReportIsDeterministic(t *testing.T) {
	...
	if len(report.PermissionManifests) != 15 {
		t.Fatalf("manifest count = %d, want 15", len(report.PermissionManifests))
	}
	...
	wantOwners := []string{"agent", "asset", "copilot", "develop", "graph", "manager", "meta", "model", "monitor", "orchestrator", "quality", "service", "standard", "system", "transfer"}
	if !reflect.DeepEqual(owners, wantOwners) {
		t.Fatalf("manifest owners = %v, want %v", owners, wantOwners)
	}
	if report.BuiltinRoleManifest.ManifestVersion != 14 ||
