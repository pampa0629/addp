...
```mermaid
graph LR
    User[用户] --> Catalog[Meta 技术资源树<br/>Native Engine 或联邦 Source Engines]
...
- **技术资源浏览**：左侧直接消费 Meta resource-tree，不新增 Develop 私有 Engine Catalog API。native query engine 展示自身原生路径；federated query runtime 不拥有可导航的 Engine Catalog，工作台按 `federation.source_engine_types` 展示当前租户可参与查询的 Source Engine。
...
- DuckDB 是 System 中注册的共享联邦查询 Runtime Engine，不属于租户注册存储引擎，也不进入 Meta resource-tree。工作台选择 DuckDB 后，执行目标仍绑定 DuckDB Runtime，左侧技术资源树从当前租户 Meta Engine 列表中按 `federation.source_engine_types` 过滤 PostgreSQL、MySQL、MinIO/S3 等 Source Engine。
- 数据源选择结果的 ResourceLocator 保留真实 Source Engine ID；插入联邦 SQL 时以 Source Engine 名称的规范标识符作为首段。SQL 引用的 Source Engine 逐个进入本次只读 Execution Authorization，不能用 DuckDB Runtime ID 替换。
...
- DuckDB 样例由 Develop 从当前租户真实 Engine Catalog / Meta 构造候选，逐个签发只读授权并调用 DuckDB Runtime 验证；没有返回数据的候选不能作为样例，也不允许退回 `SELECT 1`。
