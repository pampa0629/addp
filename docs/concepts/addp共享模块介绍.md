...
- `common/dataprotection` - Security 和参与执行 Owner 共享的 `addp.protection_projection/v2` 契约、类型化路径、默认保护决策、按主体临时授权的本地匹配、投影校验和确定性遮盖 / 抑制算法，以及 Owner schema 内统一的版本化本地投影存储；它不读写 Security 业务状态、不审批申请、不授予资源访问权，也不在 `common` schema 建立中央保护事实
