...
| glossary element association | 术语数据元关联 | Standard 内业务术语与数据元稳定身份之间的语义关系。 | 不等同于模型引用的冻结修订，也不代表数据元当前生效。管理读取必须完整返回已关联身份，不因草稿、待生效、过期或撤回而隐藏；展示优先取当前生效修订，否则取最高版次修订，并独立标明是否当前生效。展示修订不是关系绑定，修订生命周期变化不自动解除关联。 |
...
| slowly changing dimension (SCD) | 维度历史策略 | 声明维度属性变化时如何保留历史：0 保留原值，1 覆盖为当前值，2 新增历史行，3 通过额外属性列保留有限历史。 | 属于 Model 的设计声明；实际更新由 Develop 加工任务实现，选择策略不会自动生成加工逻辑或历史字段。覆盖刷新且不保留属性历史的维度应声明为类型 1，不能声明为不变化的类型 0。 |
...
| metric detail result | 指标明细结果 | 确定指标实现修订中，按原主体、时间范围、固定过滤和去重键产生的组成对象集合。 | Model 冻结数据库无关计划，Service 以独立查询服务显式发布；首期仅 count_distinct，一行对应一个主体、统计期间和去重对象，不能用原始事实行数冒充指标次数。Workbench 仅通过 Consumer SDK 和同页选择绑定消费。 |
...
| standard extraction candidate group view | 标准提炼候选聚合视图 | Standard 将同一标准文档稳定身份历次提炼中的确定性同义候选聚合成一个可裁决读取单元。 | 聚合键为候选类型、编码以及规范化名称、定义和完整候选载荷的 SHA-256 语义指纹；码值项和维度先按稳定顺序规范化。同类型、同编码但内容不同的候选仍是不同聚合项，不使用模型相似度自动合并。治理读取支持按代表候选的编码或名称进行大小写不敏感的规范化子串检索；检索在分页和实时比对结果分面统计前执行。该视图不持久化、不改写原始候选；每次出现、提炼批次、文档修订、证据、人工处置和正式化事实均完整保留。 |
| standard extraction candidate family view | 标准提炼候选族视图 | Standard 将同一文档内候选类型和稳定编码相同的候选聚合项组织为一个可折叠治理读取单元。 | 候选族键固定为 `candidate_type + code`，只负责组织同编码语义变体，不合并语义指纹、不持久化，也不是新的聚合根。公开治理读取按候选族分页，同时返回筛选后的候选族总数、语义变体总数、每族未受筛选影响的 `total_variant_count`，以及由当前全部代表候选稳定计算的 `snapshot_token`；筛选作用于变体，族内只返回命中的独立变体，但令牌始终代表完整候选族。恰好一个语义变体的候选族只能以当前代表候选执行单项处置，历史非代表出现记录必须以 409 明确拒绝；存在两个及以上语义变体时，候选族胜出裁决是唯一处置路线，请求只携带 `winner_candidate_id + snapshot_token + reason`。服务端必须在文档锁内重算当前全部代表候选与令牌，令牌变化、胜出候选不是当前代表、候选已正式化或其他并发冲突时明确拒绝并整批回滚；客户端不重复传输成员数组，也不存在固定成员上限造成的治理死锁。该命令不持久化候选族、不改写候选内容，也不阻止后续提炼产生新的语义变体。 |
| standard candidate family decision event | 标准候选族裁决事件 | Standard 对一个完整候选族选择唯一胜出语义变体时形成的追加式治理事实。 | 必须与候选状态变更在同一事务中写入，冻结来源文档、候选类型和编码、胜出候选、全部成员的候选 ID、语义指纹、裁决后版本与结果，以及必填人工理由、操作者和时间。事件成员至少两个但没有固定上限。后续重新裁决只追加新事件，不更新既有事件；事件没有独立编辑或删除入口，按来源文档生命周期清理，也不使只读候选族成为聚合根。 |
| standard candidate formalization | 标准候选正式化 | 将一个已保留标准提炼候选转化为受治理标准草稿，或确认其对应既有相同内容修订的不可变治理事实。 | 候选必须仍是其语义变体按统一确定性规则选出的当前代表；Standard 在正式化事务内重新计算代表，历史非代表出现记录以 409 `candidate_representative_stale` 拒绝。随后根据同类型、同编码的实时比对唯一决定结果：无稳定身份时创建 R1 草稿；已有稳定身份且没有工作修订时，以最新修订为基线创建候选内容的新草稿；候选与现有草稿、审核中或已发布修订内容一致时只建立来源关联。范围冲突、已有不同内容的工作修订、无法解析的码值集或计量单位引用必须拒绝。正式化不得提交审核或发布，候选状态仍保持 `retained`。 |
| standard extraction candidate comparison | 标准提炼候选比对 | Standard 在读取提炼结果时，将候选与当前租户内同类型、同编码的活动标准稳定身份进行确定性比较所得的动态投影。 | 结果固定为 `new`、`exact`、`content_conflict` 或 `scope_conflict`，差异项明确给出字段及候选值、当前标准值；枚举数据元候选的 `code_set_code` 与现有数据元修订所冻结码值集修订的稳定编码比较，不比较数据库 ID；同名不同编码不自动判为重复。候选治理读取可按该实时结果筛选，筛选必须在候选族分页前完成；候选族分面计数应用聚合状态、候选类型和编码/名称关键词过滤，但不应用当前比对结果过滤。一个候选族包含多类实时比对结果时可同时计入多个分面，因此各分类计数不要求相加等于 `all`。标准当前修订变化后同一查询结果及分面计数允许随之变化。比对不写回候选、不创建标准，也不代替人工裁决。 |
...
| 英文术语 | 中文术语 | 定义 | 备注 |
|---|---|---|---|
| preview | 预览 | 为交互查看而读取的有界数据样本或有限结果。 | 预览不是完整数据交付，不能把当前页或已加载行伪装成完整下载。 |
| download | 下载 | 将已经存在的数据资源按其原始逻辑内容交付到用户本地，不改变数据模型和格式。 | multi-layout 资源可以透明打包，但不能借下载实施格式转换。 |
| export | 导出 | 把逻辑数据转换为 CSV 等交换格式，生成临时导出产物并交付到用户本地。 | 使用一次性 Transfer execution；产物暂存在 infra，下载后按生命周期清理，不进入业务资源树和 Meta。 |
| save result | 保存结果 | 将计算或查询结果写入用户选择的业务存储，形成正式数据资源。 | 必须遵守业务目标写入、Meta scan、血缘与资源生命周期契约，不得与导出混用。 |
| export session | 导出会话 | 关联发起用户、一次性 Transfer execution、infra 临时产物和下载状态的短生命周期控制面记录。 | 不是任务定义；必须按 Tenant 与发起用户共同隔离。 |

## 数据质量

| 英文术语 | 中文术语 | 定义 | 备注 |
|---|---|---|---|
| quality record key | 质量记录键 | 方案表绑定中显式声明的稳定单列或复合记录身份。 | 同一执行快照内验证非空且唯一，用于对账具体失败记录，不自动推断或修改源表主键。 |
| accepted quality exceptions | 已接受的质量异常 | 用户针对完整观测的具体失败记录说明原因后接受其持续存在。 | 不等于数据修复，不改变质量评分或阻断；新增、恢复后复发或约束变化需重新处理。处理审计永久保留。 |
...
| 英文术语 | 中文术语 | 定义 | 备注 |
|---|---|---|---|
| Manager data task workspace | Manager 数据任务工作区 | Manager 中统一管理可重复执行任务定义的产品工作区。 | 产品导航以“数据任务”为父菜单，纵向提供“快显任务”“空间数据任务”“向量化任务”三个子入口并复用同一工作区实现；导航合并不改变不同任务定义、执行器和结果状态的存储边界。 |
...
| Service Account | 服务账号 | Tenant 管理员创建并管理的非人工作负载账号，由一个 Tenant-owned Service Principal、该 Tenant 内唯一 Membership 和一个独立 Confidential OAuth Client 共同组成。 | 不是 User，也不是 External OAuth Client；Client ID 固定、Client Secret 只在创建或轮换时展示一次。数据库和授权协议仍使用 Service Principal，不新增平行身份实体。 |
...
| External OAuth Client | 外部 OAuth 客户端 | 代表 `addp-cli`、外部 BI 或桌面工具等请求用户授权的客户端软件；管理归属可以是 Platform 或单个 Tenant。 | 不是 ADDP User、Service Principal、Tenant、Service Account 或 API Consumer；Tenant 外部 Client 固定使用 Authorization Code + PKCE、无 Client Secret，且只能获得其 owner Tenant 内当前 User 的委托授权。服务账号内部绑定的 Confidential OAuth Client 只是机器凭据，不进入外部应用管理列表。 |
...
面向 Tenant 管理员的界面不直接使用“主体类型”作为标签。用户账号、租户服务账号与平台运行账号必须分开呈现；角色管理中的“角色分配”只面向用户账号，租户服务账号的角色从“应用接入 > 租户服务账号”管理，平台运行账号的角色从“应用接入 > 平台运行账号”只读查看。Department 和 Project Group 是人员组织，成员只能来自用户账号，组织成员选择不得展示机器身份；审计筛选等确实需要跨账号类别选择时才使用“账号类型”，并按“当前账号、用户账号、服务账号”分组，不依赖名称前缀猜测。组织内“成员类型”仅用于主部门、附加部门等组织关系语义。领域模型、API 字段和审计协议仍使用 Principal、Service Principal 与 `principal_type`。
