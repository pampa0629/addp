...
| standard extraction candidate family view | 标准提炼候选族视图 | Standard 将同一文档内候选类型和稳定编码相同的候选聚合项组织为一个可折叠治理读取单元。 | 候选族键固定为 `candidate_type + code`，只负责组织同编码语义变体，不合并语义指纹、不持久化，也不是新的聚合根。公开治理读取按候选族分页，同时返回筛选后的候选族总数、语义变体总数及每族未受筛选影响的 `total_variant_count`；筛选作用于变体，族内只返回命中的独立变体。单个变体仍以代表候选独立裁决；候选族胜出裁决是对调用方显式列出的当前全部、语义指纹互异的代表候选执行的原子批量命令，必须逐项携带 `id + version`，恰好保留一个胜出候选并驳回其余候选，缺少当前变体、任一成员无效、已正式化或并发冲突时整批回滚。该命令不持久化候选族、不改写候选内容，也不阻止后续提炼产生新的语义变体。 |
