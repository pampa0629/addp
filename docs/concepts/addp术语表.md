...
| standard extraction candidate family view | 标准提炼候选族视图 | Standard 将同一文档内候选类型和稳定编码相同的候选聚合项组织为一个可折叠治理读取单元。 | 候选族键固定为 `candidate_type + code`，只负责组织同编码语义变体，不合并语义指纹、不持久化，也不是新的聚合根。公开治理读取按候选族分页，同时返回筛选后的候选族总数、语义变体总数及每族未受筛选影响的 `total_variant_count`；筛选作用于变体，族内只返回命中的独立变体。恰好一个语义变体的候选族只能以当前代表候选执行单项处置，历史非代表出现记录必须以 409 明确拒绝；存在两个及以上语义变体时，候选族胜出裁决是唯一处置路线，必须对调用方显式列出的当前全部、语义指纹互异的代表候选执行原子批量命令，逐项携带 `id + version`，恰好保留一个胜出候选并驳回其余候选。服务端必须按候选聚合视图的同一确定性规则重算并校验每个当前代表候选 ID；多变体族调用单项处置、提交历史非代表出现记录、缺少当前变体、任一成员无效、已正式化或并发冲突时必须明确拒绝并整批回滚。该命令不持久化候选族、不改写候选内容，也不阻止后续提炼产生新的语义变体。 |
| standard candidate family decision event | 标准候选族裁决事件 | Standard 对一个完整候选族选择唯一胜出语义变体时形成的追加式治理事实。 | 必须与候选状态变更在同一事务中写入，冻结来源文档、候选类型和编码、胜出候选、全部成员的候选 ID、语义指纹、裁决后版本与结果，以及必填人工理由、操作者和时间。后续重新裁决只追加新事件，不更新既有事件；事件没有独立编辑或删除入口，按来源文档生命周期清理，也不使只读候选族成为聚合根。 |
...
| Machine Identity Management View | 机器身份管理视图 | “应用接入”中统一查看当前 Tenant 可用机器身份的管理视图，同时呈现 Tenant-owned Service Account 和已加入当前 Tenant 的 Platform-owned Runtime Service Principal。 | “机器身份”只作为聚合管理入口名称，不新增领域实体。两类对象仍保持不同领域语义；平台运行账号只读，Tenant 管理员不得编辑其定义、生命周期或凭据。角色展示与分配复用同一 Role Assignment 事实和 API。 |
