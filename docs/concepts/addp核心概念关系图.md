...
mindmap
  root((ADDP 核心概念))
    ...
    数据标准
      ...
      标准稳定编码 Tenant内按类型唯一且不可变的snake_case身份
      标准分类 仅用于浏览导航
      业务术语 稳定身份与统一业务语言
        业务术语修订 不可变发布快照
        生效区间 按业务时点解析
      ...
      标准文档
        ...
        标准提炼批次 固定输入文档修订
          提炼候选 术语/数据元/码值集/指标
            候选编码命名空间 domain文档使用归属域编码前缀
            已知候选编码 同文档合规历史仅作复用提示
            枚举数据元候选 以code_set_code引用同批码值集候选
            候选聚合视图 跨批次确定性语义指纹聚合
            候选族视图 同类型同编码组织语义变体
            候选正式化 retained后创建草稿或关联一致修订
          提取证据 页码/章节/行号/文本片段
      数据字典 Meta+Catalog+Standard 组合视图
    数据建模
      业务实体模型 业务对象/事件/关联实体
      概念实现映射 Entity/Attribute/Relation到Table/Field/TableRelation
      逻辑模型与模型关系
      公共/一致性维度
      维度层级
        层级字段引用数据元修订
...
graph TB
    ...
    标准 --> 业务域[业务域]
    标准 --> 稳定编码[标准稳定编码]
    业务域 -.归属可调整且不改写稳定编码.-> 稳定编码
    业务域 -->|domain 文档以业务域编码约束新候选前缀| 提炼批次
...
6. **业务域**是跨模块复用的治理责任边界；标准适用范围和标准分类分别回答“在哪里成立”和“如何浏览”，不能相互替代
7. **数据标准**以稳定身份承接长期引用，以不可变修订承接审核、发布和历史追溯；定义按生效区间和业务时点动态解析，数据元枚举值域固定引用码值集修订
   标准稳定编码统一使用小写 `snake_case`，但不将可调整的归属业务域固化为强制前缀；业务域前缀只约束 Copilot 生成的新候选
8. **维度层级和指标实现**属于 Model；Standard 只拥有可复用的数据元、码值集和指标定义等语义契约，Model 必须冻结所采用的标准修订
   业务实体模型属于概念层，事实表和维度表属于同一 LogicalTable 逻辑层；二者通过 Model 权威的概念实现映射建立追溯，不要求先复制一套实体型逻辑表，也不允许隐式双向同步
9. **标准映射与质量检查分离**：Catalog 拥有企业落标映射；Quality 方案显式绑定物理目标，独立执行和治理问题。Standard 可提供已发布规则，不把 Catalog 作为配置或执行前置。
