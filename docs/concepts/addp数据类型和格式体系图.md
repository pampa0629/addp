...
| `cad` | 文件 / 对象承载：NFS、S3、MinIO；Meta deep scan 直接读取文件头识别格式与版本，Manager 预览在浏览器使用 LibreDWG Worker/WASM 与 WebGL。 | 当前内置：`dwg`、`dxf`。 | DWG、DXF 源 item 保留 CAD 语义，不因 entity 可投影为记录而归为 `table`。扫描和预览都不依赖 SuperMap；CAD→GIS 输出是新的 `table + capabilities.spatial` item。 |
...
- `pdf`、`wps`
- `jpeg`、`png`、`tiff`
- `glb`、`gltf`、`obj`、`stl`、`fbx`、`ifc`、`osgb`、`osgb_scene`、`s3m`、`3dtiles`
...
- 业务矢量瓦片集 = `data_type=media` + `layout=single` + `format=pmtiles` + `capabilities.spatial`；PMTiles v3 header、层级与压缩信息进入 `format_info.pmtiles`。
...
- IFC = `data_type=model_3d` + `layout=single` + `format=ifc`，BIM 语义由 `model_kind=bim_model` 表达；识别和轻量 attributes 由 Meta 完成，GLB 快显通过 `model3d_workflow.ifc_to_glb` 专用 operator 生成。
...
- S3M = `data_type=model_3d` + `layout=whole` + `format=s3m`，ADDP 产物统一以 `config/scene.scp` 作为 manifest ref；旧版 XML SCP + `.s3m` 与新版 JSON SCP + `.s3mb` 的差异写入 `format_info.s3m.manifest_encoding` 和 `tile_extension`，不拆分 data type。
...
| `ifc` | schema identifiers、schema version、entity count、entity type counts |
