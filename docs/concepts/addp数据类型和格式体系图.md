...
| `model_3d` | 文件 / 对象承载：NFS、S3、MinIO。当前没有专用原生三维模型 catalog 引擎。 | 当前内置：`glb`、`osgb`、`3dtiles`。后续扩展：`gltf`、`obj`、`stl`、`ifc`、`rvt`。 | GLB / glTF、OSGB 倾斜摄影、3D Tiles、IFC / Revit BIM 都归入 `model_3d`；网格场景、倾斜摄影、BIM、分块场景等子形态由 `type_info.model_3d.model_kind` 表达，不新增平行 data type。OSGB 源 item 先做 Meta scan，预览通过转换后的 3D Tiles item 复用链路。 |
