...
> 数据编织是一种面向分布式、异构数据环境的架构理念和能力体系。它不再要求所有数据必须先集中到数据仓库才能治理和使用，而是通过统一连接、元数据、语义、治理规则和自动化能力，把分散在不同系统中的数据组织成一个逻辑统一的数据网络；数据是否汇聚、复制或原位访问，则根据具体业务场景决定。
...
因此，数据编织并不是“不搬数据”，而是取消“所有数据必须先搬到同一个地方”这一前提。是否搬、搬多少、何时更新，应由业务用途、查询性能、历史要求、源系统承载能力、数据安全和建设成本共同决定。

两种模式的区别可以概括为：

| 对比维度 | 以数据仓库为中心的常见模式 | 数据编织模式 |
| --- | --- | --- |
| 基本路径 | 数据先集中，再统一加工和使用 | 先建立逻辑统一，再按场景选择访问和处理方式 |
...
最核心的变化是：**物理集中不再是逻辑统一的必要条件，数据治理也不再等到数据进入仓库以后才开始。**
...
传统路径是把各系统数据定期抽取到数据仓库，统一客户编码，建立客户、合同、项目、应收和服务主题模型，再计算客户价值、逾期金额和风险等级。
...
稳定、复用频率高的客户主题和核心指标仍然进入数据仓库。实时工商风险可以通过外部服务获取，原始投诉文档可以保留在文档库中，仅把必要的标签和分析结果写入仓库；临时探索则可以在权限允许的情况下进行联邦查询。

最终，客户全景和风险预警应用看到的是逻辑统一的数据，而底层数据并不一定都存放在同一个系统中。当某项外部服务不可用、客户映射发生变化或质量下降时，主动元数据和运行监控还应提示受影响的指标和应用。
...
如果需要稳定历史、高频复用和复杂计算，通常更适合汇聚；如果数据使用频率低、变化快、不宜复制或已有稳定服务，可以考虑原位访问。很多真实场景会采用混合方式，而不是只选其中一种。
...
数据可以原位访问，也可以汇聚、复制、缓存和物化。选择标准是业务价值、性能、一致性、安全和成本，而不是“不搬数据”的口号。
...
跨源查询会受到网络、源系统负载、权限和一致性的影响。高频、复杂和需要历史快照的分析，通常仍应在数据仓库或分析平台中完成。
...
5. 是否建立客户、产品、项目、地块等跨系统业务对象的关联？
6. 是否根据场景选择汇聚、联邦查询、服务调用、流处理或缓存？
...
数据编织要解决的，不是“数据集中还是分散”这道单选题，而是在数据天然分布于不同系统和环境的情况下，如何保持统一理解、统一治理和灵活使用。

它把数据治理的边界从数据仓库内部扩展到整个分布式数据环境，让物理汇聚从必经步骤变成按业务需要选择的技术手段。数据仓库仍然负责它擅长的历史保存、集中建模和高性能分析，其他数据则可以通过联邦查询、数据服务、流式订阅或专业引擎参与业务场景。
