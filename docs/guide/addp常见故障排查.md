...
### Manager 上传成功，但文件只有扫描目录后才显示

2026-09-15 排查确认：上传后的 Meta `ref_groups` 执行已经成功，Manager 也等待执行结束并重新请求目录。问题不是 Markdown 格式不支持或前端刷新太早，而是 item 的 `full_name` 为 `addp/doc/文件.md`，`node_id` 却指向 `addp` bucket；资源树按父节点读取，因此文件出现在错误层级。

历史原因：2026-06-05 引入引用组扫描（`f6c7a4f34`）时，复用了依赖“已创建扫描起点节点”的相对路径函数，但传入的基础节点仍是 bucket。函数剥离 `doc` 后将文件直接挂到 bucket。2026-07-17（`b025d4ec6`）又为 whole 数据集扫描起点补充向上找父目录的分支；扫描范围与资源归属混在同一算法中，普通对象和组合对象还维护不同的父链拼接路径。旧测试以 bucket 根对象为主，且没有约束 `full_name` 与父节点的一致性。

修复原则：删除依赖扫描起点的相对父链算法，所有对象从 bucket 内规范父路径建立同一条 prefix 链；扫描范围只限制枚举与扫描状态。写入前校验对象身份与父节点，错误归属不得报告成功。目录扫描按规范路径纠正既有归属，不因内容未变化跳过纠正。不能通过前端多刷新、额外扫描整个父目录或直接 SQL 改 `node_id` 代替修复。

排查时关联上传响应中的 execution、执行完成状态、目录刷新请求，再对照 item 的 `full_name` 与父 node 的 `full_name`。树顶部刷新只重新读取元数据；节点扫描才会重建目录事实。部署修复后，对已存在的错挂数据执行对应目录扫描，再刷新资源树。标准验证入口为 `META_POSTGRES_TEST_DSN=<addp_test连接> make test-module MODULE=meta`，新增用例由已有 Meta Go T1 自动发现。

另一个独立根因是目录内容和数量曾有两份事实来源：内容来自 `meta_item`，数量和大小却只在范围扫描结束时写入 `meta_node`。上传引用组、known-item refresh、删除和局部扫描不等于完整目录扫描，因而不能可靠维护这份副本。修复统一删除 node 统计列和扫描写入路径，Meta 查询按有效父子关系批量聚合逻辑 item 数量与已知大小，且与扫描状态解耦。不要恢复“上传后加一”、扫描后覆盖祖先统计或 Manager 按分页行数计算的路线。
...
dist/release-$(go env GOOS)-$(go env GOARCH)/addp-iam-migration-repair --migration 75 --apply
```
...
Security 模块首次开发迁移如果在事务内失败，PostgreSQL 会完整回滚业务事实，但 Migration Runner 保留 `113/dirty` 阻止自动重试。只有在已确认 Security Permission、runtime Role、Service Principal、OAuth Client 和 Role Assignment 均未落地，且 `standard.classification.*` 仍保持迁移前状态时，才可执行：

```bash
make build-iam-migration-repair
dist/release-$(go env GOOS)-$(go env GOARCH)/addp-iam-migration-repair --migration 113 --apply
```
