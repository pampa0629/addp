...
修改所有使用 API 响应的地方，删除多余的 `.data` 访问：
...
文件：`manager/frontend/src/views/DataExplorer.vue`，第 376 行
...
文件：`manager/frontend/src/views/DataExplorer.vue`，第 476 行
...
1. **代码审查检查清单**
   - 搜索 `response.data.data`（双重访问）
   - 验证 API 返回值类型
   - 检查 createAPIClient 的 extractData 配置

2. **统一 API 响应处理**
   - 明确文档化哪些 API 已自动提取 `.data`
   - 在 API 层添加 TypeScript 类型定义
   - 避免混用自动提取和手动提取

3. **使用 TypeScript（推荐）**
   ```typescript
   // 类型定义明确返回值
   function getPreview(params): Promise<TablePreview>  // 不是 Promise<AxiosResponse<TablePreview>>
   ```
