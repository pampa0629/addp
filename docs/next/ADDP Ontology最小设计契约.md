...
状态：独立 Ontology 的原生语义内核已落地，范围与标准验证入口见 [模块说明](../../ontology/CLAUDE.md)；数据库 Schema、发布服务、Tool 和 Infra 图服务尚未交付。当前不调整 Graph，待 Ontology 有初步成果后另行讨论其职责迁移。本文的目标设计不代表功能均已实现。
...
这一节定义的是目标契约，PG 事务、重建和撤回尚未由临时原型验证。
...
临时原型已验证 Go 1.24.2、CEL-Go `v0.32.0`、FalkorDB Go 客户端 `v2.1.0` 和 FalkorDB `v4.20.6` ARM64 的有限场景。语义内核采用已锁定的 CEL-Go 并登记技术栈规约；FalkorDB 客户端和镜像仍只是验证基线，不能视为正式 Infra 已接入。

正式实施必须满足：

- 锁定依赖版本及镜像 digest，与平台依赖规约协调；原型使用的客户端依赖 go-redis `v9.17.2`，与当前规约一致。
- 解决逐请求取消契约。已验证客户端 Query/ROQuery 内部使用 background context，服务端 timeout 虽有效，但不能证明及时取消。不能用“外层 goroutine 提前返回”伪装为底层查询停止。
...
当前先实施 1a 语义内核，代码位于 `ontology/backend/internal/semantic`。它是后续 owner 服务的内部库，不是临时原型，也不是可独立启动的 HTTP 服务；不提前登记空路由、权限、端口或数据库 Schema。当前只有原生类/属性/关系声明、受限 CEL 和不可变内存快照；来源引用、关系实例推导、审核发布、PG 持久化和 FalkorDB 均不伪装为已实现。
...
| 1b. 发布与投影基础 | PG 迁移、FalkorDB 唯一适配与取消、构建/激活/撤回；实际服务、Infra、权限和 CI 登记 | T0 登记一致性；T2 发布并发、恢复、重建、引用撤回与取消 |
