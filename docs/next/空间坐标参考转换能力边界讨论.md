...
ADDP 当前已经把空间能力建模为 `capabilities.spatial` 横切事实，这个方向是正确的。平台需要继续保存真实空间事实，例如几何列、SRID、CRS、extent 和 extent_srid；但是否执行 CRS transform，应按能力边界拆分，而不是让所有后端模块都隐式具备任意坐标转换能力。
...
- 为前端和引擎提供统一契约，例如 `source_crs`、`source_srid`、`target_crs`、`transform_status`。
...
- 如果后端返回原始 CRS geometry，应明确 `source_crs/source_srid`，由前端决定是否可预览转换。
...
- 从后端接收 `source_srid/source_crs` 的统一 DTO。
- 可转换时执行前端 transform，不可转换时显示明确状态。
- 对大样本设置阈值，超过阈值时不在前端执行转换。

第二阶段已先落地最小前端 CRS registry：内置 `EPSG:4326`、`EPSG:3857`，并优先尝试注册后端返回的 `source_crs` 定义；暂不接入外部 EPSG 查询。
...
- 高德等国内商业底图通常涉及 GCJ-02 坐标体系。GCJ-02 不是标准 EPSG CRS，不应进入 `capabilities.spatial.srid/crs`。
...
- 可配置 CRS 定义来源，优先使用后端返回的 WKT/PROJJSON/proj4 文本，不依赖前端运行时访问外部 EPSG 服务。
...
2. 移除后端 PROJ：
   ...
   - 删除或调整 `-tags proj` 测试。
3. 后端接口契约调整：
   - 统一返回 `source_srid/source_crs/transform_status`。
...
建议：采用“小内置 + 后端优先”的策略。前端只内置少量高频 CRS，例如 4326、3857，以及项目明确支持的少量国内常用坐标系；其他 CRS 必须由后端返回 WKT/PROJJSON/proj4 定义。第一阶段不做外部在线 EPSG 查询。
...
- PostGIS GeoJSON / geometry 预览接口返回原始 geometry 或源坐标下的 geometry 表达，并明确 `source_srid/source_crs`。
- 如果返回格式为了前端解析方便使用 GeoJSON geometry，也必须标记 `transform_status=not_transformed`，不得宣称 `target_srid=4326`。
- 前端根据 `source_srid/source_crs` 做预览级转换；无法转换时压制地图并显示原因。
