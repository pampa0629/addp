...
| JSON（含 GeoJSON 空间结构） | 已完成：同一 `FormatJSON` 插件处理 JSON / GeoJSON | 已完成：同一 descriptor 声明 `.json`、`.geojson` 和 `application/geo+json` | 部分完成：可描述 GeoJSON table，默认 data type 仍为 document | 已完成：table sample、raw 声明 | 部分完成：GeoJSON 空间能力由 `capabilities.spatial` / `SpatialInfo` 表达，Meta 仍有单资源空间识别补充 | 部分完成：Manager 有 JSON / GeoJSON 不同展示 handler，但 common format 不拆独立格式 | 已完成：json / geojson 预览 | 已完成：格式解析在 JSON 目录内，展示代码在 Manager | 部分完成：`parser.go` 同时承载 plugin、iterator 和 table reader；serializer 单独存在，后续可按 plugin / reader / iterator 整理 | 部分完成：GeoJSON iterator 可流式样本，但 JSON / GeoJSON 暂无标准 content_index，超大文件需核实首屏和分页成本 | 待核实 | 明确 JSON document / table / GeoJSON 空间结构的判定和 Manager 匹配优先级。 |
