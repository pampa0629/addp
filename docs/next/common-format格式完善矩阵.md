...
| Shapefile | 已完成 | 已完成 | 已完成：component table / spatial info | 已完成：component table sample、raw 声明 | 部分完成：Meta 仍需要 multi detector 归并组件，这是合理特例；Shapefile 信息提取已改为调用 common/format ComponentTableProvider | 部分完成：Manager 表格预览已优先消费 attributes 中的 table / spatial info，组件内容读取优先消费 `component_files`；仍有专用 shapefile DTO 组装层 | 已完成：shapefile / table 预览 | 已完成：代码在 shapefile 目录内较集中 | 部分完成：`plugin.go` 为 FormatPlugin adapter，`parser.go` 仍承载组件物化、info、sample、WKT 等职责；DescribeTableComponents 已用 SHP / DBF header 快速返回 row_count、extent、schema；Polygon GeoJSON、WKT 与 DBF 编码正确性已补测试 | 部分完成：Describe 已避免全量遍历；Sample 仍支持 offset / limit 但会物化组件，DBF / SHP 顺序访问在大文件深分页下仍需核实 | 部分完成：已覆盖 component_files 读取、缺组件提示、`.cpg` GBK 解码、WKT、Polygon hole / multipolygon 和坐标转换单测；真实大文件、更多编码和 MinIO / NFS 端到端仍待核实 | 用真实 MinIO 样例重扫并对比预览耗时；继续核实 GB18030 / UTF-8 BOM 等更多 `.cpg` 编码、真实大文件深分页与组件物化成本；后续收敛专用 shapefile content handler 到通用 component table / spatial preview DTO 组装层。 |
| Excel | 已完成 | 已完成 | 部分完成：已新增 `ContainerInfoProvider` 并由 Excel plugin 输出 workbook / sheet children；指定 sheet 时 `TableInfoProvider` 可按 `ParseOptions.SheetName` 只分析目标 sheet | 已完成：table sample、raw 声明；sheet child 预览通过 `TableSampleReader` + `SheetName` 读取 | 部分完成：Excel children 已改为 provider 通用入口，Meta 仍保留 container provider 调度和 SQLite / GeoPackage 分支 | 部分完成：Manager workbook 概览优先消费 Meta attributes；传入 `child_name` 时按 container child 通用规则路由到 `builtin:file-table`，并从 container children 还原 schema / row_count 后读取目标 sheet；仍保留 Excel 概览 handler | 部分完成：已抽出通用 `ContainerPreview` 前端壳，Excel 只做 workbook / sheet 适配；sheet 下拉切换请求 child table 预览，旧 tabs / 示例限制文案已清理，真实多 sheet / 空 sheet 体验待核实 | 部分完成：analyzer、container provider、sheet table reader 在 common/format，attributes 映射在 Meta，Manager 只做 DTO 组装和 provider 路由 | 部分完成：`plugin.go` 作为插件入口，`analyzer_internal.go` 命名清晰 | 部分完成：sheet child 可分页读取；但 XLSX ZIP 结构通常仍需读取包内容，大文件首屏和多 sheet 切换性能待核实 | 待核实 | 用真实多 sheet、空 sheet、大文件样例核实：Meta children、workbook 概览、`child_name` 表格分页和 `ContainerPreview` 切换体验；继续收敛 Excel 概览 handler 到通用 container DTO。 |
| SQLite | 已完成 | 已完成 | 部分完成：container children 由 Meta 调用 sqlite analyzer 写入；child 真实表名可通过 `table` 字段传给 reader options | 已完成：table sample、raw 声明；表 child 预览通过 `TableSampleReader` + `ExtraParams.table` 读取 | 部分完成：容器 children 仍在 Meta 中有格式分支 | 部分完成：Manager 概览仍有 SQLite handler；传入 `child_name` 时已按 container child 通用规则路由到 `builtin:file-table`，并从 container children 映射真实 table 名 | 部分完成：SQLite 运行时插件已接入通用 `ContainerPreview`；只做 database / table 适配，表切换复用 child preview 链路，旧单表示例上限 / 截断提示已不再作为预览 UI 入口 | 部分完成：analyzer 在 common/format，children 映射在 Meta，Manager 表 child 预览已复用通用 file-table provider | 部分完成：`parser.go` 作为插件入口，`analyzer_internal.go` 命名清晰；后续可补 `plugin.go` 主入口 | 部分完成：SQLite 可天然分页查询，但当前对象文件可能需要先物化本地临时文件；大库体验待核实 | 待核实 | 核实真实 SQLite 多表切换、分页、只读打开错误和大库物化成本；下一步判断 GeoPackage 是否有独立 provider / reader 后再接入同一 container preview。 |
