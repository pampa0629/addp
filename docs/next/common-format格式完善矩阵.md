...
| Excel | 已完成 | 已完成 | 部分完成：已新增 `ContainerInfoProvider` 并由 Excel plugin 输出 workbook / sheet children；指定 sheet 时 `TableInfoProvider` 可按 `ParseOptions.SheetName` 只分析目标 sheet | 已完成：table sample、raw 声明；sheet child 预览通过 `TableSampleReader` + `SheetName` 读取 | 部分完成：Excel children 已改为 provider 通用入口，Meta 仍保留 container provider 调度和 SQLite / GeoPackage 分支 | 部分完成：Manager workbook 概览优先消费 Meta attributes；传入 `child_name` 时路由到 `builtin:file-table`，并从 container children 还原 schema / row_count 后读取目标 sheet；仍保留 Excel 概览 handler | 部分完成：workbook 概览仍是 excel 预览；sheet tab 切换会请求 child table 预览，组合体验需用真实样例核实 | 部分完成：analyzer、container provider、sheet table reader 在 common/format，attributes 映射在 Meta，Manager 只做 DTO 组装和 provider 路由 | 部分完成：`plugin.go` 作为插件入口，`analyzer_internal.go` 命名清晰 | 部分完成：有 sheet / row / column limit，sheet child 可分页读取；但 XLSX ZIP 结构通常仍需读取包内容，大文件首屏和多 sheet 切换性能待核实 | 待核实 | 用真实多 sheet、空 sheet、大文件样例核实：Meta children、workbook 概览、`child_name` 表格分页、tab 切换后的前端体验；后续评估是否把 workbook + active sheet table 合成一个稳定预览模型。 |
