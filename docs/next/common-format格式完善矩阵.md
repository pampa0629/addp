...
| JSON / GeoJSON 编码 | 当前 GeoJSON 暂不升格为独立 format identity，仍按 `format=json` + `capabilities.spatial` 处理；还要补真实样例回归，核实大文件分页、复杂嵌套对象数组、GeoJSON 无 geometry / 混合 geometry、WKB / EWKB 空间渲染体验。 |
