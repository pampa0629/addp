...
| JSON（含 GeoJSON 空间结构） | 已完成：同一 `FormatJSON` 插件处理 JSON / GeoJSON | 已完成：同一 descriptor 声明 `.json`、`.geojson`、`application/geo+json`，并声明 `content_index` | 部分完成：默认 data type 仍为 document；内容解析为 GeoJSON FeatureCollection 或顶层对象数组时可提供 table info；只有实际发现 GeoJSON geometry 或 WKB / EWKB 几何字段值才提供 `SpatialInfo` | 已完成：table sample、raw 声明、JSON 稀疏行 content index；无 geometry 的记录集合不会虚构 geometry 字段，WKB / EWKB 会解码为 GeoJSON-like geometry 供前端渲染 | 部分完成：`capabilities.spatial` 仅由 `SpatialInfo` 等内容事实写入，已去除单资源 `.geojson` 默认空间能力；NFS / ObjectStorage 均复用通用 table file info 映射，避免按引擎分叉 | 部分完成：文件表预览能从 Meta attributes 还原 `SpatialInfo`，并可用 `content_index.table` 做 range preview；Manager 仍保留 JSON / GeoJSON 展示 handler，GeoJSON handler 只按显式线索匹配，解析失败回退 JSON | 已完成：json / geojson 预览；空间展示属于前端表格 + 空间渲染方式 | 已完成：格式解析在 JSON 目录内，展示代码在 Manager | 部分完成：`parser.go` 同时承载 plugin、iterator、WKB 解码、bbox 推导和 table reader；serializer 单独存在，后续可按 plugin / reader / iterator / geometry helper 整理 | 部分完成：JSON 记录集合可流式样本，并已提供 `content_index.table` 稀疏行索引和 positioned reader；content index 失效刷新由 Meta 自身流程负责，超大文件端到端体验待核实 | 待核实 | 用真实样例核实 CSV 转 JSON 对象数组、无 geometry FeatureCollection、真实 GeoJSON、WKB / EWKB 字段、MinIO 与 NFS 刷新后 attributes、Manager 表格 + 空间渲染和大文件分页体验。 |
| Parquet | 已完成 | 已完成 | 已完成：single table info；whole scope 可递归合并多个 `.parquet` 的 schema 和行数，schema 不兼容时报错 | 已完成：single table sample、whole scope 跨文件分页 sample、raw 声明 | 部分完成：single / whole detector 已有，part 文件和分区目录可识别为 whole scope；同目录无 part 语义的多个 parquet 仍保持独立 item | 部分完成：文件表预览和 scope table 预览均走 provider；scope table 已返回总行数，whole scope 路由仍需真实样例核实 | 已完成：table 预览 | 已完成 | 部分完成：已有 `parser.go`、`provider.go`、`table_file.go`，但主入口命名可向 `plugin.go` 收敛 | 部分完成：已支持 whole scope 跨文件分页，但单文件仍基于 `io.Reader` 全量读入；Parquet row group / footer / range-aware reader 需要另行设计，不复用 CSV / JSON content_index | 待核实 | 用真实 MinIO / NFS 样例核实 `part-*`、分区目录、schema 不兼容提示、大文件 row group 性能；后续设计 Parquet 专用 range / row group 读取边界。 |
