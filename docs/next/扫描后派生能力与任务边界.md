...
MVT / Quick View 的 extent、SRID、target SRID、transform status 属于空间展示派生事实，不得写回 `attributes.capabilities.spatial`。具体边界由 [Manager 快显规范与技术路线](../../manager/docs/快显规范与技术路线.md) 专题处理，本文只保留 owner 判断原则。
