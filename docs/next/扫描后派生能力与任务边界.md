...
| MVT / Quick View 物化视图 | Manager | 服务空间快显，依赖 Manager tile config、zoom、缓存策略；只清理 Manager 创建并登记的快显优化目标 |
...
MVT / Quick View 的 extent、SRID、target SRID、transform status 属于空间展示派生事实，不得写回 `attributes.capabilities.spatial`。同源 schema 下自动识别的外部 3857 物化视图是只读快显性能目标，不写入 `manager.quick_view_optimization`，也不获得 Manager cleanup 所有权。具体边界由 [Manager 快显规范与技术路线](../../manager/docs/快显规范与技术路线.md) 专题处理，本文只保留 owner 判断原则。
...
2. Manager `TileCacheTask`、`QuickViewOptimizationTask`、`EmbeddingTask` 这类任务定义仍归 Manager。
