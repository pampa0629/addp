...
| artifact state | 产物 owner 模块 | 描述外部产物当前位置、状态、版本、缓存键、失败原因和清理条件 | Manager 派生产物、embedding vectors、preview cache |
...
- 写入 preview cache、空间展示缓存、embedding、thumbnail 等派生产物。
...
以 Manager 空间快显派生产物为例，本文只确认其 owner 归 Manager；具体表、API、技术路线和外部目标边界见 [Manager 快显实现规范](../../manager/docs/快显实现规范.md)，cleanup 规则见 [Meta cleanup 边界与派生产物清理设计](meta-cleanup边界与派生产物清理设计.md)。
...
| Manager 空间快显派生产物 | Manager | 服务空间快显；具体对象由 Manager 快显文档定义 |
...
Manager 空间快显派生产物的具体边界由 [Manager 快显实现规范](../../manager/docs/快显实现规范.md) 处理，本文只保留 owner 判断原则。
...
1. Meta `ScanTask` 只定义扫描，不定义 Manager 空间快显派生产物、embedding、thumbnail。
2. Manager 派生产物任务定义仍归 Manager。
3. 各模块执行都可以写入 `common.task_executions`，通过 `module`、`task_type`、`source_task_id` 和 `source` 关联自身任务定义。
4. `trigger_type` 仍只表达 `manual` / `scheduled`，不要把派生产物格式、embedding、thumbnail 塞进触发类型。
...
sequenceDiagram
    ...
    Manager->>Manager: evaluate derived artifact policy
...
- 不改变 Manager 派生产物 / embedding 现有模块归属。
- 不新增统一扫描后派生任务表。
...
4. 用户主动生成外部空间数据结果与 Manager 派生产物的边界。
   建议：Manager 派生产物继续归 Manager；用户选择输出位置的数据处理结果进入用户 business 存储，再由 Meta scan 识别。
