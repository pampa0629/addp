...
本文定义 ADDP 对 data item、数据类型、组织方式、文件格式和横切能力的 next 阶段概念模型。配套 spec 文档已一并放在 `docs/next` 下，后续实现应以这些 next 文档为准修订。
...
本文定义 next 阶段目标结构。当前实现中的旧字段名和旧结构不保留兼容语义，应通过重新 meta 扫描和代码修正尽早暴露并解决。
