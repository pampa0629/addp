...
### 13.61 源人员引用缺失核查（2026-09-18，身份归并待确认）
...
处置结论：保留 ODS 主档引用检查及工单 21 的 `open` 状态（77 条），不把 DIM 快照补全冒充源主档问题已解决，不改为按昵称关联、不删除旧标识、不生成伪造 Persons。先确认历史数据是否经历人员 ID 更换／重建及是否存在权威映射；若没有，继续保留独立原 ID 与可追溯快照，待人工确认具体身份关系。身份归并方案尚未实施。
