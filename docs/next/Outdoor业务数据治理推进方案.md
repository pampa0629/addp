...
> 当前基线（2026-09-18）：正式编排 10 为 15 步：3 个 ODS 同步、4 个 Model 创建／校验目标表、4 个 Develop 数据刷新和 4 个分阶段 Quality 检查。业务实体直接映射为事实／维度逻辑表；不另建实体型中间表，不使用发布组或暂存批次。Service 根据 Model 已发布指标修订执行在线查询，Workbench 消费服务。最近完成的完整刷新与质量验收见 13.59—13.62；第 3—12 节及第 13 节早期记录属于历史计划与过程证据，不能据此恢复旧路线。
...
业务确认
  ...
  -> Standard 语义资产
  -> Meta 物理字段绑定
  -> Model 业务实体、概念实现映射、事实/维度逻辑表
  -> Transfer 将 MongoDB 贴源同步到 PostgreSQL ODS
  -> Model 创建/校验 DIM/DWD/DWS 目标表结构
  -> Develop 刷新 DIM/DWD/DWS；Quality 在 ODS/DIM/DWD/DWS 各阶段检查
  -> Orchestrator 编排上述 15 步依赖和执行
  -> Model 冻结指标计算规则；Service 按发布修订在线查询 DIM/DWD
  -> Workbench 消费服务并展示指标及明细
```
...
- `Model` 拥有实体、逻辑模型、概念实现映射和指标实现修订；负责创建／校验目标表结构及显式删除目标表，不刷新数据；
...
- `Service` 按固定 Model 指标修订，经引擎编译器和 Provider 执行在线查询；当前指标服务读取 DIM/DWD，DWS 是编排生成的持久汇总结果，不是在线服务的必经来源；
- `Workbench` 消费 Service 契约，负责人员选择、统计期间、图表及明细展示；`Graph`、`Asset` 等模块按各自边界消费，不新增第二套 Outdoor 事实源。
...
> 第 3—12 节保留最初的阶段计划。业务口径以随后明确确认的决定为准：主领队来源优先级见 13.60；历史异常的接受见 13.62。用户在后续质量会话中已要求停止追查剩余历史异常，13.62 末尾的排查建议不再作为当前待办。
...
不建立“领队”码值集：当前主领队由 `Outdoors.leader.personid` 优先表达，缺失时使用原始 `members[0].personid`；不能由成员状态枚举替代。有效活动统一定义为活动日期有效、已经开始，且源状态不是 `draft` 或 `cancelled`；因此已经开始但源状态仍为 `published` 或 `registration_closed` 的活动按已成行处理。
...
| 指标 | 粒度 | 核心公式/过滤 | 主要事实源 |
| --- | --- | --- | --- |
| 当前主领队活动次数 | 人员 | 当前主领队为该人员的有效活动 ID 去重数 | `Outdoors.leader.personid`，缺失时 `members[0].personid` |
...
| 当前主领队 | `Outdoors.leader.personid`、原始 `members[0].personid` | `leader` 非空优先；首位成员只作缺失回退；成员状态用于参与事实，不覆盖当前领队关系 |
...
### 13.62 记录级接受历史异常（2026-09-18，已部署并完成线上验收）
...
用户重启后已确认 migration 16 生效，Backend ready，Worker 于 2026-09-18 14:35:35 启动。只读核验成员表 6,954 条记录的 `activity_id + member_index` 非空且唯一，活动表 2,383 条记录的 `activity_id` 非空且唯一。方案 22 仍为 V2、尚未配置记录键；工单 21／22 均为 open、证据原因 not_observed，当前失败数仍为 77／7。本轮尚未修改线上方案、工单或业务数据。

线上验收暂被浏览器控制连接故障阻塞：控制通道重连失败，Chrome 原生操作也未成功，未改用直接写配置数据库。恢复连接后优先给方案 22 的成员和活动输入配置上述记录键，重新执行取得完整证据，再分别接受源人员引用缺失和历史活动缺日期；实际接受数量以新执行为准。最后重复执行验证原异常不重开、原始质量结果未被美化。此次续验只有就绪检查及只读 SQL 核验，文档执行 `git diff --check`；未重跑或新增宣称代码门禁。门禁专题、多引擎和身份归并继续不在本轮范围内。

2026-09-18 15:46–15:51 后续验收已完成，以上阻塞解除：浏览器总览发现接口仍报错，但已连接内置浏览器的标签页接口可用。Console iframe 表单点击存在焦点故障，因此沿用同一浏览器登录会话打开实际 Quality／Orchestrator 模块页面操作，未提取凭据、绕过认证或直接写配置数据库。

- 方案 22 保存为 V3，保留三个执行时输入、24 个检查项及原固定规则修订。activities 的稳定键为 `[activity_id]`，members 实际保存的有序复合键为 `[member_index, activity_id]`，persons 不配置键。本轮未改变默认物理表或规则严重级别。
- 手动检查 `5c48e314-536b-4737-bc7f-2d34ee34232c` 成功，仍为 19/24 项通过，5 项告警，error 失败为 0。根据完整证据分别接受工单 21 的 77 条人员引用缺失和工单 22 的 7 条历史活动缺日期，保留原因、操作者和执行依据；不将接受宣称为数据修复。
- 随后通过正式页面执行编排 10，执行 ID `be0337c3-09a3-426a-8aed-544f9a3e765e`，北京时间 15:49:20–15:50:36 成功，15/15 步完成。未编辑编排定义或开启定时调度；既有同步和计算按原流程刷新目标表，未直接补造源人员或活动日期。
- 四阶段子执行为 ODS `8bbf5396-baa1-4503-8dff-bca1d3a64547`、DIM `da614a02-4777-4ae5-8343-4506e1a58439`、DWD `de646943-985c-4ab8-8e97-4b6b6276c99d`、DWS `8238b62b-c770-411e-ad31-e6c7198fb7a1`。ODS 冻结方案 V3 和两组稳定键，三张物理表仍来自编排动态绑定。67 项检查仍为 58 项通过、9 项告警，无 error 失败；ODS 规则通过率仍为 79.1667%，未因接受异常提高分数。
- 复查后工单 21／22 版本均为 4，仍为 accepted，原始失败数及已接受数分别为 77／7，待处理数均为 0，最近观测更新为上述 ODS 子执行；两条人工接受审计各保留一条，说明未覆盖、没有重复建单。质量概览显示 7 个待处理工单、2 个已接受异常工单。线上没有人为制造新异常或复发，新异常／恢复后复发边界沿用前述自动化测试证据，不冒充本轮线上测试。

本次仅保存业务配置、人工处理工单、运行已有编排和更新本文档；无产品代码、API 或 CI/CD 入口变更。验收依据为实际页面、执行快照和 `BEGIN READ ONLY` 查询，文档运行 `git diff --check`；不重复宣称代码门禁或专用 T4 已重跑。下一步优先评估工单 20 的 21 条成员状态缺失是否影响参与统计，保留当前纳入规则，不继续历史 ID 归并或补造状态。

### 13.63 建模声明与实际加工一致性收尾（2026-09-18）

用户同意核查后的收尾方案。本轮通过 Model 正式页面将人员维度 5、活动维度 4 的历史策略从 SCD 0 改为 SCD 1，表达现有覆盖刷新、不保留属性历史的行为；并把活动参与事实 3 的 `person_id/activity_id`、人员指标汇总 6 的 `person_id` 标为 `dimension_fk`。关联端点仍使用既有 TableRelation，不新增关系或加工规则。四张逻辑表均完成重新审批，最终版本为表 3/V65、4/V36、5/V28、6/V42。

实施前确认无相关在途任务，全部 16 处数据元冻结引用与当前生效修订相同。审批后概念映射没有版本漂移；SCD 和字段角色不参与既有指标计算依赖哈希，未更换列、类型、主键、关联端点、物理目标或计算口径，无需创建新指标修订。Service 28/34/35 继续绑定修订 ID 12/14/15，明细服务 39/40 继续绑定 14/15；应用仍为发布修订 17。

正式编排 10 的四个 Model 节点改为“创建/校验人员维度表”“创建/校验活动维度表”“创建/校验活动参与事实表”“创建/校验人员指标汇总表”。汇总表节点移除过期的 `version=39` 覆盖，与其余三处一致使用 TaskProvider 当前审批版本的默认值。保存前后只读差异核对：15 个步骤、依赖集合、数据绑定、超时、布局和关闭的定时调度均不变；依赖数组顺序经编辑器重排，不改变依赖语义。

正式刷新执行 `714f70a7-61af-439d-bb3b-1e6d4dc7b584` 于北京时间 23:50:16—23:51:32 成功，3 个 Transfer、4 个 Model、4 个 Develop、4 个 Quality 子任务全部成功。沿既有流程覆盖刷新目标数据，没有修改源 MongoDB 或处理历史异常。质量工单仍为 7 个待处理、2 个已接受，其他已解决／忽略审计保留。用户在质量会话中已要求停止追查剩余历史异常，不延续 13.62 的末尾建议。

刷新后在应用 R17 执行默认全期查询，主领队次数 239/192、参加次数 290/197、共同活动 32、双向重叠率 0.110345/0.162437 均与调整前一致。页面将“SCD 类型”改为“维度历史策略”，中英文明确声明不会自动生成加工逻辑，类型 3 改为历史属性列而非“混合”；现有术语表和 Model 规范同步补充职责。本文首页更新为 15 步当前基线，旧计划和历史记录保留并明确其时间范围。

验证：`make test-module MODULE=model` 完整通过，包含 Platform T0、Go T1、前端 39 项确定性测试和 51 项浏览器测试及构建、MySQL 与标准 `addp_test` PostgreSQL T2；既有入口和 CI 自动发现已覆盖，无 API、Swagger、后端实现或测试依赖变化。真实业务页面验收与上述自动化门禁分别记载，不宣称重跑专用 T4。`git diff --check` 通过。本轮未提交代码。另观察到字段编辑器关联数据元名称显示 `undefined`，引用 ID 和冻结修订正常，作为后续展示问题单独处理。

### 13.64 数据元引用显示与冻结修订读取修正（2026-09-19）

用户同意继续修复 13.63 发现的名称问题。根因是 Model 前端仍读取数据元旧顶层 `name/data_type/length`，而 Standard 正式响应已拆为稳定身份和修订。逻辑表字段和实体属性现共用 Model 内的引用消费逻辑：候选只取 active 身份的当前已发布修订，不取未发布草稿；逻辑字段选择数据元后带入修订名称、类型和长度，未定义长度时清空旧值。已绑定但不再可选的身份保留只读显示，不自动解除引用。

已审批属性和字段按 `element_id + element_revision_id` 读取 Standard 精确修订，页面内重复引用去重。即使历史修订被撤回或当前名称变化，也不替换冻结名称；单个引用读取失败只提示该引用不可用，保留其余成功结果。实体审批／退回草稿后重载属性及引用，避免主状态已更新、冻结信息仍停留在前一状态。

实际人员维度页面已确认显示“人员标识 冻结修订 #11”“人员昵称 冻结修订 #4”，无需重启后端。本轮没有改动正式模型、数据元、物理表、指标或应用修订，没有运行户外数据刷新，也没有接管其他会话的指标／引擎工作。正式规范和 Model 模块说明已同步；只消费已有 API，不变更 Swagger 或后端接口。

验证记录：新增 5 项状态测试和 5 项浏览器用例，覆盖当前修订选择、长度清空、冻结名称、撤回修订、局部读取失败与实体生命周期刷新。首轮及第二轮各发现一处新增测试的控件定位问题，已按实际控件结构修正；最终完整 `make test-module MODULE=model` 退出码 0，Platform T0、Go T1、44 项前端状态测试、56 项浏览器测试、生产构建、MySQL T2 与标准 `addp_test` PostgreSQL T2 全部通过。`git diff --check` 通过。既有前端、模块入口和 CI 自动发现覆盖本次变更，没有新增依赖或测试入口；未运行专用 T4，没有提交代码。

### 13.65 冻结数据元修订的可追溯入口（2026-09-19）

实体属性与逻辑字段的“冻结修订”现统一为查看入口，由 Model 的 `FrozenElementLink` 组合共享 Standard route builder 与 Console 导航；无 `standard.element.read` 时禁用。Model 不复制数据元详情，不修改任何绑定或模型生命周期。

Standard 原详情页只在内存切换历史，刷新会选择默认修订。本轮在既有详情页补齐唯一 `revision_id` query：显式修订通过已有精确 GET 读取；历史选择同步地址，刷新、同组件切换恢复同一修订。修订不存在、无权限或参数无效时显示错误，不替换为当前版本；旧请求不得覆盖新选择。查看历史时显式创建新草稿，会进入新草稿身份。未新增 API、Swagger、后端状态、依赖或测试入口。

实际页面只读验收：从人员维度 5 的“冻结修订 #11”进入 Console `/standard/elements/15?revision_id=11`，显示 `outdoor_person_id`、“人员标识”、R1 已发布；刷新后内容与修订地址保持一致。没有点击草稿、发布、撤回等写操作，没有修改户外业务数据，也未运行编排或重启服务。

自动化验收由既有共享前端、Model、Standard 门禁及 CI 自动发现覆盖；新增共享路由身份校验与唯一所有权、两种模型入口、权限禁用、历史恢复、读取失败和新草稿跳转用例。`make test-common-frontend` 109 项通过；`make test-module MODULE=standard` 与 `make test-module MODULE=model` 均退出 0，覆盖 Platform T0、两模块 Go T1、Standard 71 项状态测试／62 项浏览器测试、Model 44 项状态测试／59 项浏览器测试、两模块生产构建、Standard／Model PostgreSQL T2 和 Model MySQL T2。PostgreSQL 仅使用标准 `addp_test`，T2 串行执行。`git diff --check` 通过。没有重跑全仓 `test-changed` 或专用 T4/T5；真实页面只读核验不等同于专用 T4。本轮未提交代码，其他会话的 Console／Service 改动保持原状。

后续建议：Standard 历史枚举数据元的码值集选框仍只从候选列表取得显示名称，应利用精确修订 API 已有的固定码值集快照补齐历史展示，避免旧码值集撤回后只显示修订 ID。它不影响本轮户外人员标识的文本型修订追溯，留待后续独立处理。

### 13.66 历史枚举数据元的码值集引用展示（2026-09-19）

承接 13.65，本轮仅补齐 Standard 数据元详情的引用展示，不修改业务数据、Model 绑定、码值集生命周期或指标／引擎实现。详情页默认选择和显式 `revision_id` 统一读取既有精确修订 API，以 `code_set_revision_id` 对应快照显示码值集名称、编码、版次和发布状态；缺失或身份不匹配明确报错，不以候选列表或当前版替代历史。

只读状态不请求码值集候选，也不增加 `standard.code_set.read` 权限要求。草稿保留已绑定但不可选版本的名称，以禁选项呈现；新选择仍限于原有类型、生效时间过滤的已发布候选，换绑后不继续展示旧版状态。详情与候选异步响应复用共享 `createLatestRequestCoordinator`，不再维护本页请求计数器；卸载后使未完成请求失效。本轮不新增快照表格、通用组件或平行 API。

新增 6 项浏览器用例覆盖默认／显式修订的已撤回码值集显示、只读权限不访问候选、草稿换绑、缺失／不匹配快照失败关闭，以及英文页面历史往返。首轮 71 项状态测试通过，67 项浏览器用例中 66 项通过；唯一失败为新用例点击被选中文字覆盖的下拉输入区，已改为点击实际选择器外框，并增加历史往返用例。最终 `make test-module MODULE=standard` 退出 0：Platform T0、Standard Go T1、71 项状态测试、68 项浏览器测试、生产构建和 PostgreSQL T2 均通过。PostgreSQL 仅使用标准 `addp_test`；现有 Make 入口、前端自动发现与 CI 登记已覆盖，无需新增测试入口、依赖或 workflow。API 契约与 Swagger 未变更；未重跑全仓 `test-changed` 或专用 T4/T5。`git diff --check` 通过，未提交代码；其他会话的 Service 改动不在本轮验收范围。

开发页面只读观察：人员标识 #11 刷新后正常显示；Standard 与 Gateway 就绪接口均返回 200。当前浏览器工具的点击操作持续报告目标失效（包括坐标点击），因此未将真实枚举详情计为验收通过，枚举场景由隔离浏览器用例验证。已恢复原人员标识冻结修订 URL，未执行草稿、发布、撤回或编排写操作，未重启服务。

下一步优先补齐数据元编辑页的未保存变更保护：当前 `ElementDetail` 未接入离页保护，切换历史修订或返回列表可能丢失本地草稿修改。应复用现有保护能力，并明确覆盖同一数据元的 `revision_id` 变化，不另建提醒流程。

### 13.67 数据元未保存修改保护（2026-09-19）

数据元详情现接入统一离页保护：返回列表、切换同一数据元的历史 `revision_id`、浏览器卸载，以及通过 Console 离开当前模块均消费共享 `useUnsavedChangesGuard`。Standard 的 `useUnsavedChanges` 删除本地路由守卫、确认框、卸载监听和重复词条，仅维护命名保存分区的基线。既有术语页同步使用该适配器，离页确认文字统一为共享词条；不改变术语业务模型和 API。

数据元治理信息与修订内容独立保存。成功只更新相应分区基线，保存修订不再重载并覆盖尚未保存的治理信息；保存失败保留输入和保护。保存期间禁止再次保存及编辑表单；有本地修改时，提交／发布／撤回／新建草稿先提示保存，不直接丢弃输入。保存响应通过共享请求协调器校验仍属于当前选中详情，迟到响应不能覆盖已切换的历史页面。离开页面不取消已经发给服务器的保存请求。

验证新增 2 项状态测试与 7 项浏览器用例，覆盖保存分区、共享唯一所有权、取消／确认返回、历史切换、两种分区保存顺序、保存失败、卸载拦截与解除、状态操作阻断、迟到保存响应和 iframe 修改状态协议。协议只传递是否修改与注册身份，不传递表单内容。首轮 73 项状态测试通过，浏览器 72／73 通过；唯一失败为新增测试误读请求记录 getter，已修正。第二轮浏览器 74／75 通过，iframe 夹具缺少父窗口认证协调器而进入只读状态；已直接复用共享 `createIframeAuthCoordinator` 初始化测试身份，未在生产代码增加权限分支。最终 `make test-module MODULE=standard` 退出 0，Platform T0、Standard Go T1、73 项状态测试、75 项浏览器测试、生产构建和 PostgreSQL T2 均通过；PostgreSQL 仅使用标准 `addp_test`。既有 Make 和 CI 自动发现覆盖本轮变更，不新增入口、依赖或 workflow；API 与 Swagger 契约不变。`git diff --check` 通过。未运行全仓 `test-changed` 或专用 T4/T5，未将隔离 iframe 夹具宣称为真实 Console 全链路验收。

本轮不修改户外业务数据，不创建草稿、不运行编排、不重启服务、不提交代码；其他会话的 Console／Service 工作保留。后续优先核查术语页的页内历史切换：它仍通过 `setRevision` 直接替换表单，不经过路由保护；需要将该入口纳入同一未保存确认契约，避免虽有离页提醒仍能在页内丢失修改。

### 13.68 术语历史修订的可恢复切换与未保存保护（2026-09-20）

术语历史选择不再直接替换内存表单，统一通过 `/glossaries/:id?revision_id=...` 和共享模块导航 replace；复用既有 `useUnsavedChangesGuard`，取消时保留 URL 与输入，重复点击当前修订不清除修改。切换、刷新均调用已有精确修订 GET，参数无效、修订不存在或无权读取时显示错误，不回退草稿或当前版。返回列表移除修订参数并保留筛选，新建草稿成功后进入新草稿 URL。加载及保存响应通过共享请求协调器校验归属，过期响应不能覆盖新选择。

同时修正组合保存的两个相邻缺口：保存开始时固定术语、修订和内容，避免切换后把第二次写入发给新选择；治理身份保存成功即更新聚合版本和对应分区基线，修订保存失败时保留修订输入，允许携带新版本重试。保存成功原位更新，不重载整个表单；保存期间禁用表单，有未保存修改时创建草稿和状态动作提示先保存。两次 API 写入仍非原子事务，离开页面不取消已经提交的保存流程。

新增 8 项路由状态测试与 10 项浏览器用例，覆盖无效身份、历史取消／确认、当前行重复点击、刷新和筛选恢复、只读历史、错误关闭、新草稿定位、两段保存各自延迟以及部分保存失败后重试。`make test-module MODULE=standard` 退出 0：Platform T0、Standard Go T1、81 项状态测试、85 项浏览器测试、生产构建和 PostgreSQL T2 均通过；日志位于 `/tmp/addp-standard-glossary-history-module.log`。PostgreSQL 只使用标准 `addp_test`；现有 Make、自动发现与 CI 已覆盖，无需新增入口、依赖或 workflow。API 契约与 Swagger 未变化，路由规范和 Standard 模块文档已同步。未运行全仓 `test-changed` 或专用 T4/T5，不将受控浏览器夹具计为真实 Console 全链路验收。

本轮未修改真实户外数据、运行编排、重启服务或提交代码，其他会话的 Console／Service 和书稿文件保持原状。下一步优先补齐术语治理信息的独立保存：当前已发布／历史修订页面仍允许编辑范围、归属域和标签，却只有可编辑草稿才显示组合保存按钮；应使稳定身份的保存不依赖草稿状态，并保留修订分区的未保存保护。

### 13.69 术语治理信息与修订的独立保存（2026-09-20）

术语治理卡片增加独立保存入口，有更新权限时不依赖草稿状态，已发布修订页面也能保存适用范围、归属域和标签。顶部保存只更新当前可编辑草稿；删除原有 `saveAll` 组合写入，不保留新旧双轨。两个入口分别校验自身字段、调用既有身份／修订 API、更新聚合版本和对应保存基线，不提交或覆盖另一区域的输入。治理保存不要求未完成草稿通过修订校验，修订保存也不提交尚未完成的归属调整。

继续复用共享离页守卫、请求协调器及 Standard 分区基线适配器。两种保存互斥，保存期间禁用表单和另一保存入口；失败保留输入与未保存提醒，迟到响应不得覆盖新选择。按钮复用现有中英文“保存”词条，以所在卡片区分职责，没有新增通用组件、API、Swagger 契约、依赖或测试入口。Standard 模块文档已同步新保存契约；13.68 的组合保存描述仅保留为历史实施记录。

新增 5 项浏览器用例，并更新原有保存、失败和延迟用例，覆盖两种保存顺序、请求字段隔离、聚合版本推进、发布态保存、中英文及英文窄屏、独立校验、权限禁用、失败保护和历史切换竞态。首轮 `make test-module MODULE=standard` 的 Platform T0、Standard Go T1、81 项状态测试通过；浏览器 89／90 通过，唯一失败为新增用例点击了被选中文字遮挡的 Select 内部输入框。改为点击实际选择器外框后，`make test-standard-frontend test-standard-postgres` 退出 0：81 项状态测试、90 项浏览器测试、生产构建和 PostgreSQL T2 全部通过。复用首轮仍有效的 T0／Go T1 证据，不将首轮完整命令误报为成功。日志分别为 `/tmp/addp-standard-glossary-ownership-module.log` 和 `/tmp/addp-standard-glossary-ownership-verified.log`；`git diff --check` 通过。

PostgreSQL 仅使用标准 `addp_test`。现有 Make／CI 自动发现覆盖变更；未运行全仓 `test-changed` 或专用 T4/T5，不宣称真实 Console 全链路验收。未修改真实户外数据、重启服务、运行编排或提交代码，其他会话工作保持原状。下一步优先处理术语“管理关联数据元”的保存：其成功后仍调用整页 `load()`，会覆盖未保存的治理或修订输入，应改为只更新关联区及聚合版本。

### 13.70 术语关联数据元的局部更新与失败恢复（2026-09-20）

关联保存删除整页 `load()` 路径：仅提交固定的数据元身份集合，PUT 成功立即更新术语聚合版本，再通过已有 GET 刷新关联列表；治理信息、修订内容和两区保存基线均保持不动。关联写入与两种表单保存互斥，保存期间禁用选择器、取消及其他保存入口，重复点击不重复提交。写入失败保留弹窗及选择；写入成功但列表读取失败则明确显示“关联已保存”，刷新按钮只重试 GET，避免误报保存失败或重复写入。

详情、关联列表和候选搜索分别复用共享请求协调器。切换详情关闭关联弹窗并使旧请求失效，离开页面后已发送的保存请求仍可完成，但其响应不覆盖新页面。候选搜索仅接受最新请求结果；不增加私有计数器、导航守卫或 API。中英文提示和 Standard 模块说明已同步，API／Swagger 契约不变。

新增 8 项浏览器用例，覆盖保存后保留两个未保存分区、仅重读关联列表、聚合版本推进、失败重试、取消无写入、单次提交、中英文部分成功恢复、历史切换和离页后的迟到响应，以及候选搜索乱序。首轮 `make test-module MODULE=standard` 的 Platform T0、Standard Go T1、81 项状态测试通过；浏览器 97／98 通过。唯一失败为测试在禁用的 Element Plus 多选框中寻找已被组件移除的输入框；核对组件实现后改为断言选择器禁用样式及输入框不存在，未修改生产逻辑。

最终 `make test-standard-frontend test-standard-postgres` 退出 0，81 项状态测试、98 项浏览器测试、生产构建和 PostgreSQL T2 均通过，复用首轮仍有效的 T0／Go T1 结果。日志为 `/tmp/addp-standard-glossary-mappings-module.log`、`/tmp/addp-standard-glossary-mappings-verified.log`。`git diff --check` 通过，现有 Make／CI 自动发现已覆盖，无需新增入口或 workflow；PostgreSQL 只使用标准 `addp_test`。未运行全仓 `test-changed` 或专用 T4/T5，未修改真实户外数据、重启服务、运行编排或提交代码，其他会话工作保留。

下一步优先补齐关联选择器的已选项回显：目前打开弹窗只加载前 50 个候选，已有关联不在当前候选页时可能只显示数字 ID。应消费已有映射列表中的名称与编码保持选中项可辨认，不改变关联身份或引入新 API。

### 13.71 术语关联选择器的已选项回显（2026-09-20）

打开关联弹窗时直接消费已有映射的名称与编码，已选数据元不在当前 50 条候选中也能正确回显；后续搜索、空结果或查询失败均保留当前已选项的展示信息。选项按数据元 ID 合并去重，只保留当前已选项和最新搜索结果，不累计未选中的历史候选。取消选择会移除其固定选项，取消整个弹窗后重新打开从已保存映射恢复。保存仍只提交身份集合与并发版本，不新增接口、逐项详情查询或共享组件。

新增三组浏览器用例覆盖首批候选缺失、加载期间回显、连续搜索与失败、身份去重、历史候选清理、取消恢复及解除关联的请求内容。Standard 模块文档先同步展示契约，再修改组件与测试；现有 Make／CI 自动发现已覆盖，不新增依赖、入口或 workflow。

首轮 `make test-module MODULE=standard` 的 Platform T0、Standard Go T1、81 项状态测试通过，浏览器 100／101 通过。唯一失败为新测试将“没有逐项详情查询”误写成“候选列表只查一次”；核对 Element Plus 展开下拉框会调用远程查询后，改为断言所有数据元读取均为列表请求。最终 `make test-standard-frontend test-standard-postgres` 退出 0，81 项状态测试、101 项浏览器测试、生产构建及 PostgreSQL T2 均通过，复用首轮仍有效的 T0／Go T1 结果。日志为 `/tmp/addp-standard-glossary-selected-labels-module.log`、`/tmp/addp-standard-glossary-selected-labels-verified.log`；`git diff --check` 通过。PostgreSQL 仅使用标准 `addp_test`，未运行全仓 `test-changed` 或专用 T4/T5，不把模拟 API 浏览器用例作为真实业务端到端证明。未修改真实户外数据、重启服务、运行编排或提交代码，其他会话工作保留。

接口核查另发现一个未在本轮修改的问题：`UpdateElements` 校验同租户 active 稳定身份并整体替换关联，但 `GetMappedElements` 只返回当前已发布且生效的修订。关联未发布数据元或既有关联暂时无生效修订时，读取结果会缺项；前端基于该集合再次整体保存存在删除未显示关联的风险。下一步应先确认并统一关联管理的读取语义，建议保留稳定身份关系、单独展示生效状态，而不是把“当前生效标准检索”直接当作“完整关联管理列表”。本轮没有用前端缓存掩盖该后端契约问题，也没有改变关联允许范围。

### 13.72 术语关联管理与当前生效标准解析分离（2026-09-20）

承接用户确认，先补齐术语表和 Standard 模块契约，再将关联 GET 从 `PublishedElementReference` 改为独立的 `GlossaryElementReference` 管理投影。一次查询完整返回同租户已关联身份，不分页，不因修订草稿、审核中、待生效、过期、撤回或身份清理状态而裁剪集合。展示取当前生效修订，否则取最高版次修订；无修订时保留稳定编码及空修订信息。独立的 `is_effective` 只表达展示修订当前是否生效，不意味着关联冻结到该修订，也不代替身份生命周期。

写入继续使用原有 PUT、术语聚合版本及同租户 active 身份校验，不放宽可新增关联范围、不新增兼容 query 或第二套接口。前端保留完整身份集合，分别显示展示版次、修订状态和当前生效情况，不可用身份显式提示；普通刷新或再次保存不再把无生效修订的关联隐式删除。真正解除关联仍是用户显式选择。跨模块正式标准解析保持原有发布与生效限制，未修改 Model、指标计算或引擎实现。

API 用例覆盖首次保存与完整读取、无修改再次保存、旧版本冲突、跨租户拒绝、显式解除及空数组；真实 PostgreSQL 用例覆盖六种修订场景、有效修订优先于后续草稿、撤回后关联身份不变、无修订及清理中身份可见。新增 PostgreSQL 用例已登记到现有 `standard-postgres-gate.sh`，沿用既有 Make／CI owner 门禁；浏览器用例覆盖中英文状态区分、完整 ID 提交及刷新恢复。Swagger 响应 DTO、双语说明和生成产物同步更新，不新增 Schema 或持久化字段。

`bash scripts/swagger/gen-swagger.sh standard` 和 `bash scripts/swagger/check-route-coverage.sh standard` 均通过，覆盖 118 个公开路由方法。首轮 `make test-module MODULE=standard` 的 Platform T0、Standard Go T1、81 项状态测试通过；浏览器 102／103 通过，唯一失败是新中文测试把既有标题“关联的数据元”误写为“关联数据元”。核对页面快照后修正测试定位，未修改生产逻辑；最终 `make test-standard-postgres test-standard-frontend` 退出 0，包含新增真实数据库用例的 PostgreSQL T2、81 项状态测试、103 项浏览器测试和生产构建全部通过。复用首轮仍有效的 T0／Go T1 结果，不将首轮完整命令误报为通过；日志为 `/tmp/addp-standard-mapping-identity-module.log` 与 `/tmp/addp-standard-mapping-identity-verified.log`。

`git diff --check` 通过；PostgreSQL 仅使用标准 `addp_test`，新增数据库夹具在事务回滚时清理。未运行全仓 `test-changed` 或专用 T4/T5，未修改真实户外数据、运行编排、重启开发服务或提交代码，其他会话工作保留。后续优先在现有服务启动方式下加载新版 Standard，再回到户外域做只读页面核验；本轮隔离门禁不代表当前运行中的旧后端已经切换响应契约。

### 13.73 运行版本与户外建模页面只读复核（2026-09-20）

现有外部启动的 Standard 已于本地时间 23:38 更新，无需本会话接管或重启。`GET http://localhost:8110/health/live` 的 `source_fingerprint` 与使用 `scripts/dev/build-identity.sh` 的 `addp_source_fingerprint standard/backend ./cmd/server` 对当前工作区计算的结果一致，均为 `sha256:ef8d7cbc45211f6eace903e51c445008ba0471239bf1f392e6babd0137ccee3c`。Standard 和 Model 的 `/health/ready` 均为 ready；运行中的 Standard `/swagger/doc.json` 已返回 `GlossaryElementReference[]` 及 `is_effective` 等新字段。本次只核对 Standard 源码指纹，不以 Model readiness 代替其版本一致性验证。

在真实 Console 会话中，通过现有可恢复路由只读打开并确认：

- 人员维度 5 的“物理目标”页集中展示预览建表语句、创建／校验、统一监控和删除目标表；目标引擎为 `Business PostgreSQL (postgresql)`，目标位置为 `outdoor`，目标表为 `dim_outdoor_person`。本次未执行任何建表、校验或删除动作，不据此判断物理表实时结构。
- 同一表的“概念实现映射”显示代表人员实体 `outdoor_person`，版本状态为同步；`person_id` 和 `person_nickname` 分别直接对应人员标识和昵称属性，无需第二套实体型中间逻辑表。
- `/standard/elements/15?revision_id=11` 显示人员标识、`outdoor_person_id`、R1 已发布，与人员维度的冻结修订入口一致。本次通过地址打开，不冒称完成点击跳转验收。
- 当前主领队术语 9 正常展示 R2 已发布及已确认的 `leader.personid` 优先、原始 `members[0].personid` 次之规则，但当前页面显示“暂未关联数据元”。该真实样例不足以验证非空关联列表的新状态列，本次未为验收新增业务关联。

本次复核不把“初始发起人”作为当前建模阻塞项：它不是当前领队指标和参与事实的必要语义，既有术语和历史记录保留，不为此改写已发布修订或启动新链路。

浏览器读取、截图与地址导航正常，但 iframe 页签的语义点击和坐标点击均返回目标不可用；改用已有路由完成只读页面检查，没有修改应用规避工具限制。按钮交互、关联保存及完整运行链路未在本轮验收。仅追加本节记录，未修改代码、真实业务数据、服务环境或提交代码；没有重跑代码门禁，前轮隔离测试证据与本轮真实页面证据分开记载。文档检查入口为 `git diff --check`。

### 13.74 当前领队判定口径收敛（2026-09-21）

用户确认本阶段只需确定活动的当前领队，不再扩展“初始发起人”语义。当前领队统一按以下单一路径计算：

1. `Outdoors.leader.personid` 非空时，直接作为当前领队；
2. `leader.personid` 缺失时，使用原始有序成员数组 `members[0].personid`；这里的 `0` 必须对应 ODS 保存的 `member_index=0`，不能先按状态过滤后再取第一条；
3. 两者都缺失时保留 `NULL` 并产生质量告警，不猜测其他成员，也不使用 `Outdoors._openid` 或 `Persons` 侧摘要数组替代。

该口径沿用 13.60 已确认的业务规则；13.73 已只读核对“当前主领队”术语 9，本节不代表重新核验全部指标发布修订。`members[].entryInfo.status=领队组` 仍只表达协助角色，不能覆盖当前领队关系。现有文档中关于“初始发起人”的内容降为历史背景，不作为当前 Outdoor 建模、维度加工、指标计算或引擎联调的前置条件。不新增第二套领队字段，不改动指标／引擎接口。

### 13.75 当前领队优先级的物化结果只读核验（2026-09-21）

只读查看 Develop 任务 52 `outdoor_dim_activity_from_ods_refresh` 的当前 SQL 配置，活动维度使用 `COALESCE(source_leader_person_id, member_index=0 的 person_id)` 计算 `current_leader_person_id`；成员输入明确说明不按成员状态过滤。任务 53 只消费活动维度的治理结果，不重复实现领队优先级；当前领队单独并入参与事实，成员状态只决定成员来源的参与资格。

只读查询 Business PostgreSQL 的实际物化表和 ODS 源表，结果如下：
...
| 当前领队为空 | 0 |
| 与 ODS 优先级重算不一致 | 0 |
| `leader` 缺失、使用 `members[0]` 回退 | 139 |
| 两个来源均缺失 | 0 |
| 两个非空来源发生冲突 | 0 |
| DWD 参与事实行数 | 4,899 |
| DWD 当前领队事实行数 | 681 |

进一步通过 `docker exec business-postgres psql -X -v ON_ERROR_STOP=1 -U business -d business` 执行只读事务（`BEGIN READ ONLY`、10 秒语句超时、末尾 `ROLLBACK`），核对六项关系差异：ODS 与 DIM 活动集合缺失／多出、领队优先级不一致、有效活动领队在 DWD 中遗漏、多出、重复，以及 `is_current_leader` 错标，差异数全部为 0。这补足了仅比较行数不能证明关系一致的缺口；139 条回退统计覆盖全部活动，不等于有效活动范围的回退数量。

因此当前保存的加工配置及已物化领队关系符合“`.leader` 优先、`members[0]` 回退”，无需为本次确认新增产品代码或改动另一会话的指标／引擎实现。验证过程中首次误将配置读取指向 `addp_test`，因没有 `develop.dev_tasks` 而失败；随后改为平台库 `addp` 的限定 ID 只读查询，业务结果则在独立 `business-postgres` 的 `business` 库读取，未创建或修改测试数据库。

本轮只调整两份既有业务文档，不改代码、任务、质量规则、业务数据或服务环境，不运行编排、不提交代码。文档检查入口为 `git diff --check`；未重跑 T0–T5 产品门禁，本次 SQL 诊断不作为自动化门禁。浏览器仅完成活动维度加工任务页面的部分读取，点击／键盘操作仍失败；随后活动维度概念映射页进入登录页，该页本轮未验收。下一步回到 Model 核对“活动—当前领队”概念关系与维度关联的追溯是否完整；不再重复调整已验证的数据加工口径。

### 13.76 活动实体到当前领队的 Model 追溯补全与剩余边界（2026-09-21）

通过 Model 自有库只读查询并在正式 Model 页面完成生命周期操作后，核对逻辑表 4「活动维度」：

- 活动实体 `outdoor`（实体 ID 7）已新增并审批“人员 → 活动（一对多）”关系 `current_leader_of`（实体关系 ID 9，人员实体 ID 29）。其描述明确：每场活动最多一名当前主领队，来源按 `leader.personid` 优先、缺失时回退 `members[0].personid`；初始发起人不进入当前领队语义。
- 活动、人员实体以及引用它们的活动参与事实、活动维度、人员维度均已通过正式页面退回草稿、保存概念映射并重新审批；三张逻辑表的实体版本引用均已恢复为“同步”。
- 活动维度仍保存 `current_leader_person_id`（字段 42）和 `current_leader_nickname`（字段 43），但当前概念实现映射模型只支持 LogicalField → EntityAttribute，不能把字段直接映射到 EntityRelation；这两个字段因此继续作为加工结果字段，不伪造为普通实体属性映射。
- 当前表关系仍只有参与事实 → 活动维度、参与事实 → 人员维度，以及人员指标 → 人员维度。Model 的唯一 TableRelation 编辑入口要求出向源表为事实表，不能创建“活动维度 → 人员维度”的维度间关系；现有参与事实 `person_id` 外键表达的是参与人员，不能冒充当前领队关系。

因此当前领队计算和指标结果已经正确，活动实体层也已经有了权威关系；剩余的是 Model 的跨层追溯能力边界，而不是 Develop 或指标引擎缺陷。若要让 Model 页面继续回答“当前领队 ID 字段如何由活动实体关系实现”，需要后续在文档确认后增加关系派生字段映射或维度间引用的单一路径；当前不通过错误的 TableRelation 或第二套字段绕过。

本轮 Model 页面和数据库只读核验结果如下：实体 7、29 为 `approved`；关系 9 为 `29 → 7 / one_to_many / current_leader_of`；逻辑表 3、4、5 均为 `approved`，对应概念映射均为 `同步`。未修改代码、任务、质量规则、真实业务数据或另一会话负责的指标／引擎配置。
