...
- 候选人员维度单路线已完成源端只读试算：以 `Persons` 为高优先级主数据，再合并与参与事实相同活动/成员范围内的 `Outdoors.members` 及当前 `leader` 身份快照，按 `person_id`、来源优先级、活动日期和活动 ID 确定性去重，共得到 2,207 人，其中 `Persons` 2,188 人、历史活动快照补齐 19 人，19 人均可取得昵称。建议增加唯一来源字段 `person_record_source=persons|activity_member_snapshot`，不再同时增加冗余 `is_inferred`；任务 source locator 改为 `Outdoor` database 级 locator，以如实表达该查询读取同库多个 collection。该方案尚未实施，需先确认“人员维度覆盖所有被治理事实引用的身份”这一业务语义。
