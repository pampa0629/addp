...
1. 概念上不作为子任务。
2. UI 上提供“先优化再生成瓦片缓存”的分步引导；空间预览中的动态 MVT 超时建议使用节流消息提示，避免按瓦片请求频率重复刷屏；可操作入口收敛在工具栏中的“执行快显优化”或“生成瓦片缓存”按钮。
...
5. 当前阶段不新增“多尺度快显优化”用户入口；ready 3857 目标仍超时的用户闭环统一落到“生成瓦片缓存”。
