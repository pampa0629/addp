...
2. Basic scan 按已注册扩展名或 MIME 保存 DWG / DXF 基础候选，不读取文件内容。
3. Deep scan 通过存储引擎 content reader 读取文件头，确认格式并写入版本与最小 CAD 类型事实，不依赖工作流引擎。
4. Manager 基础预览通过受控 `storage-stream` 交付源文件，前端按需加载 LibreDWG Worker/WASM 并通过 WebGL 绘制；不生成后端栅格瓦片。
5. Transfer 只复制原始 CAD 文件，不隐式执行 CAD→GIS 或 DWG↔DXF 转换。
6. 扫描和预览都不依赖 SuperMap；后续 CAD→GIS 导入使用何种 provider，在 `cad.import` 设计时另行确定。
...
基础扫描不建模为 Runtime 算子。后续 `cad.import` 也不得复用预览状态或 Manager 任务。
...
4. 补齐浏览器端最大源文件大小、解析时间、layout / layer / Xref 数和 Worker 内存预算验证。
