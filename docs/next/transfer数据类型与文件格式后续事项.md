...
- [Transfer 与格式能力整合方案](../plan/transfer与FormatProvider整合方案.md)
...
- [ADDP 格式与数据类型能力消费者调研](../plan/addp格式与数据类型Provider消费者调研.md)
...
FormatPlugin、info provider、content reader 不接 `engine_id`，不构造 engine reader。Transfer 编排层先根据 engine capability 组装资源抽象，再把资源交给格式与数据类型能力实现。
...
4. 最后逐步用 resource + FormatPlugin / reader adapter 替换旧 reader / writer。
