...
达梦 DM8 的 ARM64 技术验证只保留 `dm8_20260708_HWarm920_kylin10_sp1_64.zip` 一条官方介质路线：ZIP SHA-256 固定为 `d6871147cd4a04e1595d9dedf9d05245c55b17bff2ae738dded37fe568df9824`，其中 ISO SHA-256 固定为 `2a8a4844527e901718a88b4c747d7fd44460bcb2a862956bba98146760b8423e`，服务端版本标识为 `03134284604-20260707-335949-20228`。该介质的厂商目标环境是鲲鹏 920 与麒麟 10 SP1；macOS ARM64 Docker Desktop 只能在 `linux/arm64` Ubuntu 兼容层中做非认证 ABI 技术验证，不能据此扩展为通用 ARM64 或生产支持声明。Business 只构建本地、不可推送的无卷 disposable 镜像，介质与镜像均不进入仓库或 Artifact；内置试用授权固定到期日为 2027-07-07，到期后必须停止使用并改为 owner 正规申请、与正式 ARM64 环境匹配且 SHA-256 明确的 License，不得通过重建容器、替换时钟或第三方镜像绕过许可。

DM8 官方 ODBC `libdodbc.so` 及其依赖只在同一个 Linux ARM64 容器边界内接受验证；macOS ADDP 进程不能加载 Linux `.so`，因此该技术夹具和 T5 不登记 `engine_type=dameng`，不创建 Engine Instance，也不声明 capabilities。仓库只在 build tag 隔离的认证测试中使用开源 ODBC bridge，不提交、打包或重分发达梦官方 Go 驱动及其限制性授权内容。正式 Provider、SQLDialect、T2 与跨模块 T4 的前置条件是先取得可合法分发并覆盖 ADDP 目标宿主平台的驱动路线；在此之前不得用容器内代理、上层数据库类型硬编码或伪造能力声明绕过该边界。
