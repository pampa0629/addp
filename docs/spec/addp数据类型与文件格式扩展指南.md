...
新增 CAD 格式时必须优先判断是否需要保留 CAD 原生图层、块、布局、标注等语义。DWG、DXF 使用既有 `data_type=cad`，不得因为 entity 可投影为行而归为 `table`；CAD→GIS 必须作为显式转换生成新的 table item。二维 CAD deep scan 只通过存储引擎 content reader 读取文件头，Manager 预览只通过受控源文件流和浏览器端 LibreDWG/WebGL 完成；两者都不依赖工作流 Runtime，也不保留第二解析或预览路线。
