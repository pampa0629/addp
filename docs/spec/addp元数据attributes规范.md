...
`type_info.cad` 只承载跨 CAD 格式稳定的图纸结构摘要。DWG、DXF 版本、SuperMap provider/version、解释后的 dataset 数量、归一化 Geometry 提示和扫描 warning 分别进入 `format_info.dwg`、`format_info.dxf`；预览瓦片、缩略图、entity 样本和 CAD→GIS 产物不得写入 attributes。Meta deep scan 不得为计算摘要遍历 Recordset Geometry。
