...
“接受当前历史异常”是对本次完整观测中具体失败记录的人工处理，不是忽略规则、修改合格结论或解除执行阻断。正常治理状态为 `open/resolved/accepted`；既有 `ignored` 仅用于历史事实及引擎清理，不再提供人工忽略入口，也不等于记录级接受。失败行数、规则通过率和 error 阻断语义不变。当前失败记录全部已接受时显示 accepted，混有未接受记录时仍为 open，并分别显示已接受数和待处理数。

方案 `table_bindings[].record_key` 可显式声明最多 8 个稳定记录键字段（有序复合键），不自动推断主键。逐行检查在同一只读可重复读事务内验证全表键非空且唯一，并取得完整失败键集合。只保存规范键表示的 SHA-256，不存储原值或行样本；该摘要不是匿名化承诺，不向客户端输出。每条规则最多保存 10000 个失败键，超限、不支持的聚合规则、未配置键或键不满足条件时，返回明确的证据不可用原因，禁止接受，不把部分样本当完整集合。键字段不存在属于配置错误。

接受范围同时绑定工单目标集合、规则修订及实际约束/字段绑定、稳定键定义。重新检查时只保留仍连续失败的已接受记录；已恢复的记录立即退出接受集合，日后复发重新待处理；新增记录即使失败总数不变也重新待处理。规则定义/绑定/稳定键改变或证据不可用时不延续接受范围。查询异常不提交部分对账，亦不表示恢复。

工单持有正整数并发 `version`；每次完整观测和人工处理原子推进版本。`PUT /issues/{id}/status` 必须提交 version、status（resolved 或 accepted）及 note；accepted 只能接受用户所见版本的完整失败集合，旧版本返回 409 并保留输入。处理历史追加写入独立审计表，保存租户、工单/方案、执行、操作、操作者、时间、说明及接受范围证据；复查、重开、方案删除均不清除审计。工单详情返回处理历史，不返回记录键摘要。存量处理说明（包括 ignored）迁入审计且保持原状态，不伪造成精确记录接受；之后完整失败观测仍会重开没有接受证据的工单。不保留 ignored 人工写入路线，不改写执行历史。租户物理删除仍按平台清理协议删除其全部事实。

Quality migration 16 增加工单版本、记录证据与处理审计，SchemaVersion 升为 4。Backend/Worker 必须同步更新并排空活动执行；记录键随现有执行快照冻结，执行参数仍只覆盖 ResourceLocator。
