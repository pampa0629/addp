...
本文定义 ADDP 数据质量规则、规则应用、字段检查任务、数据校验任务、execution、评分结果和质量问题的唯一语义与实现边界。本文是 Quality 与 Standard 模块相关实现的正式规范；`docs/plan/` 中的早期数据治理规划只作为背景材料，不作为当前契约。
...
4. Quality 以持久 execution 执行字段检查或数据校验，并把结果摘要写入 `common.task_executions.metadata`。
...
字段检查第一版只支持 PostgreSQL 单表字段规则。数据校验第一版只支持同一 PostgreSQL Engine 上 正式物理表的六类强类型断言。Quality 不提供 owner 定时调度、事件触发、自定义 SQL 规则或其他数据库方言。
...
| Quality | 拥有基于标准映射的规则应用、检查任务、数据校验任务、执行 worker、符合性结果、评分和问题状态 | 不修改 Standard 数据元，不创建第二套字段标准映射，不创建或发布 Model 物化表，不复制引擎凭据，不自建 execution 历史表 |
| Model | 负责逻辑模型与独立建表 | Quality 不依赖 Model，不读取其内部状态 |
...
DataValidationTask 替换专用数据校验，唯一任务类型 `data_validation`、API `/data-validation-tasks`。任务保存名称、代码、并发版本、`table_bindings: [{alias, locator}]` 和原有六类强类型断言；绑定必须为同一 PostgreSQL Engine 中已存在的正式表 ResourceLocator。删除 Model 组 ID、组版本、逻辑表 ID、批次与物化读上下文依赖。

由 Orchestrator 调用唯一 TaskProvider 执行路线，执行快照冻结绑定和断言，通过父 execution 派生 Quality 精确 read 授权。运行时读取引擎字段事实，在一个只读一致性快照内完成全部断言；不写入目标，不创建或替换表。error 断言失败将 execution 置为 failed，Orchestrator 阻止后续步骤；不能回滚其他任务已提交的数据。结果保留断言计数和详细观测，不再输出发布组期望或批次身份。

已有 Outdoor 16 项校验迁入该唯一契约；旧数据校验配置、路由、权限与任务类型删除，不旁路保留。

## 5. PostgreSQL SQL 编译

### 5.1 方言边界

Quality v1 只允许目标 Engine Plugin 为 PostgreSQL。API、任务创建、规则应用创建和执行前都必须校验该事实；前端只展示 PostgreSQL Engine Instance。数据校验的物理定位只能来自 Model 物化读上下文，不从 Quality 任务定义或前端接收。
...
6. 数据校验只能由强类型断言编译器生成参数化 SQL，不得把 `message`、`value`或任何任务文本当作 SQL 片段。

Quality 的 `failed` 或 `timeout` 终态 execution 必须在 `error_details.code` 使用稳定领域错误码；原始数据库、SQL、连接和外部服务错误只写服务日志，不得返回给前端。字段检查 v1 错误码沿用现有 `quality.authorization.*`、`quality.execution.*`和 `quality.issue.*`命名空间；数据校验额外使用 `quality.data_validation.config_invalid`、`quality.data_validation.read_context_failed`、`quality.data_validation.unsupported_engine`、`quality.data_validation.authorization_failed`、`quality.data_validation.assertion_compile_failed`、`quality.data_validation.sql_execution_failed`、`quality.data_validation.assertion_failed`和 `quality.data_validation.result_invalid`。超时码仍为 `quality.execution.timeout`，失败兜底码仍为 `quality.execution.failed`。前端必须使用同一映射按错误码本地化展示终态原因；Monitor 列表和 Quality 领域详情都只能展示该稳定原因，不得展示持久化的安全摘要或内部错误文本。
...
- 数据校验只允许 Orchestrator 子 execution；先以 `addp-quality` 机器身份和 `model.materialization_read.execute` 精确 Permission 从 Model 取得读上下文，再从父 execution 派生同一 Engine 的 read 授权。
...
2. 同一事务创建 `common.task_executions(status=pending, started_at=NULL)`，冻结对应 task type 的版本化 execution 配置、任务目标和超时预算，并更新任务最近执行摘要。`check` 额外冻结启用的 RuleApplication 快照；`data_validation` 额外冻结父 execution、逻辑表绑定和断言快照。
...
- DataValidationTask 业务 API 唯一路由为 `GET|POST /api/v1/quality/data-validation-tasks`和 `GET|PUT|DELETE /api/v1/quality/data-validation-tasks/{id}`；不提供模块内直接 run 端点。
- TaskProvider 统一使用 `GET /api/v1/quality/task-provider/tasks`、`GET /api/v1/quality/task-provider/tasks/{task_type}/{id}`、`POST /api/v1/quality/task-provider/tasks/{task_type}/{id}/execute` 和 `GET /api/v1/quality/task-provider/executions/{execution_id}`；`task_type` 只允许 `check|data_validation`。整组路由仅允许 `addp-orchestrator` Service Client 的固定 Guard 与 `quality.task_provider.read|execute` 保护，不得向 User Principal 或 Quality 前端暴露。
- Quality 领域执行投影使用 `GET /api/v1/quality/executions` 和 `GET /api/v1/quality/executions/{execution_id}`，以 `monitor.execution.read` 读取当前 Tenant 中 `module=quality` 且 `task_type IN (check, data_validation)` 的执行事实。前端模块级执行历史统一进入 Monitor；Quality 只保留质量评分、规则明细和门禁断言详情。Quality cleanup 的 `cleanup_executor` 不属于业务质量任务执行历史，不得出现在该投影或详情中。
...
- CheckTask 稳定身份唯一索引。
- DataValidationTask `tenant_id + code` 唯一索引、`version > 0` check constraint，以及列表所需 `(tenant_id, updated_at DESC, id DESC)` 索引。
...
- 自定义 SQL、字段检查中的跨字段/跨表/聚合业务规则，以及数据校验已定义六类断言之外的任意表达式
...
9. 数据校验六类断言的文档校验、标识符引用与参数绑定，包括允许值集合、组合键、NULL 语义、predicate operator 白名单和 row_count 边界。
