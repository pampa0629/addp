...
Develop 为所选资源生成查询模板时，对模板可执行性的验证属于真实数据读取，必须把同一个有界验证查询准备为 `PreparedQuery`，按 `query` 动作经过上述 ReadSet、OutputLineage 和结果保护路径后执行。验证成功后只向调用方返回 Provider 生成的原始模板，不返回验证结果。该入口不得使用“资源必须未纳管”的 `BeginCatalogPath` 门禁替代字段级查询保护，也不得另行提交一份绕过保护的验证查询。Service 表模式必须从发布快照的输出契约枚举完整源字段构造基础查询，再由结构化查询计划收窄请求输出；不得用 `SELECT *` 使可证明的直接表查询退化为血缘未解析。

第一阶段以 MongoDB 和 PostgreSQL 作为完整验证范围，并补充 MySQL 的窄范围精确查询门禁。MongoDB PreparedQuery 的 ReadSet 必须覆盖主集合以及 `$lookup`、`$graphLookup`、嵌套 `$unionWith`；PostgreSQL PreparedQuery 的 ReadSet 必须按真实 `search_path` 解析关系并递归展开普通视图。PostgreSQL 普通标量、聚合和窗口函数只有在 Provider 根据当前连接目录证明全部可匹配候选均为不新增数据源的可信 `pg_catalog` 内置实现时才继续分析；该证明不使用函数名白名单，用户函数、扩展函数、表函数、集合返回函数和副作用边界不明的函数继续 fail-closed。MySQL 只在方言 AST 完整解析、没有普通函数调用，且当前连接目录确认 JOIN / 派生子查询的所有引用都是非系统 database 中的 InnoDB `BASE TABLE` 时返回完整 ReadSet；View、FEDERATED 等可引入额外数据源的存储引擎、存储函数、UDF、临时表和无法证明的语法继续 read-set unresolved。命中纳管 MySQL DataItem 后，OutputLineage 进一步只接受能从实时表结构唯一证明的直接列和显式别名，并跨 JOIN 与带别名派生子查询逐层组合 direct bindings；wildcard、表达式、聚合、UNION、重复输出名、歧义或不存在的列继续 output-lineage unresolved。MySQL-compatible Provider 生成样例查询时必须从实时 Catalog fields 枚举直接列，确保模板自身能够进入该窄范围保护路径；这不扩大任意用户查询对 wildcard 的支持。过滤、连接、排序或分页引用但不输出字段值的来源可以保留空 bindings。PostgreSQL 结果血缘开放直接列、显式别名和可证明的单来源 wildcard；MongoDB 开放 `find`、`count`、`distinct` 和透明 aggregate，透明 aggregate 只允许 `$match`、`$unwind`、`$project`、`$sort`，投影只允许字段保留、排除、直接别名和不改变非空原值的 `$ifNull: ["$field", null]`。其他 MongoDB aggregate、PostgreSQL 派生敏感值、View 底层敏感值，以及 MySQL 非直接投影或无法消歧的多来源输出在命中纳管资源时拒绝，不以不完整遮盖冒充字段保护。Transfer、Develop、Service 等查询 Owner 必须统一消费这一 PreparedQuery 契约，不得按引擎类型建立“某引擎字段级保护、其他引擎资源级拒绝”的分支。其他尚不能证明完整读取闭包或结果血缘的 Provider 返回对应类型化 unresolved，只有该 Tenant 已存在纳管目标且当前出口需要判定时才拒绝，不影响完全未纳管 Tenant 的原路径。SQL PreparedQuery 的 `ReadOnly` 不能只做语句分类，还必须由 Provider 执行路径建立数据库只读事务；门禁通过后执行的仍是同一个 PreparedQuery。
