...
避免跨模块的代码重复。后端共享能力提取到 `common/`，前端共享能力提取到 `common-frontend/`。

同一语义职责、相同输入输出与交互契约的前端能力只能存在一个规范实现。开发新组件前必须检索 `common-frontend/` 和各业务模块；已有实现应直接复用或提炼，不得复制后局部修改。业务模块可以保留协议适配器或领域组合组件，但不得重复实现基础渲染、状态管理、格式化和通用交互。新实现替代旧实现时必须删除旧路径，并通过测试约束唯一所有权。

“唯一实现”按语义职责和契约判断，不按底层 Element Plus 标签判断。不同业务职责的任务列表与查询结果表格可以分别存在；多个模块分别实现相同的查询结果表格则属于重复建设。

- ✅ 单点维护 - 修复一次错误,所有模块受益
- ✅ 一致性 - 所有模块使用相同的实现
- ✅ 减少错误风险 - 无需记住更新多个位置
- ✅ 更容易重构 - 在一处更改逻辑
- ❌ 不得以模块边界、命名差异或少量样式差异为由复制已有前端能力

### 9. 变更必须同步闭环到测试与 CI/CD

代码实现、测试契约和 CI/CD 门禁是同一次变更的不同部分，不得把 CI/CD 适配留到推送失败后再补。

测试分层、标准入口、数据与身份隔离、Online / Release 验收和 CI 编排的详细契约统一遵循 [ADDP 测试与验收规范](addp测试与验收规范.md)。
