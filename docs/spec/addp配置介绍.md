...
Business KingbaseES 不进入通用 `.env` 或 `scripts/start.sh -all`。`business/scripts/kingbase.sh` 只从当前 shell 读取 `KINGBASE_DATABASE`、`KINGBASE_USER`、`KINGBASE_PASSWORD`、`KINGBASE_PORT`、`ADDP_KINGBASE_LICENSE_FILE`和 `ADDP_KINGBASE_LICENSE_SHA256`；License 必须是仓库外绝对路径，密码和 License 不得写入仓库。固定介质只支持 owner-managed Linux x86_64，容器只绑定 `127.0.0.1:${KINGBASE_PORT:-5436}`，不使用数据卷；System 必须以 `engine_type=kingbase` 和 PG 模式连接信息登记。
