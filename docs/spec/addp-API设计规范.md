...
聚合子资源不建立独立版本。子资源的新增、更新、删除以及关联集合变更都使用聚合根的 `version`，并在同一数据库事务中校验和递增聚合根版本。例如 EntityAttribute 使用 Entity 版本，LogicalField、TableRelation 和 DimensionHierarchy 使用 LogicalTable 版本；MetricImplementation 使用自身版本，其修订使用实现的版本。具有独立生命周期、可脱离任一聚合单独编辑的关系事实属于主资源，应维护自己的版本，例如 Catalog StandardMapping 使用自身版本。
