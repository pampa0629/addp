...
SuperMap Workflow 的 `osgb_scene_to_s3m` 同时支持 workflow 与 direct，使用 `addp.workflow.access-plan/v1`。源当前使用 NFS `mounted_path` whole scope；目标支持 NFS `mounted_path` 和 MinIO/S3 `object_store`。运行时必须调用 iObjects Java `OSGBCacheBuilder.generateConfigFile` 与 `CacheBuilderOSGBTool.osgb2s3m`，产物统一为 `config/scene.scp + Data/**/*.s3m`，纹理压缩固定为 DXT。Develop workflow 模式写入用户选择的业务存储，结果经 Meta 扫描形成 `format=s3m + layout=whole` item；Manager direct 模式写入 Manager infra MinIO，登记为 `manager.model3d_tiles` 中 `target_format=s3m` 的受管快显结果，不形成 Meta item。
