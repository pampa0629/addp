...
| **SuperMap Workflow** | 8103 | Java + SuperMap iObjects Java / SPS | 超图数据格式、空间分析与 SPS DAG 内存对象传递 | 24 个真实算子（20 个支持 workflow，5 个支持 direct，其中 `osgb_scene_to_s3m` 同时支持两种模式） |
...
SuperMap Workflow 的 `osgb_scene_to_s3m` 同时支持 workflow 与 direct，使用 `addp.workflow.access-plan/v1`。源当前使用 NFS `mounted_path` whole scope；目标支持 NFS `mounted_path` 和 MinIO/S3 `object_store`。运行时必须先调用 iObjects Java `OSGBCacheBuilder.generateConfigFile` 生成源 SCP，再通过 `ObliquePhotogrammetryBuilder` 构建唯一的 S3M 结果路径；构建参数固定为纹理 `TextureCompressType.TEXTURECOMPRESS_DXT`、几何 `VertexOptimizationType.VO_DRACO`、版本 `S3MVersion.VERSION_301` 和文件类型 `CacheFileType.S3MB`。Builder 必须保留 S3MB 内部和 `rootTiles[].boundingBox` 的场景局部坐标；构建完成后，运行时使用 SuperMap `CoordSysTranslator` 按源 EPSG 将 JSON SCP 的 `position` 与 `geoBounds` 统一规范化为 `EPSG:4326`，不得启用会改变瓦片局部坐标语义的 Builder `MODIFY_CENTER`，也不得让前端猜测投影或为单个 EPSG 硬编码转换参数。当前多根场景产物为 JSON `config/scene.scp + config/scene/Data/**/*.s3mb`，manifest 中的 `./scene/Data/...` 始终相对 `config/scene.scp` 解析。ready 前必须验证 manifest 的 `crs=epsg:4326`、`position.unit=Degree`、位置经纬度范围和全部根瓦片引用。`root_tile_count` 以 Builder 输出 manifest 的实际 `rootTiles` 为事实；Builder 可以跳过空或无有效几何的源根占位，因此不要求输出根数与源目录候选数严格相等；但 manifest 必须至少包含一个根瓦片，且每个引用均必须存在。Develop workflow 模式写入用户选择的业务存储，结果经 Meta 扫描形成 `format=s3m + layout=whole` item；Manager direct 模式写入 Manager infra MinIO，登记为 `manager.model3d_tiles` 中 `target_format=s3m` 的受管快显结果，不形成 Meta item。
