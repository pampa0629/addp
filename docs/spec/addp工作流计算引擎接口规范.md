...
Model3D Workflow 是三维模型转换专用运行时，`engine_type=model3d_workflow`，默认端口 `8101`。第一版暴露 direct operators：`osgb_to_glb` 用于单 OSGB 快显 GLB artifact 生成，`gltf_to_glb` 用于 glTF 多资源模型打包为 GLB 快显 artifact，`fbx_to_glb` 用于 FBX 单体网格模型生成 GLB 快显 artifact，`obj_to_glb` 用于 OBJ 单体网格模型生成 GLB 快显 artifact，`osgb_scene_to_3dtiles` 用于 OSGB Scene 到业务存储 3D Tiles 数据集生成，`gaussian_splat_to_ksplat` 用于高斯泼溅 KSplat 受管 artifact 发布。运行时内部可以通过外部专业 CLI 执行转换，其中 `_3dtile` 负责 OSGB 单文件快显和 OSGB Scene 到 3D Tiles 相关路线，`assimp` 负责 glTF / FBX / OBJ 这类 mesh 模型到自包含 GLB 的打包转换；`gaussian_splat_to_ksplat` 第一版只接受源已经是 `.ksplat` 的受管发布，PLY / SPLAT 到 KSplat 的真实转换器后续单独接入。生成的 GLB artifact 不应保留对源目录贴图的外部相对引用；对 ADDP 只暴露 `addp.workflow/v1` 的 operator、参数、进度和结果事实；Manager 不得直接调用转换 CLI。
