...
PointCloud Workflow 是点云处理专用运行时，`engine_type=pointcloud_workflow`，默认端口 `8102`。第一版暴露 direct operators：`las_to_copc`、`laz_to_copc` 和 `e57_to_copc`，用于将 LAS / LAZ / E57 源点云转换为 Manager 受管 COPC 快显 artifact。PDAL 是该 engine runtime 的内部依赖，应随容器镜像或 engine 运行时一同分发；Manager、System 和宿主机全局环境不得成为 PDAL 依赖来源。运行时未绑定 PDAL 时 `/health` 应返回 `degraded`，且不应自注册到 System。生成的 COPC artifact 存放在 Manager infra MinIO 中，不自动升格为业务 data item；源数据本身已经是 `format=copc` 时由 Manager 基础预览直接读取，不进入该转换任务。
