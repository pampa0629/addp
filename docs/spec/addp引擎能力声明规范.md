...
- TiDB Engine 声明 database/table Engine Catalog、表字段与统计 Facts、参数化 SQL 查询、BatchRead、非空间普通表的 BoundedWatermarkRead，以及 TableWritePrepare / TableWriteSession / Delete / TableUpsert。首版仅复用经 TiDB 8.5.8 真实 T2 认证的 MySQL-compatible 主路径：目录事实来自 `information_schema`；普通只读 SQL 采用 `validated_statement` 边界，先经共享 SQL 副作用分析拒绝非只读语句；一致性读取在 TiDB 对外声明为 repeatable-read 的 Snapshot Isolation 事务中冻结单字段或复合上界，且只执行 Provider 内部生成的 `SELECT`。TiDB 的 `READ ONLY` 事务语法只有 noop 实现，不得通过 `tidb_enable_noop_functions` 把它伪装为数据库强制只读。写入复用显式建表、可空列演进、事务性 insert 和严格唯一键 upsert；upsert 冲突值固定用 TiDB 官方支持的 `VALUES(column)` 引用，不使用 TiDB 8.5.8 无法解析的 MySQL 8 insert row alias。TiDB 为兼容而将表引擎元数据报告为 InnoDB，但该事实不表示采用 MySQL 存储引擎。不声明 BatchWrite、MySQL replication、CDC、分区变化应用、空间类型或空间索引；TiCDC 等独立产品能力不得由普通 TiDB Engine 自动推导。
- KingbaseES Engine 首版只声明 `DB_MODE=pg` 下经 V9R1C10 `V009R001C010B0004` 真实介质认证的 schema/table Engine Catalog、普通表 Facts、参数化 SQL 查询、BatchRead、TableReadSession、QueryReadSession、非空间 BoundedWatermarkRead、TableWritePrepare / TableWriteSession / Delete / TableUpsert。插件只显式转发已实测通过的 PostgreSQL wire/catalog、`lib/pq`、COPY、repeatable-read 和 `ON CONFLICT(keys)` 路径，并独立过滤 KingbaseES 系统 schema 及 `public` 下的内置统计 leaf `sys_stat_statements`、`sys_stat_statements_all`；这些对象不得通过枚举、leaf count 或直接目录路径进入业务目录。不声明 PostGIS、GIS、CDC、PostgreSQL 扩展、分区变化应用、Oracle/MySQL/SQL Server 模式或 openGauss `MERGE`。任何新能力必须先在同一固定介质和合法 License 下扩展 T5/T2，再单路开放。
- DM8 Engine 首版只声明固定 ARM64 官方介质与同版本官方 Go 驱动在 Linux ARM64 Docker 内真实 T2 验证的 schema/table Engine Catalog、普通表字段/统计/主键与唯一约束 Facts、参数化 SQL 查询、BatchRead、TableReadSession、QueryReadSession、非空间 BoundedWatermarkRead、TableWritePrepare / TableWriteSession / Delete / TableUpsert。参数占位符固定使用官方驱动支持的 `?`，分页和标识符规则由独立 `dameng` SQLDialect 表达，upsert 固定使用 `MERGE INTO ... USING (SELECT ... FROM DUAL)`。不声明空间、CDC、分区变化应用、Oracle Provider、宿主 macOS 数据面、通用 ARM64 或生产认证能力；无官方驱动的进程即使能加载能力模板，也不得执行或报告在线。
...
`bounded_watermark_read` 与普通 `batch_read` / `table_read_session` 不等价。前者必须冻结 execution 上界、使用稳定单字段或复合游标并能从读取行生成 committed position。空 tie breaker 的单字段游标只识别新增，watermark field 必须自身精确匹配非空主键或唯一约束；非空 tie breaker 的复合游标识别新增和更新，tie breaker 必须精确匹配非空主键或唯一约束。PostgreSQL、MySQL、MySQL 模式 OceanBase、TiDB、openGauss 与 KingbaseES 当前声明该能力：PostgreSQL/openGauss/KingbaseES 在一致性只读事务中冻结上界，MySQL-compatible Provider 在 InnoDB repeatable-read 事务中冻结上界；PostgreSQL 与 MySQL 读取空间字段时都必须按协商后的标准 geometry row encoding 返回，不得泄漏数据库内部二进制，OceanBase、TiDB、openGauss 与 KingbaseES 当前只开放非空间表。`table_upsert` 也不能从 `batch_write` 推导；只有目标 Provider 能校验唯一键并以幂等冲突处理提交批次时才能声明。PostgreSQL、MySQL、MySQL 模式 OceanBase、TiDB、openGauss 与 KingbaseES 声明幂等 `table_upsert`，其中 PostgreSQL 与 KingbaseES 使用 `ON CONFLICT(keys)`，openGauss 使用不更新匹配键的原生 `MERGE INTO`，OceanBase、TiDB、openGauss 与 KingbaseES 当前只支持非空间表。MySQL-compatible 目标必须使用 InnoDB，并要求配置键精确匹配非空主键或唯一约束；为避免 `ON DUPLICATE KEY UPDATE` 被其他唯一约束触发，目标表不得存在与配置键不同的唯一约束。
...
原生认证在调用方提供的同一连接／事务中只读获取事实，再通过共享编译器生成固定、无业务来源的语义探测。PG 要求服务器与客户端 UTF8；MySQL 要求连接、客户端及结果字符集均为 utf8mb4，并拒绝 READ-UNCOMMITTED。探测核验精确整数／decimal、日期范围与月份偏移、NULL、文本大小写、尾空格和字面子串包含。已知条件不满足返回 supported=false 与稳定诊断码；连接、权限、取消、超时等探测错误返回 error，不能降格为成功或缓存旧结论。探测默认最长 5 秒并尊重更短的调用方 deadline。
