...
- TiDB Engine 声明 database/table Engine Catalog、表字段与统计 Facts、参数化 SQL 查询、BatchRead、非空间普通表的 BoundedWatermarkRead，以及 TableWritePrepare / TableWriteSession / Delete / TableUpsert。首版仅复用经 TiDB 8.5.8 真实 T2 认证的 MySQL-compatible 主路径：目录事实来自 `information_schema`，一致性读取在 TiDB 对外声明为 repeatable-read 的 Snapshot Isolation 事务中冻结单字段或复合上界，写入复用显式建表、可空列演进、事务性 insert 和严格唯一键 upsert。TiDB 为兼容而将表引擎元数据报告为 InnoDB，但该事实不表示采用 MySQL 存储引擎。不声明 BatchWrite、MySQL replication、CDC、分区变化应用、空间类型或空间索引；TiCDC 等独立产品能力不得由普通 TiDB Engine 自动推导。
