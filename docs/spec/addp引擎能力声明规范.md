...
`partitioned_table_change_apply` 与普通 `table_upsert` 不等价。PostgreSQL 当前声明 `atomic_position_commit=true`、`monotonic=true`、`position_types=["kafka_offset/v1"]`、`operations=["upsert","delete"]`；其位置账本位于业务目标 PostgreSQL，并与目标 upsert/delete 在同一事务提交。该能力不表示 Kafka 与 Infra PostgreSQL 之间存在分布式 exactly-once。`operations` 只允许规范定义的 `upsert|delete`，不得重复或声明未实现操作。
