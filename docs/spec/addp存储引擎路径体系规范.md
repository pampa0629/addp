...
| MySQL/OceanBase/TiDB/Doris/ClickHouse | host、port、user | `server`，标题使用引擎实例名称，`full_name=""` | database |
...
| PostgreSQL / openGauss / MySQL / OceanBase / TiDB / Doris / ClickHouse | table / view | `table` / `view` | 通常 `data_type=table` |
...
### 关系型数据库（PostgreSQL / Oracle / MySQL / OceanBase / openGauss / TiDB / Doris / ClickHouse）

full_name 使用引擎原生术语：

- PostgreSQL/openGauss：`<schema>.<table>`
- MySQL / OceanBase / TiDB / Doris / ClickHouse：`<database>.<table>`
...
| MySQL/OceanBase/TiDB/Doris/ClickHouse database 节点 | `analytics` | database 名 |
...
### 关系型数据库（PostgreSQL / MySQL / OceanBase / TiDB / Doris / ClickHouse）
...
TiDB 使用与 MySQL 相同的 `server(root) -> database -> table/view` 路径模型，但 Engine Instance 必须保持独立 `engine_type=tidb`；MySQL 协议、SQL 方言和路径同构只属于插件内部复用事实，不得把 TiDB 登记为 MySQL，也不得据此声明 MySQL replication、CDC、分区变化应用或空间能力。
...
| 关系型数据库（PostgreSQL/openGauss/MySQL/OceanBase/TiDB/Doris/ClickHouse） | schema 或 database | 用户按引擎术语选择（PostgreSQL/openGauss 选 schema；MySQL/OceanBase/TiDB/Doris/ClickHouse 选 database） |
...
1. 通过 `EngineCatalogProvider.ListChildren(root)` 获取 namespace 列表（PostgreSQL/openGauss 为 schema；MySQL/OceanBase/TiDB/Doris/ClickHouse 为 database）
...
- 关系型数据库：`schema + "." + table`（PostgreSQL/openGauss）或 `database + "." + table`（MySQL/OceanBase/TiDB/Doris/ClickHouse）
