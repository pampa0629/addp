...
`pptx_pdf_generation` 声明 `supports_schedule=false`。普通预览按需触发：`GET /manager/preview` 只返回 artifact 状态，不得隐式创建任务或 execution；前端收到缺失或失败状态后，必须先读取 `GET /manager/quick-view/capability?locator=...`，再显式提交其声明的 `generate_pptx_pdf` 动作到 `POST /manager/quick-view/actions`。已有 `pending` / `running` execution 时复用其状态，失败后只允许用户显式重试。不得为 PPTX 保留独立的任务创建或执行 API。Orchestrator 只在用户显式配置预热或批量编排时调用同一 Manager TaskProvider 任务，不直接调用 Runtime，也不建立第二条执行路线。
...
14. 子 execution 的授权事实只允许三个完整状态：尚无身份血缘和执行授权、仅有完整 `actor_principal_id + actor_tenant_membership_id + issued_authorization_version` 身份血缘、已附加 `execution_authorization_id + authorization_expires_at` 的完整执行授权引用。`common.task_executions` 不复制 Engine Access Scope；精确范围只由 System 的不可变 Execution Authorization 保存和校验，禁止保留会丢失引擎与效果对应关系的 `authorization_effects` 摘要。任意残缺组合必须由数据库约束拒绝。owner 可在创建子 execution 的事务中先固化父执行身份血缘，提交后再由 System 沿 `parent_execution_id` 校验并签发 Execution Authorization。静态资源绑定应在 claim 前完成签发，失败的 pending execution 不得进入 worker；只有资源身份必须由当前 lease attempt 的受控 owner API 动态解析时，worker 才可在 claim 后、任何数据访问前签发，并提交当前 `attempt + lease_token`。System 必须直接校验子 execution 仍为 `running`、attempt/token 精确匹配且租约未过期，将二者固化为不可变授权边界，并在每次授权消费时复核仍为当前租约。owner 只能以同一 lease 条件附加授权；安全恢复为新 attempt 时必须清除子 execution 上一 attempt 的两个授权字段（保留身份血缘）并签发新授权，旧授权保留审计但不再可消费。失败时由当前 lease 收敛为终态。
...
Orchestrator 来源的 Develop 查询必须把参数解析后的 `content`、`engine_id`、timeout 和有效执行输入冻结在 `execution_config`，Worker 不得在 claim 后重新读取可变任务定义。关系参数先应用任务默认绑定，再由 Orchestrator 提交值覆盖；关系结果写入只消费该有效绑定及独立的 `target_locator`，并按当前 `attempt + lease_token` 签发精确的同引擎 `read + write` 授权。Develop 不得取得 Model attempt、调用 Model 或接管 Model staging；租约失效或写入失败时当前 execution 和父编排直接失败，重算必须从新的 Model prepare 批次开始。
...
Transfer 的内部任务语义统一收敛为 `sync`。可复用同步配置保存为 `transfer.transfer_tasks` 并通过 TaskProvider 关联任务定义；一次性导出等用户动作直接创建 `module=transfer + task_type=sync + source_task_id=null` 的 bounded ad-hoc execution，不得先创建临时任务定义。Manager 导出和 Develop 查询完整结果导出统一调用 Transfer 的一次性执行入口，并由 Common 导出会话能力保存统一 `execution_id`、infra 临时产物 manifest 与下载状态；不得保存 Transfer 私有 task ID，也不得在 Transfer 侧并行保留 `import`、`export`、`transfer` 等旧任务类型。模块 Service Client 创建一次性 execution 使用不可委派的 `transfer.execution.create` Permission，回查结果使用不可委派的 `transfer.execution.read` Permission；`tenant.manager_runtime` 与 `tenant.develop_runtime` 仅因导出会话获得这两个窄权限，不因此获得 Transfer 任务定义的创建、读取或启动权限。Transfer 从已认证的 `addp-<module>` Client ID 推导 execution `source`，请求不得提交可伪造的 `source_module`；结果回查也必须校验 execution `source` 与当前 Client ID 对应模块完全一致，不得允许模块读取其他来源的 execution。调用方通过 Common 强类型 Client 的窄接口提交标准 Locator、查询快照、infra 目标父 Locator、文件名、格式和字段结构；Transfer 独占 planner、流式读取、编码、目标写入、保护和执行状态。导出产物不进入业务资源树、不触发 Meta scan、不形成正式业务 DataItem；需要写入业务引擎时必须使用独立的“保存结果”能力。Manager 导入仍属于可复用同步任务创建语义时继续使用持久 `sync` 任务，不与一次性导出混用。

Model 的 `materialization_prepare|materialization_seal|materialization_publish` 以已审批 LogicalTable 作为来源驱动任务定义，task ID 均为 LogicalTable ID。`prepare` 冻结逻辑表版本和结构指纹，由 Model 创建本批次唯一 staging，稳定输出 `batch_id + staging_locator`；DDL、Schema 和表名仍不进入任务定义或用户输入。`seal` 必须通过运行时输入接受 prepare 的 `batch_id`、通用 writer 的 `execution_id` 和 writer 回显的 `target_locator`；Model 校验 writer 已成功、同 Tenant、同 Actor 血缘、同一父编排，target 精确等于本批 staging，并完成物理管理标记和结构校验后才把批次置为 `sealed`。`seal` 不校验 writer 所属业务模块，不引入 Transfer/Develop 硬编码。`publish` 只消费同一父编排下的 sealed 批次。
...
Transfer 的 bounded `sync` 无论目标由任务定义固定保存，还是由 Orchestrator 在执行期绑定，都必须声明同一组稳定输出 `execution_id + target_locator + row_count`。固定目标任务返回闭合空输入契约，`target_locator` 由 execution 冻结的 Transfer 目标配置解析并回显；执行期目标任务才把 `target_locator` 声明为必填输入。固定创建目标必须保留其 locator 体系：业务引擎父资源产生 `addp://engine/...` 子资源，Manager 等模块传入的 Infra 父资源产生 `addp-infra://...` 子资源，不得用业务 ResourceLocator 解析器处理 Infra locator。不得因为目标已固定而返回空输出契约，也不得让下游从 Transfer 私有配置、任务 ID 或物理字段拼接目标位置。

Develop 保存的 `query` 可以声明通用“关系参数 -> 已存在表结果”模式。`content.query_parameters[]` 是唯一参数定义事实源；`type=relation` 的参数可以在 `default.locator` 保存同一查询 Engine 中已有表的标准 ResourceLocator，也可以不保存默认绑定并由每次执行提供。执行契约把每个关系参数声明为 ResourceLocator 输入；只有缺少默认绑定的关系参数才是必填覆盖，同时把编排写入使用的 `target_locator` 声明为必填运行时输入。SQL 以未加引号、未限定 schema 的裸参数名引用已声明关系，例如参数 `activities` 写作 `FROM activities`；`input_ui_schema.<参数名>` 必须声明 `control=resource_tree_picker` 和 source 资源绑定语义，使 Orchestrator 可以直接使用默认表，也可以把不同上游的稳定 ResourceLocator 输出分别绑定到同名参数，不得要求用户手写绑定模板。Worker 校验全部有效关系参数 locator 与查询 Runtime、目标位于同一 PostgreSQL Engine，通过 AST 只改写已声明的裸关系节点，并在事务中执行 `INSERT INTO <target> SELECT ...`。CTE 与参数重名、未声明参数、未使用声明、真实物理关系、schema 限定关系、表函数数据源、非 PostgreSQL 查询或跨 Engine 输入必须拒绝。稳定输出至少包含 `execution_id + target_locator + row_count`。Develop 不得保存 LogicalTable ID、调用 Model API 或持有 Model Permission。
...
Manager 的快显和业务派生任务细节由 Manager 专题确认。本文只要求 Manager 用同一个 provider 声明多个任务类型，并按 `module + task_type + source_task_id` 关联执行记录。快显与空间派生任务定义统一存入 `manager.task_definitions`，资源归属投影到 `manager.task_resource_bindings`；其中 `vector_tile_cache_generation` 的结果表为 `manager.vector_tile_cache`，结果是 Manager infra PMTiles artifact，`vector_tile_set_generation` 的结果是 Business 存储中 `data_type=media + format=pmtiles + layout=single` 的 Meta item，不设 Manager 结果表。两者按源能力选择唯一生成路径：PostgreSQL/PostGIS 表由 Manager 原生 `ST_AsMVT` 生成，MySQL、Oracle 等标准 EWKB 可读数据库表由 Manager 物化临时 FlatGeobuf 后调用 GeoPython Workflow `vector_to_pmtiles`，NFS、MinIO/S3 文件或对象由受控访问计划调用同一 operator；MVT 是 PMTiles 内部 tile encoding，不是任务类型。统一定义存储不改变矢量物化视图、栅格、三维、点云与 embedding 各自的结果边界。
...
查询结果预览只用于有界展示，不提供“下载预览”旁路。“导出全部结果”在单 Engine 表格查询成功后提供，不以预览是否截断或是否已有预览行为前提；DuckDB 联邦查询在具备统一流式查询 Provider 前不开放。该动作固定使用 `POST /api/v1/develop/executions/{execution_id}/exports`，并同时要求查询执行权限与数据读取权限；由 Develop 从已成功查询 execution 的冻结内容、有效参数、有效关系输入、Engine 和结果字段生成只读查询快照，再通过 Common 强类型 Client 创建 Transfer ad-hoc `sync` execution。查询 source 使用一个 `locator` 作为同 Engine 连接锚点，并在 `source.query.inputs[]` 按参数名稳定排序保存本次执行实际绑定的全部 `{name, locator}`；Transfer 必须校验每个输入都是与 source 同 Engine 的标准 ResourceLocator，使用将要执行的同一 `PreparedQuery` 的 Provider `ReadSet` 精确验证全部输入，并据此写出完整 `lineage_facts.inputs`。Transfer 不得解析 SQL 猜测资源，不得用未验证的调用方声明写血缘，也不得只记录连接锚点。前端只提交格式和文件名，不选择业务存储引擎，也不得拼装查询文本、Transfer 配置、infra 路径或临时 task ID。Develop 创建按 Tenant 与发起用户隔离的短生命周期导出会话，在 Workbench 内轮询会话状态，并在成功后通过会话下载入口把 infra 暂存产物交付到本地；用户可以另外查看对应的统一 execution。Transfer 必须通过 `QueryReadSessionProvider` 流式读取全部查询结果并写出目标格式，不得复用 Develop 的有限预览行，也不得把完整结果写回 Develop execution metadata。导出产物不触发 Meta scan；写入业务存储并登记为 DataItem 属于独立“保存结果”能力。
...
所有 TaskProvider 声明必须使用独立 `/task-provider/` 路由空间和以上 endpoint 后缀语义；公共声明校验器必须拒绝落在人用路由空间的声明。取消接口不是必选能力；未声明支持取消的任务类型不得注册或展示取消入口。确需处理现有路径时，只能作为入口层迁移工作，不得形成长期双轨命名。
...
Quality `check|materialization_gate` 是纯手动/Orchestrator 显式执行类型，当前不保存或返回 `enabled`、`schedule`、`next_run_at` 等调度活状态字段。其中 `materialization_gate` 仅允许 Orchestrator 触发。`execution_contract` 是具体任务可执行输入和稳定输出的唯一事实源：
...
不支持执行输入覆盖或没有稳定输出的任务仍必须返回对应的闭合空对象。`input_defaults` 只提供任务定义中已保存的可覆盖默认值；它不能隐式填充任务定义本身必须持久的业务字段，也不能绕过执行时 required 校验。`input_ui_schema` 只描述 `input_schema.properties` 中已声明字段的控件语义，不得增加输入字段或覆盖服务端 Schema 约束。需要稳定展示顺序时，字段和分组必须在对应 UI Schema 节点声明从 `0` 开始的 `order`；消费者必须按 `order` 排序，不能依赖 JSON 对象属性顺序。算子工作流的分组顺序使用稳定 DAG 拓扑顺序：上游算子在前，同层并行算子按任务定义 `tasks[]` 的保存顺序排列；分组内字段按公开参数声明顺序排列，不依赖画布坐标。`format=resource-locator` 的值在契约和请求中仍使用标准 ResourceLocator，但用户界面只能展示解析后的资源路径、名称和本地化类型，不得直接显示 `addp://` URI、Engine ID、`node_id` 或 `item_id`；无法解析时统一显示“已配置资源”，不得回退为原始内部值。
...
8. 普通查询参数只允许绑定值，不得动态替换集合名、字段名、排序方向、关键字或任意查询片段。唯一例外是 Develop 明确声明的 PostgreSQL `relation` 参数：它以裸关系节点出现，并由 AST 按 ResourceLocator 安全编译；不得通过字符串值或模板插值模拟关系参数。
...
| --- | --- | --- | --- |
| `manager` | `vector_tile_cache_generation` | `manager.task_definitions` | Manager |
| `manager` | `vector_tile_set_generation` | `manager.task_definitions` | Manager |
| `manager` | `vector_materialized_view_generation` | `manager.task_definitions` | Manager |
