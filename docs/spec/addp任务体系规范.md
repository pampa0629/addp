...
Transfer 的 bounded `sync` 无论目标由任务定义固定保存，还是由 Orchestrator 在执行期绑定，都必须声明同一组稳定输出 `execution_id + target_locator + row_count`。固定目标任务返回闭合空输入契约，`target_locator` 由 execution 冻结的 Transfer 目标配置解析并回显；执行期目标任务才把 `target_locator` 声明为必填输入。不得因为目标已固定而返回空输出契约，也不得让下游从 Transfer 私有配置、任务 ID 或物理字段拼接目标位置。
