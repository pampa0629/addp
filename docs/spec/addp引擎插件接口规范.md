...
`ListChildren` 只能返回当前连接身份按引擎原生权限可访问的 catalog entry，并必须在计算 branch 的 leaf 摘要前过滤系统命名空间和无权访问的命名空间。关系型引擎至少要求当前身份具有 schema `USAGE` 或等价的原生访问权限。协议兼容引擎的厂商系统命名空间由具体插件通过协议实现的参数化身份声明，上层模块不得按 `engine_type` 维护过滤分支或名单。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| PostgreSQL | schema | `information_schema.schemata/tables/columns` + `pg_class` + `pg_stat_user_tables` | `BASE TABLE` -> `table`，`VIEW` -> `view`，其他 `table_type` 转小写下划线 | `information_schema.columns`，主键来自约束表，注释来自 `col_description` | 列表使用 `pg_class.reltuples` 写入 `estimated_row_count`；显式统计执行 `COUNT(*)` 写入 `row_count`，不主动 `ANALYZE` | 只保留当前身份具有 `USAGE` 的 schema；过滤 `pg_catalog`、`information_schema`、`pg_toast`、`pg_temp_*`、`pg_toast_*`；当实例检测到 SuperMap `sdx_postgis` 或 `sdx_postgresql` 时过滤 `sm*` 系统 leaf | 暂留插件内；PostgreSQL 原生 catalog 语义较强，不与 MySQL/Doris 合并 |
| openGauss | schema | 复用已认证的 PostgreSQL wire protocol catalog 查询 | 与 PostgreSQL 兼容路径一致 | 普通表字段、主键、约束与索引 | 列表使用估算行数；显式统计执行 `COUNT(*)` | 只保留当前身份具有 `USAGE` 的 schema；在 PostgreSQL 系统 schema 之外，由 openGauss 插件向协议实现声明原生系统 schema：`blockchain`、`cstore`、`db4ai`、`dbe_perf`、`dbe_pldeveloper`、`dbe_sql_util`、`dbe_pldebugger`、`snapshot`、`sqladvisor`、`pkg_service`、`coverage` | 以独立 `engine_type=opengauss` 声明已认证的 catalog、facts、查询、有界水位读取和非空间普通表写入；只复用显式转发的 PostgreSQL 协议实现，不继承 PostGIS、CDC 或分区 change apply |
| KingbaseES（PG 模式） | schema | 复用已认证的 PostgreSQL wire protocol catalog 查询 | 与 PostgreSQL 兼容路径一致 | 普通表字段、主键、约束与索引 | 列表使用估算行数；显式统计执行 `COUNT(*)` | 只保留当前身份具有 `USAGE` 的 schema；在 PostgreSQL 系统 schema 之外过滤 `anon`、`dbms_job`、`dbms_scheduler`、`kdb_schedule`、`perf`、`pg_bitmapindex`、`src_restrict`、`sys_catalog`、`sys_hm`、`sysaudit`、`sysmac`、`xlog_record_read`，并过滤 `public` 下的 KingbaseES 内置统计 leaf `sys_stat_statements`、`sys_stat_statements_all`；过滤同时作用于枚举、leaf count 与直接路径解析 | 以独立 `engine_type=kingbase` 声明 V9R1C10 PG 模式已认证的 catalog、facts、查询、有界水位读取和非空间普通表写入；upsert 使用实测通过的 `ON CONFLICT(keys)`，不使用 openGauss `MERGE`，不继承 PostGIS、CDC、PostgreSQL 扩展或分区 change apply |
...
| TiDB | database | `information_schema.schemata/tables/columns/statistics` | 同 MySQL 兼容逻辑；表引擎元数据按官方兼容约定报告为 InnoDB | `information_schema.columns`，主键来自 `column_key`，注释来自 `column_comment` | 列表将 `information_schema.tables.table_rows` 写入 `estimated_row_count`；显式统计执行 `COUNT(*)` 写入 `row_count` | `INFORMATION_SCHEMA`、`INSPECTION_SCHEMA`、`METRICS_SCHEMA`、`PERFORMANCE_SCHEMA`、`mysql`、`sys` | 与 MySQL/OceanBase/Doris 共享 `MySQLCompatibleCatalogFactsDialect`，与 MySQL/OceanBase 共享 MySQL 方言的 `QueryReadSet` / `QueryOutputLineage`、非空间 `BoundedWatermarkReadProvider` 和普通表写入核心；共享 upsert 必须由插件显式声明冲突值引用语法，TiDB 8.5 固定使用官方支持的 `VALUES(column)`，不得套用 MySQL 8 的 insert row alias；`engine_type` 固定为 `tidb`，bounded watermark 只接受真实 TiDB T2 认证的 InnoDB 兼容基表，并在 TiDB Snapshot Isolation 事务中冻结上界；不声明 MySQL replication、TiCDC、分区变化应用或空间能力 |
...
- 当前 source Provider 由 PostgreSQL、MySQL、MySQL 模式 OceanBase、TiDB、openGauss 与 KingbaseES 实现：PostgreSQL/openGauss/KingbaseES 在 repeatable-read 只读事务中冻结上界，MySQL-compatible 引擎在 InnoDB repeatable-read 事务中冻结上界；所有游标字段不得为 NULL，OceanBase、TiDB、openGauss 与 KingbaseES 当前只开放非空间表。
...
首个 `QueryOutputLineage` 验证范围包括 PostgreSQL、MongoDB，以及 MySQL / MySQL 模式 OceanBase / TiDB 的直接列投影子集。PostgreSQL 对直接列、显式别名和可证明的单来源 `*` 产生 direct / identity 映射；表达式依赖标记为 derived，View 底层依赖、无法消歧的多来源 wildcard 等不能证明字段映射的 source 标记 opaque。MongoDB `find` 的原结构输出声明 identity，`distinct` 把指定源字段 direct 映射到 `value`，`count` 声明无字段值输出。MongoDB aggregate 只对按顺序完全由 `$match`、`$unwind`、`$project`、`$sort` 组成的透明管道产生输出血缘：`$project` 只接受字段保留、排除、直接别名和 `$ifNull: ["$field", null]`，`$unwind.includeArrayIndex` 作为数组结构派生值记录；任一其他阶段、表达式、复合对象、动态字段路径或无法从前序输出唯一回溯到源字段的引用必须返回 output-lineage unresolved。MySQL、MySQL 模式 OceanBase 及 TiDB 只对前述基础表可信 ReadSet 内的直接列、显式别名、JOIN 与派生子查询逐层产生 direct bindings，不支持 wildcard 或 derived binding。未纳管 Tenant 不调用 `OutputLineage()`，不会承担结构读取或结果来源分析成本。

openGauss 与 KingbaseES 分别通过显式转发已认证的 PostgreSQL `PreparedQuery` 实现获得相同的 `QueryReadSet` 与 `QueryOutputLineage` 证明范围，但引擎身份与 catalog path 仍分别保持 `opengauss` 与 `kingbase`；这不得扩张为 PostGIS、CDC 或其他未声明能力。
...
当 `QueryRequest.Options.ReadOnly=true` 时，Provider 必须使用数据库只读事务、只读路由、只读命令白名单或其他等价的运行时约束执行查询。SQL Provider 必须通过 `ControlledReadOnlySQLProvider.ControlledReadOnlySQLBoundary()` 显式声明唯一边界：`database_transaction` 表示驱动或数据库能够建立真实只读事务，`validated_statement` 表示 Provider 先用共享 SQL 副作用分析拒绝非只读语句，再由自身普通执行路径运行已验证语句；需要冻结一致性快照的读取仍须建立普通一致性事务。调用方只消费该声明，不维护 `engine_type` 白名单；未知值必须 fail-closed。不得忽略 `ReadOnly` 后使用未经验证的普通高权限连接执行，无法建立可靠只读边界的 Provider 必须显式拒绝。

TiDB 8.5.8 的 MySQL 协议层会拒绝驱动为 `ReadOnly=true` 生成的事务语句，并提示 `READ ONLY` 只有 noop 实现；因此 TiDB 固定声明 `validated_statement`，普通查询必须先通过共享只读 SQL 校验，bounded watermark 只能在 Snapshot Isolation 事务中执行 Provider 内部生成的 `SELECT`。不得启用 `tidb_enable_noop_functions` 伪装数据库事务只读，也不得把 TiDB 回退为 MySQL 的 `database_transaction` 声明。
...
节点集合为 `scan`、`filter`、`project`、`join`（inner/left/cross）、`distinct`、`aggregate`、`union_all`、`constant_rows`、`date_buckets`、`sort`、`limit`。`aggregate` 提供计数与通用 `sum_decimal`；求交、分母和补零由这些操作组合，不能设计指标专用节点。条件表达式包含明确的日期转换、月桶、整数／decimal 转换和精确文本比较语义。未知操作、重复节点、悬空引用、环、超预算计划和不成立的类型推导均拒绝。
...
- 普通比较遇 NULL 得到 unknown，filter 只保留 true；`is_null` 显式判断 NULL，普通算术传播 NULL。count_rows 统计所有输入行，count_value 只计非 NULL 值；`sum_decimal` 只接受 DECIMAL(38,18) 表达式，忽略 NULL，空输入或全 NULL 结果为 NULL，非空和必须仍落入 DECIMAL(38,18) 范围，越界作为独立求值错误，不能靠原生窄化截断。distinct 将相同键中的 NULL 合并，group 将 NULL 键归为同组；去重后再 count_rows 不能被擅自替换为忽略 NULL 的 COUNT(DISTINCT)。当前指标若要求排除 NULL，应由 Model 显式构建过滤／断言。补零必须显式由连接与条件表达式表达。
- `filter` 的输出类型可收窄显式非空检查：谓词为 `not(is_null(column))`，或其位于 `and` 的合取项时，该输入列在输出中为非空。推导只作用于此过滤节点，不改写来源字段，不穿透 `or` 或其他表达式；独立质量断言仍须完整执行。
...
- 四则运算在每个逻辑表达式节点规范化结果：两个整数的加减乘返回有符号 bigint；含 decimal 的加减乘及所有除法返回 DECIMAL(38,18)。小数乘除按最近值、中点远离零舍入到 18 位后检查目标范围。整数加减乘不得先发生原生 bigint 溢出；有效非 NULL 操作数除以零必须报错，普通 NULL 传播仍适用（包括 NULL / 0），已有子表达式错误不能被 NULL 传播隐藏。
- 原生算术组件必须使用精确十进制中间值，不能依赖会话默认除法精度。有限精度实现可使用放大后的整数、拆分乘积和精确余数校正；余数决定最终舍入，近似商只能作为候选值。工作精度上限属于原生编译策略，不进入 Plan 或指标模块；不满足该策略要求的引擎应使用自己的已认证实现或明确拒绝。
...
- 原生表达式的求值失败通过编译产物内的 `EvaluationCheck{Node, Code}` 表达，绑定已有计划节点；它是编译器从运算语义派生的检查，不写入 owner 的业务断言或冻结 Plan。检查按 node、code 排序，生成独立的 `eval_ok:<index>`／`eval_fail:<index>` 记录，纳入编译产物指纹，必须完整且唯一。执行层先消费全部检查，再规范化业务值；求值失败返回稳定节点和错误码，不将内部 NULL 占位当成合法结果。表达式的错误条件须保留其逻辑求值范围，后续过滤或分页不能掩盖已经求值的错误。
...
排序在过滤与限行后保留；投影仅在完整保留排序列的直接映射时传播排序，若无法保留则明确返回不支持，不能静默丢弃显式排序；连接／聚合／合并／去重不继承输入顺序。SQL 编译器对缺少可继承显式排序的 limit 明确返回不支持，不依赖数据库物理行序选择结果；需要确定分页时，调用方必须提供包含稳定键的完整排序。分组与去重对文本使用精确比较键，输出仍为原逻辑文本类型，不能依赖默认排序规则或将二进制中间键暴露为业务字段。尚未实现的节点明确拒绝，不回退到固定测试计划或拼接原生片段。

通用表达式编译入口 `sqlcompile.CompileExpression` 先调用 `plan.AnalyzeExpression` 校验形状、作用域、参数声明与类型；后者与完整 Plan 校验复用同一语义实现。原生表达式作用域只保存逻辑列到已绑定关系别名／列名的映射，标识符由引擎引用，不接受 SQL 片段。类型化参数保留命名占位符，常量使用引擎的安全编码；文字比较必须明确采用精确语义，不能继承实例默认排序规则。布尔 AND/OR、比较与一元判断保留操作数已求值的错误，条件求值范围只由 CASE/COALESCE 表达。尚未实现的合法运算返回不支持，不能回退或原样拼接操作名。
