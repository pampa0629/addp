...
openGauss 通过显式转发已认证的 PostgreSQL `PreparedQuery` 实现获得相同的 `QueryReadSet` 与 `QueryOutputLineage` 证明范围，但引擎身份与 catalog path 仍保持 `opengauss`；这不得扩张为 PostGIS、CDC 或其他未声明能力。
