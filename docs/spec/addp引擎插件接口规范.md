...
| openGauss | schema | 复用已认证的 PostgreSQL wire protocol catalog 查询 | 与 PostgreSQL 兼容路径一致 | 普通表字段、主键、约束与索引 | 列表使用估算行数；显式统计执行 `COUNT(*)` | `pg_catalog`、`information_schema`、`pg_toast`、`pg_temp_*`、`pg_toast_*` | 以独立 `engine_type=opengauss` 声明已认证的 catalog、facts、查询、有界水位读取和非空间普通表写入；只复用显式转发的 PostgreSQL 协议实现，不继承 PostGIS、CDC 或分区 change apply |
