...
| KingbaseES（PG 模式） | schema | 复用已认证的 PostgreSQL wire protocol catalog 查询 | 与 PostgreSQL 兼容路径一致 | 普通表字段、主键、约束与索引 | 列表使用估算行数；显式统计执行 `COUNT(*)` | 只保留当前身份具有 `USAGE` 的 schema；在 PostgreSQL 系统 schema 之外过滤 `anon`、`dbms_job`、`dbms_scheduler`、`kdb_schedule`、`perf`、`pg_bitmapindex`、`src_restrict`、`sys_catalog`、`sys_hm`、`sysaudit`、`sysmac`、`xlog_record_read` | 以独立 `engine_type=kingbase` 声明 V9R1C10 PG 模式已认证的 catalog、facts、查询、有界水位读取和非空间普通表写入；upsert 使用实测通过的 `ON CONFLICT(keys)`，不使用 openGauss `MERGE`，不继承 PostGIS、CDC、PostgreSQL 扩展或分区 change apply |
