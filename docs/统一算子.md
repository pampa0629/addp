...
**新增函数**:
- `SupportsDevMode(engine, mode)` - 检查引擎是否支持指定开发模式
- `GetSupportedDevModes(engine)` - 获取引擎支持的所有开发模式
- `FilterEnginesByDevMode(engines, mode)` - 过滤引擎列表
- `IsAPIEngine(engine)` - 判断是否为API引擎
- `IsStandardLibraryEngine(engine)` - 判断是否为标准库引擎
...
```go
// SQL编辑器自动过滤
sqlEngines := utils.FilterEnginesByDevMode(allEngines, "sql")
...
curl -X POST -H "Authorization: Bearer $TOKEN" \
  ...
  -d '{"params": {"engine_id": 1}, "execute_now": true}' \
