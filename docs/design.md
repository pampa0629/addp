...
后续提供基于角色的权限管控
后续提供外部账号的对接（思路待研究，关键是搞清楚应用场景和使用模式）
...
所有模块，均采用一套内置的系统存储。
当前已使用：
Postgresql：管理结构化数据（开启spatial空间扩展和vector向量扩展）
redis：任务队列、消息中间件、内存缓存
minio：瓦片缓存
meillisearch：全文检索
...
其他内容，则各模块均可以根据需要予以直接读取，避免通过api访问带来的性能损耗。（这一条可以商议，容易造成模块间强依赖）
