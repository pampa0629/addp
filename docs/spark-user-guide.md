...
本文档介绍如何在 ADDP 平台中使用 Spark SQL 和 Sedona 进行大规模数据分析和空间计算。

## 目录

- [1. 概述](#1-概述)
- [2. 部署和启动](#2-部署和启动)
- [3. 注册 Spark SQL 资源](#3-注册-spark-sql-资源)
...
Apache Spark SQL 是 Spark 的一个模块，用于处理结构化数据。它提供了：
...
在 ADDP System 模块中注册 Spark SQL 作为数据源。
...
2. 点击"新建资源"
3. 填写信息：
   - **资源名称**（英文标识）: `spark_sql_default`
   - **显示名称**（中文）: `Spark SQL 分析引擎`
   - **资源类型**: `spark_sql`
   - **连接配置**:
     ```json
     {
     ...
     ```
   - **描述**: Spark SQL 分布式查询引擎，支持 Sedona 空间函数
4. 点击"测试连接"验证
5. 保存
...
curl -X POST http://localhost:8080/api/engines \
  ...
  -d '{
    ...
    "description": "Spark SQL 分布式查询引擎，支持 Sedona 空间函数",
    "is_active": true
  }'
```
