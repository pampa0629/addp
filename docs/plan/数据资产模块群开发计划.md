...
|------|------|---------|----------------------|
| Meta | `GET /api/v1/meta/assets/discoverable` | MetaItem.scanned_at IS NOT NULL | `{engine_id}:{full_name}` |
| Service | `GET /api/service/internal/assets/discoverable` | RegisteredService.status = "active" | `{service_id}` |
| Standard | `GET /api/standard/internal/assets/discoverable` | Metric.status = "approved" | `{metric_id}` |
| Develop | `GET /api/develop/internal/assets/discoverable` | DevItem.status = "active" | `{devItem_id}` |
