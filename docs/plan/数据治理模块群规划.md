...
| **Meta（扩展）** | 元数据扫描 + 血缘关系证据/投影 + 图查询 | 数据工程师、分析师 | 现有能力扩展 |
...
- **并发控制**: 独立 Quality Worker 从 `common.task_executions` 使用 PostgreSQL claim、execution lease 和有界槽位并发执行
...
血缘阶段 1 已完成，现行设计和边界以 [ADDP 数据血缘能力规范](../spec/addp数据血缘能力规范.md) 为准。本规划只保留模块群层面的定位：血缘是 Meta 管理的关系视图，不新增 Govern 模块，不新增独立图数据库。

当前闭环为：真实读写 owner 将版本化 `lineage_facts` 写入统一 execution，Meta 解析 locator / fingerprint 并维护关系证据、当前投影和服务依赖；Develop 在事实持久化后立即通知 Meta，周期 collector 负责漏采和重试。查询统一使用 Meta lineage graph API，查看器实现位于 `common-frontend/graph`，由 Manager、Service、Asset 等宿主嵌入。

阶段 1 只保证 data item 级关系和 published service `serve` 关系。字段级血缘、SQL 方言解析、Model / Quality 独立关系图层和 UDBX Dataset 独立身份均属于后续专题，不能按本规划初稿中的推测实现。
...
graph TB
    ...
    Transfer -->|"提交 lineage_facts"| Meta
    Develop -->|"提交 lineage_facts"| Meta
    Orchestrator -->|"提供 parent_execution_id"| Meta
...
### 阶段三：Meta 血缘扩展（阶段 1 已收口）
**现状**: 已按正式血缘规范完成 Meta 关系投影、统一 execution facts、Develop 立即通知、周期漏采/重试、统一图查询 API 和共享查看器。

**未纳入阶段 1**: 任意 SQL 自动解析、字段级关系、独立影响分析 API、Model / Quality 关系图层和未被 ADDP 观察的外部流转推断。

**现行依据**: [ADDP 数据血缘能力规范](../spec/addp数据血缘能力规范.md)。后续扩展必须先更新正式规范，不在本规划恢复早期 `lineage_nodes` / `lineage_edges` 方案。
...
| **Meta（扩展）** | 8082（不变） | 5175（不变） | `meta`（血缘关系表） | 无 |
...
| **血缘查询** | PostgreSQL 关系表 + 递归 CTE | Meta 统一图查询；关系表是唯一事实源 |
| **有界执行领取** | PostgreSQL claim + execution lease | 各 owner 模块附属 Worker 领取持久 execution；Redis 不承载 bounded execution 队列 |
...
| 风险 | 应对措施 |
|-----|---------|
| **后续字段/SQL 血缘复杂度高** | 阶段 1 不自动推断；先扩展正式规范，再按引擎能力逐步实现 |
| **血缘采集性能影响** | 异步采集，不阻塞业务流程；支持开关控制 |
| **脱敏规则配置复杂** | 提供常用模板，支持自定义扩展 |
| **数据级权限性能** | RLS/CLS 通过 SQL 改写实现，需优化查询性能 |
| **模块间依赖过多** | 通过统一 execution facts、明确 Meta API 和共享前端组件集成，避免复制事实和查询逻辑 |
...
**阶段三（阶段 1）结束**:
- [x] data item 级血缘图查询和共享查看器
- [x] execution facts、关系证据和当前投影闭环
- [ ] 字段级血缘、SQL 自动解析和独立影响分析（后续）
...
- [数据血缘能力规范](../spec/addp数据血缘能力规范.md)（现行规范）
