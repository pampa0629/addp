...
**目标**: 在 ADDP 平台中引入通用的**空间计算引擎**架构，首个实现为 **GeoPandas Engine**
...
6. **仅注册引擎本身**: System 中只注册 `geopandas.engine.default`，不注册 21 个具体算子
7. **动态任务发现**: Orchestrator 通过 `task_api_config.endpoints["list"]` 动态查询 GeoPandas Engine 的任务列表（类似 Transfer 模式）
...
│  │ ✅ GeoPandas: "geopandas.engine.default" (新增)           │ │
│  └───────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
                           ▲
                           │ (启动时注册)
                           │
┌──────────────────────────┴──────────────────────────────────────┐
│  GeoPandas Engine (独立微服务，HTTP Sidecar)                    │
│  容器名: geopandas-engine, 端口: 8090                            │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ Flask API Server                                           │ │
...
│  • 返回引擎列表: [transfer, meta, geopandas]                     │
│  • 查询任务列表: GET /api/tasks/list?unique_identifier=...       │
│  • 解析 task_api_config.endpoints["list"]                       │
│  • 调用引擎 API: GET http://geopandas-engine:8090/api/spatial/tasks │
...
│  • 左侧任务库：点击 "GeoPandas" 展开                             │
│  • 动态加载任务列表（从 GeoPandas Engine 获取）                  │
...
| **引擎部署形态** | HTTP Sidecar（独立容器） | 语言隔离，可并存多引擎（geopandas-engine, postgis-engine） |
| **引擎命名** | GeoPandas Engine | 明确技术栈，未来可扩展 PostGIS Engine、GDAL Engine |
...
POST /api/develop/spatial/execute
{
  "workflow": {
    ...
    ↓
Develop Backend → GeoPandas Engine: POST /api/spatial/workflow
    ↓
GeoPandas Engine 执行（内存中）
    ↓
返回最终 GeoJSON
    ↓
...
func (e *Executor) executeStep(ctx context.Context, step *models.Step, stepResults models.StepResults) {
    ...
    // 3. 调用引擎执行（可能是 GeoPandas Engine, 也可能是 Transfer Worker）
...
| 文件路径 | 说明 | 代码量 |
|----------|------|--------|
| `geopandas-engine/workflow_engine.py` | GeoPandas 工作流引擎（DAG 执行 + 内存传递） | ~300 行 |
| `geopandas-engine/api_server.py` | Flask API Server | ~200 行 |
| `geopandas-engine/operators.py` | 空间算子库（buffer, centroid, intersection 等） | ~500 行 |
| `geopandas-engine/Dockerfile` | 容器化配置 | ~30 行 |
...
| 文件路径 | 改动说明 | 改动量 |
|----------|----------|--------|
| `docker-compose.yml` | 新增 geopandas-engine 服务 | +40 行 |
...
### 阶段 1: GeoPandas Engine 实现（3-4 天）

**任务**:
1. 创建 `geopandas-engine` 目录结构
...
**任务**:
1. **Backend**:
   - 创建 `spatial_workflow_service.go`（转发到 GeoPandas Engine）
...
| 脚本文件 | 改动说明 |
|---------|----------|
| `scripts/dev/start.sh` | 新增 geopandas-engine 启动步骤（端口8090健康检查） |
| `scripts/prod/start.sh` | 新增 geopandas-engine 容器启动和健康检查 |
| `scripts/infra/up.sh` | 确认 develop schema 存在（spatial_tasks 表依赖） |
| `scripts/build/compile.sh` | 无需改动（Python 服务不需编译） |
| `scripts/build/build-images.sh` | 新增 geopandas-engine 镜像构建 |
| `Makefile` | 新增 `make geopandas-engine` 和 `make geopandas-logs` 命令 |

### 需要更新的文档

| 文档文件 | 改动说明 |
|---------|----------|
| `CLAUDE.md` | 新增 GeoPandas Engine 章节（架构、端口、技术栈） |
| `docs/PORTS.md` | 新增端口8090（geopandas-engine） |
| `scripts/README.md` | 更新启动脚本说明（包含 geopandas-engine） |
| `develop/CLAUDE.md` | 新增空间工作流设计器章节 |
| `orchestrator/CLAUDE.md` | 更新任务发现机制（支持 GeoPandas 任务） |
...
| `geopandas-engine/workflow_engine.py` | GeoPandas 工作流引擎（DAG 执行 + 内存传递） | ~300 行 |
| `geopandas-engine/api_server.py` | Flask API Server | ~200 行 |
| `geopandas-engine/operators.py` | 空间算子库（21个算子实现） | ~500 行 |
| `geopandas-engine/requirements.txt` | Python 依赖（geopandas, flask, gunicorn, psycopg2, sqlalchemy） | ~15 行 |
| `geopandas-engine/Dockerfile` | 容器化配置 | ~30 行 |
| `develop/backend/internal/service/spatial_workflow_service.go` | 空间工作流服务（转发+任务保存） | ~400 行 |
| `develop/backend/internal/api/spatial_handler.go` | API 处理器 | ~250 行 |
| `develop/frontend/src/views/SpatialWorkflowDesigner.vue` | DAG 画布（基于 @antv/g6） | ~600 行 |
...
| `orchestrator/backend/internal/service/executor.go` | 扩展参数模板解析 | +100 行 |
| `develop/backend/internal/api/router.go` | 新增空间工作流路由组 | +15 行 |
| `scripts/dev/start.sh` | 新增 geopandas-engine 启动步骤 | +20 行 |
| `scripts/prod/start.sh` | 新增 geopandas-engine 容器启动 | +25 行 |
| `scripts/build/build-images.sh` | 新增 geopandas-engine 镜像构建 | +15 行 |
| `Makefile` | 新增 geopandas-engine 相关命令 | +10 行 |
| `CLAUDE.md` | 新增 GeoPandas Engine 章节 | +50 行 |
...
curl http://localhost:8080/internal/registry/capabilities | \
  jq '.[] | select(.unique_identifier == "geopandas.engine.default")'
```
...
**验证**:
- [ ] Orchestrator 任务库显示 GeoPandas Engine
...
2. **双模式访问**:
   ...
   - **Orchestrator 模块**: 从 GeoPandas Engine 动态发现任务，混合编排
