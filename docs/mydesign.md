...
后续超级管理员登录后，提供单独的系统配置页面，可动态刷新.env中的配置项
### 租户管理员
...
### 角色权限
后续提供基于角色的权限管控
后续提供外部账号体系的对接（思路待研究，关键是搞清楚应用场景和使用模式）
...
### 多租户强隔离 
当前是根据租户id进行隔离，后续可考虑根据租户进行内置数据库的database隔离，各租户管理自己的用户名密码
...
### 手动扫描
### 定时扫描
提供设置自动扫描时间的功能，分为每天、每周和每月，默认为该周期起始点。
### 局部扫描
在manager模块的数据预览中提供
## 扫描监控
...
扫描后的元数据，存储到系统数据库的meta schema中，供其他模块使用。
### 对象存储
