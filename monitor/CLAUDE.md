...
- provider health 从 System 读取模块定义中的 TaskProvider 声明及当前 Backend 租约，复用模块 `/health` 与标准 `GET /tasks?task_type=` 做无副作用探活；Monitor 不复制 capabilities、不修复模块声明、不读取 owner 私有表。
