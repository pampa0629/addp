<template>
  <div class="page-shell">
    ...
    <el-card shadow="never" class="filter-card"><el-row :gutter="12">
      ...
      <el-col :span="5"><BusinessDomainSelect v-model="filters.owner_domain_id" clearable :placeholder="t('standard.common.selectDomain')" style="width:100%" @change="search" :options="domains" /></el-col>
    ...
    <el-dialog v-model="dialog" :title="t('standard.metric.createTitle')" width="700px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="140px">
        <el-row :gutter="12"><el-col :span="12"><el-form-item :label="t('standard.metric.codeLabel')" prop="code"><el-input v-model="form.code" :placeholder="t('standard.common.codePlaceholder')" /></el-form-item></el-col><el-col :span="12"><el-form-item :label="t('standard.metric.nameLabel')" prop="name"><el-input v-model="form.name" /></el-form-item></el-col></el-row>
        ...
        <el-row :gutter="12"><el-col :span="12"><el-form-item :label="t('standard.common.scopeLabel')" prop="scope_type"><el-select v-model="form.scope_type" style="width:100%"><el-option v-for="scope in editableScopes" :key="scope" :value="scope" :label="scopeLabel(scope)" /></el-select></el-form-item></el-col><el-col :span="12"><el-form-item v-if="requiresOwnerDomain(form.scope_type)" :label="t('standard.common.ownerDomainLabel')" prop="owner_domain_id"><BusinessDomainSelect v-model="form.owner_domain_id" style="width:100%" :options="domains" /></el-form-item></el-col></el-row>
        <el-row :gutter="12"><el-col :span="12"><el-form-item :label="t('standard.metric.unitLabel')"><UnitSelect v-model="form.unit_id" :units="units" /></el-form-item></el-col></el-row>
...
import {buildStandardCodeRules,isValidStandardStableCode} from '../utils/standardCode'
...
const loadReferences=async()=>{try{const [domainRows,categoryRows,unitRows,candidates]=await Promise.all([domainAPI.list(),metricCategoryAPI.list(),unitAPI.list(),metricAPI.list({page_size:500})]);domains.value=buildBusinessDomainOptions(domainRows||[]);categories.value=categoryRows||[];units.value=unitRows||[];dependencyCandidates.value=candidates.data||[]}catch{domains.value=[];categories.value=[];units.value=[];dependencyCandidates.value=[]}}
...
const create=async()=>{if(!await formRef.value.validate().catch(()=>false))return;creating.value=true;try{const {dependency_ids,...fields}=form;const relation_kind=form.metric_type==='derived'?'base':'component';const row=await metricAPI.create({...fields,...buildStandardOwnership(form.scope_type,form.owner_domain_id),dependencies:dependency_ids.map(metric_definition_id=>({metric_definition_id,relation_kind}))});dialog.value=false;ElMessage.success(t('standard.common.createSuccess'));detail(row.id)}catch(error){ElMessage.error(getStandardErrorMessage(error,t))}finally{creating.value=false}}
