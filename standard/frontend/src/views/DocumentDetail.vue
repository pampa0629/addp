            ...
            <el-collapse v-model="expandedCandidateFamilies" class="candidate-families">
              <el-collapse-item v-for="family in candidateFamilies" :key="family.family_key" :name="family.family_key" class="candidate-family">
                ...
                <el-card v-for="(group, variantIndex) in family.variants" :id="candidateVariantCardId(family.family_key, group.semantic_fingerprint)" :key="group.semantic_fingerprint" shadow="never" class="candidate-card" tabindex="-1">
                  ...
                  <div v-if="canUpdate && (canDecideSingleCandidate(family, group) || canDecideCandidateFamily(family))" class="candidate-actions"><el-button v-if="canDecideSingleCandidate(family, group)" size="small" type="success" @click="decideCandidate(group.candidate, 'retained')">{{ $t('standard.document.retainCandidate') }}</el-button><el-button v-if="canDecideSingleCandidate(family, group)" size="small" @click="decideCandidate(group.candidate, 'rejected')">{{ $t('standard.document.rejectCandidate') }}</el-button><el-button v-if="canDecideCandidateFamily(family)" size="small" type="primary" plain :loading="decidingCandidateFamily === family.family_key" @click="decideCandidateFamily(family, group)">{{ $t('standard.document.selectCandidateFamilyWinner') }}</el-button></div>
