          ...
          <div v-loading="candidateLoading">
            <el-collapse v-model="expandedCandidateFamilies" class="candidate-families">
              <el-collapse-item v-for="family in candidateFamilies" :key="family.family_key" :name="family.family_key" class="candidate-family">
                ...
                <el-card v-for="group in family.variants" :key="group.semantic_fingerprint" shadow="never" class="candidate-card">
                  <div class="candidate-header"><div><strong>{{ group.candidate.name }}</strong><code>{{ group.candidate.code }}</code></div><el-tag :type="candidateGroupStateTagType(group.state)">{{ candidateGroupStateLabel(group.state) }}</el-tag></div>
...
const candidateFamilies = computed(() => candidateFamilyResponse.data || [])
