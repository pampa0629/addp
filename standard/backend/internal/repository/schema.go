...
func Migrate(db *gorm.DB) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.AutoMigrate(
			...
			&models.DocumentCandidateFamilyDecision{},
...
func postgresStandardSchemaStatements() []string {
	statements := []string{
		...
		"DO $do$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'standard.document_candidate_family_decisions'::regclass AND conname = 'ck_standard_document_candidate_family_decisions_type') THEN ALTER TABLE standard.document_candidate_family_decisions ADD CONSTRAINT ck_standard_document_candidate_family_decisions_type CHECK (candidate_type IN ('glossary','element','code_set','metric')); END IF; END $do$",
		"DO $do$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'standard.document_candidate_family_decisions'::regclass AND conname = 'ck_standard_document_candidate_family_decisions_reason') THEN ALTER TABLE standard.document_candidate_family_decisions ADD CONSTRAINT ck_standard_document_candidate_family_decisions_reason CHECK (CHAR_LENGTH(BTRIM(reason)) BETWEEN 1 AND 1000); END IF; END $do$",
		"ALTER TABLE standard.document_candidate_family_decisions DROP CONSTRAINT IF EXISTS ck_standard_document_candidate_family_decisions_members",
		"ALTER TABLE standard.document_candidate_family_decisions ADD CONSTRAINT ck_standard_document_candidate_family_decisions_members CHECK (jsonb_typeof(members) = 'array' AND jsonb_array_length(members) >= 2)",
		...
		"CREATE INDEX IF NOT EXISTS idx_standard_document_candidate_family_decisions_family ON standard.document_candidate_family_decisions (document_id, candidate_type, code, id DESC)",
	...
	for _, foreignKey := range []struct {
		...
		{"standard.document_candidate_family_decisions", "fk_standard_document_candidate_family_decisions_document", "document_id", "standard.documents(id)", "CASCADE"},
...
func sqliteStandardSchemaStatements() []string {
	return []string{
		...
		"CREATE INDEX IF NOT EXISTS standard.idx_standard_document_candidate_family_decisions_family ON document_candidate_family_decisions (document_id, candidate_type, code, id DESC)",
