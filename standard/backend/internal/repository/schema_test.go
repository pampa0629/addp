...
func TestPostgresSchemaStatementsDefineDeletePolicies(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"CONSTRAINT fk_standard_document_candidate_family_decisions_document FOREIGN KEY (document_id) REFERENCES standard.documents(id) ON DELETE CASCADE",
		"CONSTRAINT ck_standard_document_candidate_family_decisions_type CHECK",
		"CONSTRAINT ck_standard_document_candidate_family_decisions_reason CHECK",
...
func openStandardSchemaTestDB(t *testing.T) *gorm.DB {
	...
	statements := []string{
		...
		`CREATE TABLE standard.document_candidate_family_decisions (id INTEGER PRIMARY KEY, document_id INTEGER NOT NULL, candidate_type TEXT NOT NULL, code TEXT NOT NULL, winner_candidate_id INTEGER NOT NULL, reason TEXT NOT NULL, members TEXT NOT NULL, created_by INTEGER NOT NULL, created_at DATETIME)`,
