...
func TestBuildDocumentExtractionRejectsInvalidCandidateDataType(t *testing.T) {
	tests := []struct {
		...
		{name: "retired text type", candidateType: "element", dataType: "text"},
...
func openDocumentServiceTestDB(t *testing.T) *gorm.DB {
	...
	statements := []string{
		...
		`CREATE TABLE standard.documents (id INTEGER PRIMARY KEY AUTOINCREMENT, tenant_id INTEGER NOT NULL, scope_type TEXT NOT NULL, owner_domain_id INTEGER, code TEXT NOT NULL, doc_type TEXT NOT NULL, source_org TEXT, tags TEXT, draft_revision_id INTEGER, created_by INTEGER NOT NULL, updated_by INTEGER, created_at DATETIME, updated_at DATETIME, version INTEGER NOT NULL DEFAULT 1, lifecycle_state TEXT NOT NULL)`,
		...
		`CREATE TABLE standard.document_candidate_family_decisions (id INTEGER PRIMARY KEY AUTOINCREMENT, document_id INTEGER NOT NULL, candidate_type TEXT NOT NULL, code TEXT NOT NULL, winner_candidate_id INTEGER NOT NULL, reason TEXT NOT NULL, members TEXT NOT NULL, created_by INTEGER NOT NULL, created_at DATETIME)`,
