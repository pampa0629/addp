...
func TestElementRevisionResolutionReturnsEffectiveSnapshotAndExactCodeSet(t *testing.T) {
	...
	for _, statement := range []string{
		`INSERT INTO standard.elements (id, tenant_id, domain_id, code, lifecycle_state) VALUES (10, 7, 3, 'gender', 'active'), (20, 7, NULL, 'future', 'active'), (30, 8, NULL, 'foreign', 'active')`,
		...
		`INSERT INTO standard.code_sets (id, tenant_id, code, lifecycle_state) VALUES (50, 7, 'gender_codes', 'active')`,
	...
	if snapshot == nil || snapshot.ElementRevisionID != 101 || snapshot.DomainID == nil || *snapshot.DomainID != 3 || snapshot.ValueDomainKind != "enumeration" {
		t.Fatalf("snapshot = %#v", snapshot)
	}
	if snapshot.CodeSetRevision == nil || snapshot.CodeSetRevision.RevisionID != 501 || snapshot.CodeSetRevision.Status != "withdrawn" || len(snapshot.CodeSetRevision.Items) != 2 || snapshot.CodeSetRevision.Items[0].Code != "F" {
...
func openElementResolutionTestDB(t *testing.T) *gorm.DB {
	...
	for _, statement := range []string{
		`CREATE TABLE standard.elements (id INTEGER PRIMARY KEY, tenant_id INTEGER NOT NULL, domain_id INTEGER, code TEXT NOT NULL, lifecycle_state TEXT NOT NULL)`,
		...
		`CREATE TABLE standard.code_sets (id INTEGER PRIMARY KEY, tenant_id INTEGER NOT NULL, code TEXT NOT NULL, lifecycle_state TEXT NOT NULL)`,
