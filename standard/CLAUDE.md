...
- 候选族胜出裁决是多语义变体候选族的唯一处置路线，使用服务端候选族快照令牌执行原子命令，不把候选族升级为聚合根。候选族读取必须对未受筛选影响的当前全部代表候选按稳定顺序冻结 `candidate_id + version + semantic_fingerprint`，连同候选类型和编码计算 SHA-256 `snapshot_token`；请求只提交该令牌、唯一胜出代表候选 ID，以及去除首尾空白后 1–1000 字符的人工理由，不再由客户端重复提交无上限的成员数组。Standard 先锁定来源文档，再按候选 ID 稳定加锁，在同一事务中重算全部当前代表候选与快照令牌；令牌变化返回 409 `candidate_family_snapshot_stale`，胜出 ID 不是当前代表、候选族不足两个语义变体、候选已正式化或理由无效时整批拒绝。校验通过后把胜出候选置为 `retained`、其余当前代表置为 `rejected`、递增各自版本并追加一条 `document_candidate_family_decisions` 不可变治理事件。事件冻结候选类型、编码、胜出候选、理由、操作者、时间和全部成员的候选 ID、语义指纹、名称、裁决后版本及结果；成员数量只要求至少两个，不设置会造成候选族治理死锁的固定上限。后续重新裁决只追加新事件，不覆盖既有事件；原始内容、提炼批次与证据不改写，后续新提炼仍可产生新的待裁决变体。候选族读取同时返回未受筛选影响的 `total_variant_count` 和该族 `decision_count`，前端只有在当前返回完整候选族时才提供胜出裁决入口，并且不得同时提供单项处置入口。裁决历史通过独立分页读取接口按事件 ID 倒序返回，禁止把无界事件历史嵌入候选族列表。
- `standard.document_candidate_formalizations` 为候选的一对一不可变正式化事实，保存服务器判定的 `created_identity|created_revision|linked_existing`、目标稳定身份/修订及操作者。正式化要求候选为 `retained` 且仍是其语义变体的当前代表，使用候选 `version` 并在同一事务中重新计算代表、创建目标草稿或确认既有修订、写入正式化事实、递增候选版本；历史非代表候选返回 409 `candidate_representative_stale`，重复正式化返回 409。正式化后候选继续保持 `retained` 且不再允许重新裁决，避免来源事实与目标修订失配。
...
PUT /api/v1/standard/document-extraction-candidates/:candidate_id # 仅单语义变体候选族的 retained/rejected 人工处置
