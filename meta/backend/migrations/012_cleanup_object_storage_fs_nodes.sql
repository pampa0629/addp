...
--       导致写入 node_type=root/dir 和 item_type=file/lake_table，
--       与正确的 bucket/prefix/object 语义混用。
-- 本迁移仅清理对象存储引擎的错误数据，NFS/NAS 的 root/dir/file 不受影响。

BEGIN;

-- 备份受影响的 meta_item（item_type=file/lake_table，挂在对象存储引擎下）
...
WHERE e.engine_type IN ('minio', 's3', 'oss', 'object_storage', 'object-storage')
  AND mi.item_type IN ('file', 'lake_table')
...
-- 硬删除对象存储引擎下 file/lake_table 类型的 meta_item
