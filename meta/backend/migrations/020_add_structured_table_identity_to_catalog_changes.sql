...
BEGIN
    ...
    INSERT INTO meta.data_item_changes (
        ...
        jsonb_strip_nulls(jsonb_build_object(
            ...
            'schema_name', NULLIF(changed_item.attributes #>> '{storage,schema_name}', ''),
            'table_name', CASE WHEN changed_item.item_type IN ('table', 'view') THEN changed_item.name END,
...
SELECT
    ...
    jsonb_strip_nulls(jsonb_build_object(
        ...
        'schema_name', NULLIF(item.attributes #>> '{storage,schema_name}', ''),
        'table_name', CASE WHEN item.item_type IN ('table', 'view') THEN item.name END,
