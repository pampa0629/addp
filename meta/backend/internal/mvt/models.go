...
// PreprocessMetadata 预处理元数据（存储在 MinIO）
...
// TableStats 表统计信息（来自 pg_stat_user_tables）
type TableStats struct {
	TotalChanges int64     `json:"total_changes"` // n_tup_ins + n_tup_upd + n_tup_del
	LiveTuples   int64     `json:"live_tuples"`   // n_live_tup
	DeadTuples   int64     `json:"dead_tuples"`   // n_dead_tup
	LastAnalyze  time.Time `json:"last_analyze"`
}

// PreprocessProgress 预处理进度
