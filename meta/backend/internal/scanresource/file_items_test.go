...
func TestFileCatalogDetectedItemNameUsesPrimaryContentPathForMultiFile(t *testing.T) {
	...
	name, fullName := FileDetectedItemName("/shp", &metaitem.DetectedItem{
...
func TestFileCatalogDetectedItemNameKeepsWholeScopePath(t *testing.T) {
	...
	name, fullName := FileDetectedItemName("/lake/sales", &metaitem.DetectedItem{
		ResolvedItem: dataitem.ResolvedItem{
...
func TestPlanFileCatalogDetectedItemBuildsStablePlan(t *testing.T) {
	...
	plan, ok := PlanFileDetectedItem(9, "/tables", item, "file")
	if !ok {
		t.Fatalf("PlanFileDetectedItem ok = false")
	}
