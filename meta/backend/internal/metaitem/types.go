...
func (item *DetectedItem) ComponentFilePaths() []string {
	if item == nil || len(item.ComponentList) == 0 {
		return nil
	}
	paths := make([]string, 0, len(item.ComponentList))
...
func DetectedItemFromCompositeInfo(info *CompositeItemInfo, physicalPath string, fallbackSize int64) *DetectedItem {
	...
	return &DetectedItem{
		ResolvedItem: dataitem.ResolvedItem{
			...
			SizeBytes:     &sizeBytes,
			ComponentList: ComponentRefsFromPaths(info.ComponentFiles),
		},
...
func ComponentRefsFromPaths(paths []string) []dataitem.ComponentRef {
	if len(paths) == 0 {
		return nil
	}
	components := make([]dataitem.ComponentRef, 0, len(paths))
	for _, path := range paths {
		if path == "" {
			continue
		}
		components = append(components, dataitem.ComponentRef{Path: path})
	}
	return components
}
