...
func (s *FilesystemCatalogRuntime) persistFileCatalogDetectedItem(
	...
	result, err := scanprocessor.New(s.repo, s.indexer, s.log).WithCADInspector(s.cadInspector).WithContainerInspector(s.containerInspector).Process(ctx, scanprocessor.FileDetectedInput(
		resource,
		tenantID,
		parentNode,
		itemPlan,
