...
func NewS3Scanner(connStr string) (format.Scanner, error) {
	var cfg s3Config
...
func (s *S3Scanner) ListSchemas() ([]format.SchemaInfo, error) {
	s.ensureBuckets()
	var schemas []format.SchemaInfo
	for bucket := range s.allowedBucket {
		schemas = append(schemas, format.SchemaInfo{Name: bucket})
	}
...
func (s *S3Scanner) ScanTables(schemaName string) ([]format.TableInfo, error) {
	return []format.TableInfo{}, nil
}

// ScanFields not applicable
func (s *S3Scanner) ScanFields(schemaName, tableName string) ([]format.FieldInfo, error) {
	return []format.FieldInfo{}, nil
}

func (s *S3Scanner) ListNodes(path string) ([]format.ObjectNode, error) {
	s.ensureBuckets()
	path = strings.TrimSpace(path)
	if path == "" {
		var nodes []format.ObjectNode
		for bucket := range s.allowedBucket {
			nodes = append(nodes, format.ObjectNode{
				Name: bucket,
				Path: bucket,
				Type: "bucket",
			})
	...
	dirMap := make(map[string]*format.ObjectNode)
	var objectNodes []format.ObjectNode
...
func (s *S3Scanner) ScanPath(path string) ([]format.ObjectMetadata, error) {
	s.ensureBuckets()
	path = strings.TrimSpace(path)
	if path == "" {
		var meta []format.ObjectMetadata
		for bucket := range s.allowedBucket {
	...
	if hasReservedObjectSegment(prefix) {
		return []format.ObjectMetadata{}, nil
	}
	...
	if err == nil {
		...
		// 尝试提取详细元数据
		var extractedMetadata *format.ExtractedMetadata
		if s.resourceID > 0 {
...
func (s *S3Scanner) scanBucket(bucket, prefix string) ([]format.ObjectMetadata, error) {
	ctx := context.Background()
	...
	objects := []format.ObjectMetadata{}
	dirAgg := map[string]*format.ObjectMetadata{}
	...
	results := []format.ObjectMetadata{}

	bucketMeta := format.ObjectMetadata{
		Bucket:       bucket,
		Path:         bucket,
...
func (s *S3Scanner) ensureDirAggEntry(dirAgg map[string]*format.ObjectMetadata, bucket, dirPath string) {
	...
	dirAgg[dirPath] = &format.ObjectMetadata{
		Bucket:       bucket,
...
func filterReservedNodes(nodes []format.ObjectNode) []format.ObjectNode {
	var filtered []format.ObjectNode
...
func filterReservedMetadata(metas []format.ObjectMetadata) []format.ObjectMetadata {
	var filtered []format.ObjectMetadata
...
// size: 文件大小
// lastModified: 最后修改时间
func (s *S3Scanner) extractObjectMetadata(resID uint, bucket, objectKey, contentType string, size int64, lastModified time.Time) *format.ExtractedMetadata {
