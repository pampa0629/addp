...
func (e *DefaultExtractor) Extract(ctx context.Context, input format.ExtractInput) (*format.ExtractedMetadata, error) {
	// 1. 读取内容/计算校验和
	var checksum string
	var sample []byte
	if input.Reader != nil {
	...
	if len(sample) > 0 && isTextLike(input.ContentType, ext, fileType) {
		...
		if text != "" && !isLikelyBinary([]byte(text)) {
			trimmed := formatutils.TruncateRunes(text, defaultPlainTextLimit)
			metadata.CustomAttrs["plain_text"] = trimmed
			metadata.CustomAttrs["plain_text_preview"] = formatutils.TruncateRunes(trimmed, defaultPlainPreviewLimit)
		}
	}
