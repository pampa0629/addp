...
新建应用在第一个组件应用后显示连续创作引导，直接选择尚未配置选择来源的条件进入同一搜索列表向导，再进入同一整页预览设置初始值。候选仅来自契约就绪、可接收标量选择、没有固定选项且被结果组件消费的参数；排除已有选择目标和选择源自身专用搜索参数，不依据业务字段名推断。创建态新增搜索列表排在其首个目标组件之前；引导不写入快照，草稿创建后恢复常规编辑。中英文浏览器回归覆盖无初始人员标识的新建、取消、选人后设置默认值与保存重开，仍使用现有模块门禁与 CI 自动发现。

筛选条件的“创建搜索列表”复用 ApplicationComponentEditor：用户显式选择已发布 Query Service、支持 contains 的文本搜索字段、可选择的展示字段、与目标条件同类型且满足必填约束的取值字段。向导一次生成现有 Table Component、独立搜索参数和 Selection Binding，仍走同一组件编译、试查和保存路径；取消不写入，服务切换清空字段，目标契约未就绪或字段不兼容时阻止应用。不增加 API、持久化向导配置或领域字段推断；既有 Workbench T1/T3 和模块门禁覆盖。
