...
// @Description Chart 的 total_as_value 同步保存并校验期间维度、1–4 个度量与显式精度 | Chart total_as_value persists and validates its period dimension, 1–4 measures, and explicit precision
// @Description 统计期间展示绑定随草稿和发布修订保存；拒绝不存在、类型不匹配或开始结束相同的参数绑定 | Period presentation bindings persist in drafts and revisions; unknown, mismatched or identical start/end parameter bindings are rejected
// @Tags Workbench Data Applications
// @Accept json
// @Produce json
// @Param id path string true "Data Application UUID"
