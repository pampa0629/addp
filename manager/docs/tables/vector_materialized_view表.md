# vector_materialized_view 表结构说明

> 状态：当前实现说明。`manager.vector_materialized_view` 表达 Manager 创建并拥有生命周期的 3857 矢量物化视图结果状态。

## 一、表定位

`manager.vector_materialized_view` 回答：

> 当前 spatial item 是否存在可复用的 3857 矢量物化视图目标，该目标在哪里、是否可用、由哪次任务执行生成或刷新。
...
| `status` | varchar | 矢量物化视图结果 artifact state |
...
| 索引名 | 字段 | 说明 |
| --- | --- | --- |
| `idx_vector_materialized_view_tenant_item_fingerprint` | `tenant_id, item_fingerprint` | 查询某 item 的优化结果 |
...
| `idx_vector_materialized_view_status` | `status` | 按结果状态过滤 |
| `idx_vector_materialized_view_current_unique` | `tenant_id, item_fingerprint, source_geometry_column, target_srid` | 同一 item、几何列和目标 SRID 只允许一个当前目标 |

## 五、删除语义

删除矢量物化视图结果时，只删除 Manager 创建并登记的 3857 目标及其索引、结果记录和相关运行时缓存。
