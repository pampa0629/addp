# quick_view_optimization 表结构说明

> 状态：当前实现说明。`manager.quick_view_optimization` 表达 Manager 创建并拥有生命周期的 3857 快显性能优化结果状态。

## 一、表定位

`manager.quick_view_optimization` 回答：
...
2. `manager.quick_view_optimization_tasks` 的任务定义。
3. `manager.tile_cache` 的瓦片缓存结果。
...
| `task_id` | bigint | 产生或最近刷新该结果的 `manager.quick_view_optimization_tasks.id` |
...
- [快显概念说明](../快显概念说明.md)
- [快显实现规范](../快显实现规范.md)
- [quick_view_optimization_tasks 表结构说明](./quick_view_optimization_tasks表.md)
- [数据库架构](../数据库架构.md)
