# vector_quick_view_target_tasks 表结构说明

> 状态：当前实现说明。`manager.vector_quick_view_target_tasks` 表达快显性能优化任务定义，TaskProvider `task_type=vector_quick_view_target_generation`。

## 一、表定位

`manager.vector_quick_view_target_tasks` 回答：

> 以后按什么配置为哪个 PG 空间表创建或刷新 3857 快显优化目标。

它不替代：

1. `manager.vector_quick_view_targets` 的结果状态。
...
```text
GET  /api/v1/manager/tasks?task_type=vector_quick_view_target_generation
GET  /api/v1/manager/tasks/vector_quick_view_target_generation/{id}
POST /api/v1/manager/tasks/vector_quick_view_target_generation/{id}/execute
...
```text
GET    /api/v1/manager/vector_quick_view_target_tasks
POST   /api/v1/manager/vector_quick_view_target_tasks
GET    /api/v1/manager/vector_quick_view_target_tasks/{id}
PUT    /api/v1/manager/vector_quick_view_target_tasks/{id}
DELETE /api/v1/manager/vector_quick_view_target_tasks/{id}
```
...
- [快显概念说明](../快显概念说明.md)
- [快显实现规范](../快显实现规范.md)
- [vector_quick_view_target_generation 表结构说明](./vector_quick_view_targets表.md)
- [数据库架构](../数据库架构.md)
