...
瓦片缓存生成必然先创建任务定义，再执行。即使用户从空间预览页点击“生成瓦片缓存”，也应跳转瓦片缓存页面的“任务”tab 创建 `manager.vector_tile_cache_tasks`。如果当前 item 处于源表转换慢路径、`optimization.status=stale` 或瓦片响应提示 `vector_quick_view_target_generation`，UI 应先引导用户执行快显性能优化；优化结果 ready 后再生成瓦片缓存。
