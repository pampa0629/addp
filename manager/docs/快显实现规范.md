...
PPTX 在线预览的唯一技术路线是：Manager Backend 返回受管 artifact 状态；前端缺少结果时显式创建并执行 `pptx_pdf_generation`；独立 `manager-worker` 下载已授权源内容，在隔离临时目录中调用 LibreOffice 无头转换并原子发布 PDF 到 Manager infra MinIO；结果 ready 后，Manager Preview API 返回 `content.kind=pdf + preview_material=url + frontend_renderer=pdf`。前端不得下载完整 PPTX 后用 JSZip 解析或渲染，也不得动态依赖仓库外的 `open-file-viewer`。

Worker 默认单实例并发 1，可通过增加副本横向扩容；每个执行使用独立临时目录和 LibreOffice profile；根文件系统只读、以非 root 用户运行，只有临时目录可写；禁用宏、扩展和外部链接，生产环境不允许访问公网。转换超时、取消或租约丢失时必须终止 LibreOffice 子进程。发布前必须验证输出是非空且页数大于零的 PDF；失败不得覆盖上一版 ready 对象。

源内容版本优先使用 storage ETag；没有 ETag 时使用 `last_modified_at + size_bytes` 的规范化快照。`item_fingerprint` 只标识稳定数据项，不作为内容版本。结果对象按 `tenant_id + item_fingerprint + pdf_static + source_version` 组织，数据库中的当前结果更新与 execution 终态必须在持有当前 lease 的事务中完成；MinIO 与 PostgreSQL 无法组成事务，已上传但终态提交失败的对象由 cleanup 清理，不能补写数据库伪造成功。
