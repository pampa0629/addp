...
| `vector_tile_cache_generation` | `tile.format`，当前为 `mvt` |
...
每次执行都新建 execution，并刷新同一当前结果。同一任务已有 `pending` / `running` execution 时禁止并发再次执行；其他结束状态不阻止重新生成。结果失效使用 source version 与产物记录的源版本快照比较，不把 `item_fingerprint` 当作内容版本。

`raster_mosaic_generation` 是业务派生任务，不按单一源 item 合并。其语义身份必须包含规范化源范围、目标业务存储、dataset 名称、placement 和关键生成配置；完整语义不同时可以并存。
...
| `view_state` | 用户交互产生的轻量渲染状态；顶层按 `basic_preview` / `quick_view` 区分显示模式，模式内按 `map` / `scene_3d` 区分渲染域。`scene_3d` 内用 `render_source` 和可选 `artifact_version` 防止不同坐标语义或不同结果版本交叉复用相机。 |
...
非表型空间 item 通过 GeoPython Workflow 生成 MVT 时，`options` 中的 MVT 参数直接映射到 GDAL MVT driver。默认策略偏向快显质量：`simplification=0`、`simplification_max_zoom=0` 禁用额外抽稀；`mvt_extent=8192` 提高 tile 坐标量化精度；`max_tile_size_bytes=5000000` 避免 GDAL 默认 500KB 单瓦片上限过早触发降精度或丢要素；`num_threads=ALL_CPUS` 使用 GDAL 自身并行生成能力；`publish_concurrency=8` 并发发布生成的瓦片对象。若后续针对超大数据需要更激进的体积控制，应调整这些参数，而不是新增另一条瓦片生成路线。

执行路线仍然只有一个对外任务：

1. `source_kind=table` 且运行目标可由 PostGIS 提供时，Manager Backend 内部生成 MVT 并写入 `manager.vector_tile_cache`。
2. 非表型空间 item 由 Manager Backend 解析 locator、数据源连接和目标对象存储，生成不含 ADDP locator 的 `access_plan`，再 direct 调用 GeoPython Workflow 的向量瓦片算子。GeoPython Workflow 只处理物理 URI、格式、CRS、tile/options、progress_callback 和对象存储发布，不识别 ADDP locator，不写 Manager 任务表或结果表。
...
GeoPython Workflow 不直接写 `common.task_executions`。mosaic 生成过程中的阶段进度统一通过 Manager 内部事件入口汇总：
