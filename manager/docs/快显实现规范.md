...
> 状态：Manager 模块快显实现规范。本文定义 Manager 快显、快显性能优化、瓦片缓存结果和瓦片缓存生成任务的表结构、API 契约、内部技术路线、失效规则和验证记录。概念边界见 [快显概念说明](快显概念说明.md)。
...
`quick_view_optimization` 已实现后端 API、执行逻辑、TaskProvider 能力注册和 Manager 前端承载页面。Console 承载路由为：

```text
/manager/quick-view-optimization
```

该页面提供快显性能优化任务创建、编辑、执行、结果查看和从 ready 优化结果跳转生成瓦片缓存的入口。TaskProvider capabilities 中的 `create_url` / `edit_url` 指向该路由；Orchestrator 和 Monitor 仍应通过标准任务 API 和 execution API 交互，不依赖前端页面完成编排执行。
...
`config.optimization` 只控制 MVT 生成时的属性裁剪、瓦片体积阈值和 extent 降级策略。它不创建、不刷新 3857 物化视图，也不表达快显性能优化目标；3857 快显优化目标统一由 `quick_view_optimization` 任务管理。
...
X-ADDP-Tile-Recommendation: quick_view_optimization | tile_cache_generation
...
快显性能优化当前不提供 TaskProvider 标准取消能力。Manager provider 不注册 `task_cancel_endpoint`，`quick_view_optimization.supports_cancel=false`；Orchestrator 和 Monitor 不应展示标准取消入口。后续如需支持取消，必须先能用 execution 定位 worker 和 PG backend pid，可靠中断 `CREATE MATERIALIZED VIEW` / 建索引 / `ANALYZE`，处理 staging 对象清理、swap 临界区、旧 ready 结果保留和最终 execution 状态一致性。

快显性能优化当前也不声明任务自身定时调度能力，`quick_view_optimization.supports_schedule=false`。`schedule`、`next_run_at`、`last_run_at` 字段仅作为任务体系公共结构保留；周期性刷新应由 Orchestrator 定时编排间接触发已保存任务定义。开放 Manager 自身调度前，必须先确认 stale 判定后的刷新窗口、旧 ready 结果保留策略、源表和派生目标并发限制、失败保护、租户级速率控制和执行去重。
...
2. 空间预览中，当 capability 返回 `optimization.status=stale`、动态 MVT 处于 `source_transform_path` 或瓦片返回 `X-ADDP-Tile-Recommendation=quick_view_optimization` / `X-ADDP-Tile-Retry-Policy=suppress_tile` 时，展示“执行快显优化”入口；当瓦片返回 `X-ADDP-Tile-Recommendation=tile_cache_generation` 时，使用节流消息提示生成瓦片缓存，不在预览区域重复展示第二个提示块。
...
| 源 SRID 非 3857，但同源 schema 下已有可验证的 3857 外部物化视图 | 可作为只读快显性能目标使用；必须满足 populated、`geom_3857` 存在、实际 SRID 为 3857、GiST 索引有效，不写入 `manager.quick_view_optimization`，不获得 Manager 删除或刷新所有权。 |
...
4. 自动识别的外部 3857 物化视图不写入 `manager.quick_view_optimization`，因此没有 Manager 侧 `stale` 生命周期；每次只做轻量校验，校验失败即不采用。

外部 3857 目标的人工登记属于后续专题，应与自动识别和 Manager 自建目标保持三者分离：

1. 自动识别是运行时只读候选，不落结果表，不出现在结果列表中。
2. 人工登记是用户对外部只读目标的授权确认，应使用新的 artifact state 表达外部引用、授权确认人、最近校验摘要和失效诊断，不能写入 `manager.quick_view_optimization`。
3. Manager 自建目标才由 `quick_view_optimization` 任务创建、刷新和删除，并进入 `manager.quick_view_optimization` 生命周期。
...
1. 诊断阶段只读 catalog、统计信息和 `manager.quick_view_optimization`，判断几何列、SRID、空间索引、3857 优化目标、统计信息和估算行数。
2. 推荐阶段输出能力判断：可直接动态 MVT、建议执行快显性能优化、可生成缓存、或暂不可用。
3. 快显性能优化阶段必须由用户显式创建并执行 `quick_view_optimization` 任务，可创建 3857 物化视图或派生表、GiST 索引，并执行 `ANALYZE`。
...
这些字段不要求新增表结构；当前写入 `common.task_executions.metadata` 即可。`manager.tile_cache` 仍只表达结果状态和可消费位置，不承载详细执行统计。
...
自动识别的外部 3857 物化视图仍是只读消费对象，不写入 `manager.quick_view_optimization`，不进入结果列表，也不拥有 Manager stale、refresh、delete 生命周期；校验失败时只是不采用。
