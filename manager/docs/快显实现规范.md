...
> 状态：Manager 模块快显实现规范。本文定义 Manager 快显、矢量快显目标、矢量瓦片缓存、栅格 COG、栅格 mosaic 和相关任务的表结构、API 契约、内部技术路线、失效规则和验证记录。概念边界见 [快显概念说明](快显概念说明.md)。
...
| 矢量快显目标 | `manager.vector_quick_view_targets` | 当前 item 是否存在可复用的 3857 可索引渲染目标，以及该目标位置、状态和最近执行。 |
| 矢量快显目标任务 | `manager.vector_quick_view_target_tasks` | 可执行、可编排的 3857 渲染目标构建或刷新任务定义。 |
| 矢量瓦片缓存结果 | `manager.vector_tile_cache` | MVT 瓦片缓存是否可用、存储引用、格式、范围、层级和最近执行。 |
| 矢量瓦片缓存任务 | `manager.vector_tile_cache_tasks` | 可执行、可调度、可编排的 MVT 瓦片缓存生成任务定义。 |
...
空间快显管理由 Manager 前端统一承载，Console 承载路由为：

```text
/manager/spatial-quick-view/vector-optimization
...
前三个页面分别提供“任务 / 结果”管理。栅格 Mosaic 页面至少提供任务管理和生成结果跳转；mosaic 结果本身是业务 item，应在资源树和 Meta item 视角治理，不进入 Manager 私有结果表。TaskProvider capabilities 中的 `create_url` / `edit_url` 指向对应路由；Orchestrator 和 Monitor 仍应通过标准任务 API 和 execution API 交互，不依赖前端页面完成编排执行。

矢量快显目标固定生成 `source_row_id` 和 `geom_3857`。任务配置中的 `attributes` 只能包含最小业务属性列，不得包含源几何列、`geom_3857`、`source_row_id` 或重复列。
...
`manager.quick_view` 是快显偏好和预览交互状态表。
...
| `view_state` | 用户交互产生的轻量渲染状态；顶层按 `basic_preview` / `quick_view` 区分显示模式，模式内按 `map` / `scene_3d` 区分渲染域。 |
...
不得在 `view_state` 中按 `model_3d`、`tiles_3d`、`gaussian_splat` 等数据类型继续拆分状态；这些差异属于渲染器内部字段，而不是 `quick_view` 表结构。
...
当前实现中，同一租户下 `item_fingerprint + tile_format` 表达同一份源数据、同一种瓦片格式的当前结果。重复执行刷新当前结果；当次生成配置由任务定义、execution 和 manifest 承载。
...
### 3. `manager.vector_quick_view_targets`

`manager.vector_quick_view_targets` 是矢量快显目标结果状态表。
...
| `item_id` | 当前 Meta item 行引用，仅用于回查。 |
| `locator` | Resource Locator。 |
| `task_id` | 产生或最近刷新该结果的 `manager.vector_quick_view_target_tasks.id`。 |
...
矢量快显目标只登记 Manager 创建并拥有生命周期的 3857 可索引目标。删除结果时，只删除登记的 Manager 目标及其索引，不删除源表、源表原有索引或已生成矢量瓦片缓存。

### 4. `manager.vector_quick_view_target_tasks`

`manager.vector_quick_view_target_tasks` 是矢量快显目标生成任务定义表。
...
`schedule`、`next_run_at`、`last_run_at` 只是任务定义公共字段，不等于启用 Manager 自身定时刷新。当前能力仍声明 `supports_schedule=false`，因此 Manager 不启动内部 due-task scheduler；若需要周期性刷新，应由 Orchestrator 定时编排间接触发已保存任务定义。
...
`config.optimization` 只控制 MVT 生成时的属性裁剪、瓦片体积阈值和 extent 降级策略。它不创建、不刷新 3857 物化视图，也不表达矢量快显目标；3857 可索引目标统一由 `vector_quick_view_target_generation` 任务管理。
...
POST /api/v1/manager/raster_cog_tasks
GET  /api/v1/manager/raster_cog_tasks/{id}
PUT  /api/v1/manager/raster_cog_tasks/{id}
...
标准 TaskProvider 使用 `task_type=raster_cog_generation` 触发执行。执行器主路径为：Manager 解析 ResourceLocator、源 engine connection 和目标 `storage_ref`，派生 GDAL `source_uri` / `target_uri` / `gdal_env`，通过 `WorkflowRuntimeProvider.InvokeOperator("tiff_to_cog")` direct 调用 Python Workflow，直接生成并写入 infra MinIO COG，最后登记 `manager.raster_cog`。如果 Python Workflow 不可用、源 engine 不受支持、NFS 挂载不可达、对象存储 presigned URL 生成失败或 GDAL 写入失败，执行必须写入 failed execution 和 failed COG 结果，不能返回 ready。
...
4. `vector_tile_cache_generation` 和 `vector_quick_view_target_generation` 不承载 TIFF / COG 第一阶段快显。
...
4. 常规在线预览不得调用 Python Workflow；Python Workflow 只参与 `raster_mosaic_generation`、重建 overview、重建 leaf COG、重算 stats 或重建 tile cache 等离线任务。
...
标准 TaskProvider 使用 `task_type=raster_mosaic_generation` 触发执行。执行器主路径为：Manager 校验源 node locator、目标业务存储 locator 和任务配置，选择可用 Python Workflow runtime，并把 ADDP 业务语义解析为 `access_plan` 后调用 Python Workflow 的 `build_raster_mosaic` direct 算子。`access_plan` 包含 GDAL 可读写 root URI、GDAL 环境变量、候选发现规则、目标数据集 root、进度回调地址和诊断 metadata；原始 locator 只作为 metadata 保留，不由 Python 解析。Python Workflow 内部使用 GDAL Python API 组合落地：`gdal.Translate` 负责 TIFF 到 COG，`gdal.BuildVRT` 负责 virtual mosaic，`gdal.Translate` 负责生成全局 overview COG，并通过 GDAL callback 向 Manager 上报进度。源和目标都是 `/vsis3/` 且 endpoint/凭据不同时，Python Workflow 使用 GDAL path-specific option 将 source root 和 target root 分别绑定到各自的访问参数，避免把业务库凭证写入任务结果或 manifest。任务完成后由 Manager 或任务体系触发 Meta 刷新目标业务存储，形成 raster mosaic 业务 item。

Python Workflow 不直接写 `common.task_executions`。mosaic 生成过程中的阶段进度统一通过 Manager 内部事件入口汇总：
...
Manager 收到事件后必须校验租户、`execution_id`、`module=manager` 和 `task_type=raster_mosaic_generation`，并只更新 `progress`、`current_step`、`execution_time_ms`、`metadata.progress_event` 和 `updated_at`。已完成 execution 不再接受进度事件。最终成功或失败状态仍由 mosaic 任务执行主流程写入，失败时应保留最近一次 `progress_event` 作为诊断摘要。
...
X-ADDP-Tile-Recommendation: vector_quick_view_target_generation | vector_tile_cache_generation
...
`X-ADDP-Tile-Timeout-Budget-MS` 来自 `QUICK_VIEW_REALTIME_TILE_TIMEOUT_MS`，必须与后端实际动态 MVT 查询超时预算一致。`Retry-After` 只在 `X-ADDP-Tile-Retry-Policy=ttl` 时返回，默认来自 `QUICK_VIEW_REALTIME_TILE_RETRY_AFTER_SEC`。
...
快显性能优化当前不提供 TaskProvider 标准取消能力。Manager provider 不注册 `task_cancel_endpoint`，`vector_quick_view_target_generation.supports_cancel=false`；Orchestrator 和 Monitor 不应展示标准取消入口。后续如需支持取消，必须先能用 execution 定位 worker 和 PG backend pid，可靠中断 `CREATE MATERIALIZED VIEW` / 建索引 / `ANALYZE`，处理 staging 对象清理、swap 临界区、旧 ready 结果保留和最终 execution 状态一致性。

快显性能优化当前也不声明任务自身定时调度能力，`vector_quick_view_target_generation.supports_schedule=false`。`schedule`、`next_run_at`、`last_run_at` 字段仅作为任务体系公共结构保留；周期性刷新应由 Orchestrator 定时编排间接触发已保存任务定义。开放 Manager 自身调度前，必须先确认 stale 判定后的刷新窗口、旧 ready 结果保留策略、源表和派生目标并发限制、失败保护、租户级速率控制和执行去重。

结果状态：

```http
GET    /api/v1/manager/vector_quick_view_targets
GET    /api/v1/manager/vector_quick_view_targets/{id}
DELETE /api/v1/manager/vector_quick_view_targets/{id}
```

前端交互入口：

1. Manager 左侧菜单提供“快显性能优化”页面。
2. 空间预览中，当 capability 返回 `optimization.status=stale`、动态 MVT 处于 `source_transform_path` 或瓦片返回 `X-ADDP-Tile-Recommendation=vector_quick_view_target_generation` / `X-ADDP-Tile-Retry-Policy=suppress_tile` 时，展示“执行快显优化”入口；当瓦片返回 `X-ADDP-Tile-Recommendation=vector_tile_cache_generation` 时，使用节流消息提示生成瓦片缓存，不在预览区域重复展示第二个提示块。
3. 空间预览中，如果 capability 的 `optimization.target_kind=external_3857_materialized_view`，展示“已识别外部 3857 优化目标”，并引导生成瓦片缓存，不展示“执行快显优化”。
4. 瓦片缓存任务创建页区分三类提示：Manager ready 优化目标和外部 3857 只读目标只展示可复用提示；Manager stale 结果或非 3857 慢路径展示先执行快显性能优化的提示和跳转入口，但不禁用瓦片缓存任务创建。
5. 快显性能优化结果 ready 后，结果列表提供“生成瓦片缓存”入口；stale 结果提供刷新闭环。目标对象或索引失效类 stale 可重新执行原任务，源事实变化类 stale 应引导按当前 capability 新建或调整任务。
...
```json
{
  "optimization": {
    ...
    "task_type": "vector_quick_view_target_generation",
    "target_kind": "source_schema_materialized_view",
    "result_id": null
  }
}
...
2. 或存在 Manager 登记为 `ready` 的 `vector_quick_view_target_generation` 目标，且 `geom_3857` 有 GiST 索引。
...
1. 非 PG 空间数据缺少 PostGIS 动态 MVT、GiST、3857 优化目标和数据库级超时诊断，当前仍按 `direct_geojson_max_rows` 做直接快显准入控制。
...
4. PG 空间表大于 `direct_geojson_max_rows` 时，默认推荐 `realtime_tile`；没有可索引 3857 目标时标记为慢路径并提示快显性能优化。
...
| 几何有效性可接受 | 无效几何不能导致整条瓦片路径不可控，快显性能优化或生成阶段应记录失败原因。 |
| 记录数或规模可估计 | 用于非 PG 直接快显准入、PG direct GeoJSON 默认推荐和风险诊断；不作为 PG 动态 MVT 的硬性准入条件。 |
| 源空间索引 | 源表空间列应有 GiST 索引，用于源 CRS 诊断、快显性能优化前过滤和非 MVT 空间查询。 |
...
| 源 SRID 非 3857，但已有 Manager 登记的 ready 快显性能优化目标且 `geom_3857` 有有效 GiST 索引 | 可使用优化目标作为 `realtime_tile` 目标。 |
| 源 SRID 非 3857，但同源 schema 下已有可验证的 3857 外部物化视图 | 可作为只读快显性能目标使用；必须满足 populated、`geom_3857` 存在、实际 SRID 为 3857、GiST 索引有效，不写入 `manager.vector_quick_view_targets`，不获得 Manager 删除或刷新所有权。 |
...
| 缺少可索引 3857 目标 | 可继续按源表能力提示直接 GeoJSON、小规模动态瓦片或瓦片缓存生成，同时暴露“建议执行快显性能优化”。 |
| 源 SRID 已是 3857 但缺少 GiST 索引 | 可使用源表作为动态 MVT 目标，但标记为 `source_3857_unindexed`；当前不创建快显性能优化任务，超时闭环引导生成瓦片缓存并抑制重复慢请求。 |

源几何列已是 3857 时，当前不创建快显性能优化任务，也不生成冗余 3857 物化视图；capability 应直接诊断为 `source_3857_indexed` 或 `source_3857_unindexed`。
...
4. 没有可索引 3857 目标时，动态 MVT capability 仍可开放，但应标记 `performance_mode=source_transform_path` 或等价慢路径，并提示“建议执行快显性能优化以降低超时风险和改善稳定性”。
5. 有可索引 3857 目标后，动态 MVT 标记为 `performance_mode=ready_3857_target` 或 `source_3857_indexed`，仍应保留单瓦片超时、体积和并发保护。
6. 瓦片缓存生成不因数据量大而禁止执行；没有可索引 3857 目标时，应给出更强提示，说明任务可执行但会走源表转换路径，建议先执行快显性能优化。
7. 无可索引 3857 目标时，如果某个动态 MVT 瓦片超时，前端应对该 tile key 做更强抑制，不反复请求同一慢瓦片，并通过节流消息提示执行快显性能优化。
...
4. 自动识别的外部 3857 物化视图不写入 `manager.vector_quick_view_targets`，因此没有 Manager 侧 `stale` 生命周期；每次只做轻量校验，校验失败即不采用。

外部 3857 目标的人工登记属于后续专题，应与自动识别和 Manager 自建目标保持三者分离：

1. 自动识别是运行时只读候选，不落结果表，不出现在结果列表中。
2. 人工登记是用户对外部只读目标的授权确认，应使用新的 artifact state 表达外部引用、授权确认人、最近校验摘要和失效诊断，不能写入 `manager.vector_quick_view_targets`。
3. Manager 自建目标才由 `vector_quick_view_target_generation` 任务创建、刷新和删除，并进入 `manager.vector_quick_view_targets` 生命周期。
4. 删除人工登记只删除登记记录，不删除外部表、外部物化视图或外部索引；刷新外部目标不由 Manager 自动执行。
5. 如果用户希望由 Manager 接管外部目标，应通过新的快显性能优化任务迁移为 Manager 自建目标，而不是原地改变外部对象所有权。
...
| `X-ADDP-Tile-Timeout-Budget-MS` | 当前动态 MVT 单瓦片超时预算，来自 `QUICK_VIEW_REALTIME_TILE_TIMEOUT_MS`。 |
| `X-ADDP-Tile-Recommendation` | 当瓦片语义状态为 `timeout` 或 `degraded` 时返回推荐动作，例如 `vector_quick_view_target_generation` 或 `vector_tile_cache_generation`。 |
| `X-ADDP-Tile-Retry-Policy` | 当瓦片语义状态为 `timeout` 或 `degraded` 时返回重试策略；无可索引 3857 目标的慢路径超时使用 `suppress_tile`，可索引 3857 目标超时使用 `ttl`。 |
| `Retry-After` | `X-ADDP-Tile-Retry-Policy=ttl` 时返回，表示前端可按 TTL 再次请求；前端应优先使用该值作为冷却时间，默认来自 `QUICK_VIEW_REALTIME_TILE_RETRY_AFTER_SEC`。 |
...
3. 每组样本至少验证四个场景：无可索引 3857 目标且走源表 `ST_Transform` 路径、源表本身 3857 且有 GiST 索引、存在 Manager ready 快显性能优化目标且 `geom_3857` 有 GiST 索引、存在可验证外部 3857 物化视图。
...
7. 验证产物应给出两组配置建议：无可索引 3857 目标时的响应时间预算与超时提示策略，以及可索引 3857 目标存在时的响应时间预算与缓存推荐策略；两组策略都必须保留单瓦片超时、体积和并发保护，并回填到 `QUICK_VIEW_REALTIME_TILE_TIMEOUT_MS` / `QUICK_VIEW_REALTIME_TILE_RETRY_AFTER_SEC` 的默认值或部署建议。
...
```json
{
  ...
  "scenarios": [
    ...
    {
      "name": "roads quick view optimization target",
      "target_kind": "source_schema_materialized_view",
      "schema": "public",
      "table": "addp_qvo_8f4c6a2d9b1e7c0a5d3f6b42",
...
1. 诊断阶段只读 catalog、统计信息和 `manager.vector_quick_view_targets`，判断几何列、SRID、空间索引、3857 优化目标、统计信息和估算行数。
2. 推荐阶段输出能力判断：可直接动态 MVT、建议执行快显性能优化、可生成缓存、或暂不可用。
3. 快显性能优化阶段必须由用户显式创建并执行 `vector_quick_view_target_generation` 任务，可创建 3857 物化视图或派生表、GiST 索引，并执行 `ANALYZE`。
4. 消费阶段中，动态 MVT 和瓦片缓存生成都优先使用同一套可索引 3857 目标解析结果。
5. 清理阶段只清理 Manager 创建并登记的快显性能优化目标，不能让派生对象成为业务库里的隐形遗留物。

快显性能优化目标不存储到 ADDP infra PG 的 `manager` schema 中。infra PG 只保存 Manager 自身的任务定义、结果记录和执行摘要，不承载用户源空间数据的派生副本。
...
快显性能优化不修改源表记录、不改写源几何列、不删除源数据。它创建的目标是 Manager 为快显和瓦片缓存生成登记管理的派生计算目标。
...
update manager.vector_quick_view_targets ready
```
...
读取源 item 和快显能力事实
  ...
  -> 优先选择可索引 3857 目标
  -> 缺少可索引 3857 目标时使用源表生成并记录快显性能优化推荐
  -> 生成 MVT 瓦片并保存到任务配置指定的 storage_ref
```

瓦片缓存生成执行必须：

1. 读取 `manager.vector_tile_cache_tasks.config`。
2. 创建 execution。
3. 创建或更新 `manager.vector_tile_cache` 为 `generating`。
4. 如果当前源 item 已有同格式缓存结果，先清理旧 `storage_ref` 指向的对象前缀，保证本次执行是真正刷新，不复用旧瓦片。
5. 同时按 `vector_tile_cache:{id}` 失效内存 LRU 和 Redis 中的运行时瓦片缓存，避免对象存储已刷新但预览仍命中旧瓦片。
6. 解析可索引 3857 生成目标；存在 Manager ready 优化目标或可验证外部 3857 物化视图时优先使用，缺少目标时使用源表生成并在 execution metadata 中记录快显性能优化推荐，不自动执行准备动作。
7. 生成瓦片和 manifest。
8. 更新 `manager.vector_tile_cache` 为 `ready` / `failed` / `cancelled`。
9. 更新任务最近执行摘要。
...
| 源 SRID 非 3857，且已有 Manager 登记的 ready 快显性能优化目标和 `geom_3857` GiST 索引 | 快显性能优化目标。 |
...
1. 不删除 `manager.vector_quick_view_targets` 结果记录。
2. 结果状态进入 `failed`。
...
自动识别的外部 3857 物化视图仍是只读消费对象，不写入 `manager.vector_quick_view_targets`，不进入结果列表，也不拥有 Manager stale、refresh、delete 生命周期；校验失败时只是不采用。
