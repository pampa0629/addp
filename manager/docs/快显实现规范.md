...
Manager 的可重复执行任务由统一“数据任务”工作台承载，Console 承载路由为：

```text
/manager/derived-tasks?category=managed_quick_view
/manager/derived-tasks?category=spatial_business
/manager/derived-tasks?category=embedding
```

统一“数据任务”工作台提供“快显任务 / 空间数据任务 / 向量化任务”三个一级 Tab。前两个 Tab 复用生成类任务的列表、分页、详情、执行、监控和删除实现；向量化 Tab 复用既有向量化任务与向量化结果子视图。界面合并不改变存储边界：生成类任务继续使用 `manager.task_definitions`，向量化任务继续使用 `manager.embedding_tasks`。快显任务不提供独立“结果”跳转：其当前结果是 Manager infra 快显材料，用户应通过“源数据”回到 data item 预览；空间业务结果仍在资源树和 Meta item 视角治理。TaskProvider capabilities 中的 `create_url` / `edit_url` 统一指向该路由；Orchestrator 和 Monitor 仍通过标准任务 API 和 execution API 交互。

快显任务列表必须从 Manager 快显结果表读取当前状态，不得用 `last_execution_status` 推断快显是否可用。空间数据任务的“目标数据”入口必须按目标 Engine 和 catalog path 向 Meta 查询当前 DataItem，然后构造带 `item_id` 的规范 ResourceLocator；不得把任务配置中尚未被 Meta 发现的目标路径伪装成可导航的 item。
...
该创建表单归入统一“数据任务 → 空间数据任务”Tab，唯一前端路由为 `/manager/derived-tasks?category=spatial_business&task_type=vector_tile_set_generation`。Console 导航与资源树空间数据项快捷操作必须复用该路由；资源树入口通过 `create=1&locator=...` 预选来源，不建立第二套表单或执行路径。
...
上述受管当前结果任务统一声明 `supports_schedule=false`，Manager 不为它们启动 owner scheduler，但允许 Orchestrator 定时 Pipeline 调用。周期性刷新由用户在 Step 参数中显式配置 `existing_result_action=overwrite`；该动作由 Orchestrator 每次原样提交，Manager 不按 scheduled 来源自动补充。`pptx_pdf_generation` 的普通交互入口仅在用户打开预览且缺少当前结果时由前端显式提交一次；Meta scan、上传和 Manager Preview GET 都不预生成。该提交和统一“数据任务”页都必须使用 Quick View Capability 返回的 `generate_pptx_pdf` 动作，并通过 `POST /quick-view/actions` 执行；不得再保留 PPTX 私有的创建或执行路由。Embedding 的逐 item 调度语义独立保留。
...
该地址在统一“数据任务”页展示只读任务定义，也是 TaskProvider `edit_url`。TaskProvider `create_url` 指向 `/manager/derived-tasks?category=managed_quick_view&task_type=raster_cog_generation&create=1`，由统一页面选择源 item；该入口不是空白任务配置表单，不允许调用方提交目标位置或 Manager 私有生成参数。
