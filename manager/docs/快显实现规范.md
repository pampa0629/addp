...
| 派生任务定义 | `manager.task_definitions` | 统一保存全部 Manager 可重复执行任务；`task_type` 决定强类型配置、执行器和结果策略。 |
...
统一“派生任务”页面提供“快显管理 / 空间任务”两个分类。任务列表、分页、详情、执行、监控和删除只有一个实现；类型专属创建与编辑表单作为同一路由内的领域组件存在。业务结果在资源树和 Meta item 视角治理，不进入 Manager 私有结果表。TaskProvider capabilities 中的 `create_url` / `edit_url` 指向统一路由；Orchestrator 和 Monitor 仍通过标准任务 API 和 execution API 交互。
