...
地图与表格联动的单要素 centroid / geometry 读取统一调用 `SpatialFeatureReadProvider`。Manager Handler 只负责租户校验、Resource CatalogPath 构造、EWKB 转 GeoJSON、extent 组装和 CRS 响应契约；PostGIS、MySQL、Oracle 的原生空间 SQL、标识符引用和 geographic axis order 由各自插件负责，不保留 PostGIS 专用 Handler 路线。
