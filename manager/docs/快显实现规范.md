...
> 状态：Manager 模块快显实现规范。本文定义 Manager 快显、矢量物化视图、矢量瓦片缓存、栅格 COG、栅格 mosaic 和相关任务的表结构、API 契约、内部技术路线、失效规则和验证记录。概念边界见 [快显概念说明](快显概念说明.md)。
...
`target` 的主身份是 `source_engine_id + locator + item_fingerprint`。`schema/table` 只用于 `type=table` 的数据库表执行事实；文件、对象等非表型空间 item 不伪造 `schema/table`。Manager 解析 ResourceLocator 得到 `source_kind/full_name/item_id`，并用 `GenerateItemFingerprint(source_engine_id, full_name)` 统一校验或生成 `item_fingerprint`。

非表型空间 item 通过 Python Workflow 生成 MVT 时，`options` 中的 MVT 参数直接映射到 GDAL MVT driver。默认策略偏向快显质量：`simplification=0`、`simplification_max_zoom=0` 禁用额外抽稀；`mvt_extent=8192` 提高 tile 坐标量化精度；`max_tile_size_bytes=5000000` 避免 GDAL 默认 500KB 单瓦片上限过早触发降精度或丢要素；`num_threads=ALL_CPUS` 使用 GDAL 自身并行生成能力；`publish_concurrency=8` 并发发布生成的瓦片对象。若后续针对超大数据需要更激进的体积控制，应调整这些参数，而不是新增另一条瓦片生成路线。

执行路线仍然只有一个对外任务：

1. `source_kind=table` 且运行目标可由 PostGIS 提供时，Manager Backend 内部生成 MVT 并写入 `manager.vector_tile_cache`。
2. 非表型空间 item 由 Manager Backend 解析 locator、数据源连接和目标对象存储，生成不含 ADDP locator 的 `access_plan`，再 direct 调用 Python Workflow 的向量瓦片算子。Python Workflow 只处理物理 URI、格式、CRS、tile/options、progress_callback 和对象存储发布，不识别 ADDP locator，不写 Manager 任务表或结果表。
3. 两条内部执行分支的结果都回写同一张 `manager.vector_tile_cache`，瓦片读取仍统一走 `/quick-view/tiles/{z}/{x}/{y}.mvt?locator=...` 和 `render_source=cached_tile`。
...
`raster_cog_tasks.config.target`、`config.raster` 和 `config.cog` 必须由 Manager 后端根据 capability 和 Preview source 事实生成。前端不得自行拼接 raster COG 任务私有配置；任务管理页仍可通过 `raster_cog_tasks` 管理显式任务定义。
...
矢量物化视图当前也不声明任务自身定时调度能力，`vector_materialized_view_generation.supports_schedule=false`。`schedule`、`next_run_at`、`last_run_at` 字段仅作为任务体系公共结构保留；周期性刷新应由 Orchestrator 定时编排间接触发已保存任务定义。开放 Manager 自身调度前，必须先确认 stale 判定后的刷新窗口、旧 ready 结果保留策略、源表和派生目标并发限制、失败保护、租户级速率控制和执行去重。
...
6. 解析可索引 3857 生成目标；存在 Manager ready 优化目标或可验证外部 3857 物化视图时优先使用，缺少目标时使用源表生成并在 execution metadata 中记录矢量物化视图推荐，不自动执行准备动作。
7. 生成瓦片和 manifest。
