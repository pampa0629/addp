...
上述受管当前结果任务统一声明 `supports_schedule=false`，Manager 不为它们启动 owner scheduler，但允许 Orchestrator 定时 Pipeline 调用。周期性刷新由用户在 Step 参数中显式配置 `existing_result_action=overwrite`；该动作由 Orchestrator 每次原样提交，Manager 不按 scheduled 来源自动补充。`pptx_pdf_generation` 的普通交互入口仅在用户打开预览且缺少当前结果时由前端显式提交一次；Meta scan、上传和 Manager Preview GET 都不预生成。Embedding 的逐 item 调度语义独立保留。

### PPTX 静态 PDF 预览

PPTX 在线预览的唯一技术路线是：Manager Backend 返回受管 artifact 状态；前端缺少结果时显式创建并执行 `pptx_pdf_generation`；Manager 通过 Common `WorkflowRuntimeProvider` direct 调用 Document Workflow `document_to_pdf`。交互入口只接收带 `item_id` 的 ResourceLocator，Manager 必须联查当前租户 Meta DataItem，并在服务端生成 `item_fingerprint`、`source_version` 与源大小，不信任前端提交的缓存身份。同一源版本转换失败后，普通查询只返回失败状态；用户点击重试时在同一交互入口显式提交 `retry=true`，不得因重新打开页面而自动重复执行。Manager 负责把已授权源和 infra PDF 目标派生为 `addp.workflow.access-plan/v1`，Document Workflow 在隔离工作目录中调用 LibreOffice 无头转换并按目标计划原子发布；结果 ready 后，Manager Preview API 返回 `content.kind=pdf + preview_material=url + frontend_renderer=pdf`。前端不得下载完整 PPTX 后用 JSZip 解析或渲染，也不得动态依赖仓库外的 `open-file-viewer`。
