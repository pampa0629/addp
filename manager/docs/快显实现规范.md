...
4. PG 空间表大于 `direct_geojson_max_rows` 时，默认推荐 `realtime_tile`；没有可索引 3857 目标时标记为慢路径并提示矢量物化视图。
...
| 缺少可索引 3857 目标 | 可继续按源表能力提示直接 GeoJSON、小规模动态瓦片或瓦片缓存生成，同时暴露“建议执行矢量物化视图”。 |
