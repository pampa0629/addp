...
`pptx_pdf_generation` 的唯一运行入口是独立 `manager-bounded-worker`。Manager Backend 只创建 `pending` execution；Worker 使用 PostgreSQL claim + lease 领取后调用 Document Workflow，并在执行期间续租。进程退出或失联导致 lease 过期时，Worker 必须将 execution、任务摘要以及属于该 execution 的 `building` 结果原子收敛为 `failed`，错误码固定为 `manager.pptx_pdf.lease_expired`；不得在缺少跨 Runtime fencing 的情况下自动重放同一 execution。旧 attempt 的迟到终态必须因 attempt/token 不匹配而被拒绝。
