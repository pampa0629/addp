...
| 向量化任务定义 | 用户从“数据任务 → 向量化任务”入口创建的可重复执行、可调度、可编排配置 | `manager.embedding_tasks` | 是 |
...
只有用户显式从“数据任务 → 向量化任务”入口创建的向量化配置，才是向量化任务定义，才可以通过 TaskProvider 暴露给 Orchestrator。
...
Manager 在统一“数据任务 → 向量化任务”入口中区分两个页内视图：
