...
**data item 是向量化对象，node 是批量选择范围；资源树向量化是一次性操作，“数据任务”的向量化 Tab 创建的配置才是任务；向量化结果按 item 指纹去重，必须可查看、可重建、可清理。**
...
| 向量化任务定义 | 用户在“数据任务”的“向量化任务”Tab 创建的可重复执行、可调度、可编排配置 | `manager.embedding_tasks` | 是 |
...
只有用户显式在“数据任务”的“向量化任务”Tab 创建的向量化配置，才是向量化任务定义，才可以通过 TaskProvider 暴露给 Orchestrator。
...
Manager 不拥有模型厂商或推理端点配置。它只拥有 `semantic_search_embedding` 场景绑定和向量化业务策略：

- Manager 在 `manager.inference_scenario_bindings` 保存平台默认和租户覆盖的 Model Profile ID，解析顺序固定为 Tenant 显式绑定 > 平台默认绑定 > 明确未配置错误。
- Manager 在 `manager.embedding_configuration` 保存距离阈值、最大文件大小和批处理并发等业务策略。
- Platform System Administrator 维护平台默认绑定和平台策略；Tenant Administrator 只维护本租户覆盖绑定。
- Console 只聚合 Manager 发布的配置管理入口；System 只登记入口能力、提供 AuthContext、Permission 和统一审计，不保存或解释模型字段。
- Provider Connection、Model Deployment、Model Profile 和加密凭据全部由 Inference Runtime 管理；Manager 不保存、返回或接收 API Key。
- 当前不建立 text/image/video 三套隐藏绑定；数据项向量化与检索查询必须使用同一个场景绑定，确保向量空间一致。
- 模型输出维度同时约束 pgvector 列结构，因此是只读结构事实，不是可热更新的普通配置。

Profile ID 或 Profile Version 变化时，既有 `ready` 结果统一视为 `outdated`，必须显式重新向量化后才能参与检索。不同维度切换必须先完成数据库结构迁移和全量重建方案，Manager 普通策略 API 不负责修改模型维度。

任务定义和 execution 必须保存实际使用的 Model Profile ID、Profile Version、Deployment ID 与维度快照。绑定变化不能静默改写已有任务定义或历史 execution；已有任务若与当前有效绑定不一致，必须先显式更新任务配置才能再次执行。
...
资源树行内只保留刷新快捷操作，不展示向量化状态或向量化快捷图标。用户选中资源后，在右侧资源操作区使用“向量化”，或在“数据任务”的“向量化任务” Tab 中创建可重复执行的任务。
...
Manager 在统一“数据任务”工作台的“向量化任务” Tab 中区分两个视图：
...
4. “数据任务”的向量化 Tab 创建的配置才是任务定义，并可被 Orchestrator 发现。
...
10. Model Profile 绑定归 Manager，支持平台默认与租户覆盖；模型资源和凭据归 Inference，System 不保存业务配置值，Console 不解释配置字段。
