...
2. 空间预览中，当动态 MVT 处于 `source_transform_path` 或瓦片返回 `X-ADDP-Tile-Recommendation=quick_view_optimization` / `X-ADDP-Tile-Retry-Policy=suppress_tile` 时，展示“执行快显优化”入口。
3. 瓦片缓存任务创建页如果 capability 显示非 3857 慢路径且没有 ready 优化结果，展示先执行快显性能优化的提示和跳转入口。
4. 快显性能优化结果 ready 后，结果列表提供“生成瓦片缓存”入口，形成“先优化、再缓存、回到快显”的用户闭环。
