...
2. 空间预览中，当动态 MVT 处于 `source_transform_path` 或瓦片返回 `X-ADDP-Tile-Recommendation=quick_view_optimization` / `X-ADDP-Tile-Retry-Policy=suppress_tile` 时，展示“执行快显优化”入口；当瓦片返回 `X-ADDP-Tile-Recommendation=tile_cache_generation` 时，提示生成瓦片缓存。
3. 空间预览中，如果 capability 的 `optimization.target_kind=external_3857_materialized_view`，展示“已识别外部 3857 优化目标”，并引导生成瓦片缓存，不展示“执行快显优化”。
4. 瓦片缓存任务创建页如果 capability 显示非 3857 慢路径且没有 ready 优化结果，展示先执行快显性能优化的提示和跳转入口；如果识别到外部 3857 优化目标，展示可复用提示但不提供快显优化任务入口。
5. 快显性能优化结果 ready 后，结果列表提供“生成瓦片缓存”入口，形成“先优化、再缓存、回到快显”的用户闭环。
...
4. PG 空间表大于 `direct_geojson_max_rows` 时，默认推荐 `realtime_tile`；没有 ready 快显性能优化目标时标记为慢路径并提示快显性能优化。
...
| 无 ready 快显性能优化目标 | 可继续按源表能力提示直接 GeoJSON、小规模动态瓦片或瓦片缓存生成，同时暴露“建议执行快显性能优化”。 |
...
5. 有 ready 快显性能优化目标后，动态 MVT 标记为 `performance_mode=ready_3857_target`，仍应保留单瓦片超时、体积和并发保护。
6. 瓦片缓存生成不因数据量大而禁止执行；没有 ready 快显性能优化目标时，应给出更强提示，说明任务可执行但会走源表转换路径，建议先执行快显性能优化。
7. 无 ready 快显性能优化目标时，如果某个动态 MVT 瓦片超时，前端应对该 tile key 做更强抑制，不反复请求同一慢瓦片，并提示执行快显性能优化。
8. 有 ready 快显性能优化目标时，如果某个动态 MVT 瓦片仍超时，前端按既有 TTL 允许后续重试，并提示生成瓦片缓存；此类超时通常意味着单瓦片内容过重、低层级面过复杂或数据库临时压力过高。
...
| `X-ADDP-Tile-Retry-Policy` | 当瓦片语义状态为 `timeout` 或 `degraded` 时返回重试策略；无 ready 快显性能优化目标的慢路径超时使用 `suppress_tile`，ready 快显性能优化目标超时使用 `ttl`。 |
| `Retry-After` | `X-ADDP-Tile-Retry-Policy=ttl` 时返回，表示前端可按 TTL 再次请求。 |

前端只消费上述 header 做运行时请求抑制和诊断，不把缺少 ready 快显性能优化目标实现为动态 MVT 或瓦片缓存生成的入口禁用。
...
3. 每组样本至少验证三个场景：无 ready 快显性能优化目标且走源表 `ST_Transform` 路径、源表本身 3857 且有 GiST 索引、存在 ready 快显性能优化目标且 `geom_3857` 有 GiST 索引。
...
7. 验证产物应给出两组配置建议：无 ready 快显性能优化目标时的响应时间预算与超时提示策略，以及 ready 快显性能优化目标存在时的响应时间预算与缓存推荐策略；两组策略都必须保留单瓦片超时、体积和并发保护。
...
```json
{
  ...
  "scenarios": [
    ...
    },
    {
      ...
      "target_kind": "quick_view_optimization_target",
      "schema": "public",
...
4. 消费阶段中，动态 MVT 和瓦片缓存生成都优先使用同一 ready 快显性能优化目标。
...
| 源 SRID 非 3857，且缺少可索引快显性能优化目标 | 使用源表生成，不做数据量限制，并在 metadata 中记录 `optimization_recommended=true` 与强提示。 |

任务定义、`manager.tile_cache` 结果和对象存储路径仍然以源 item 为身份，不能因为生成目标切到派生产物而改变 `item_fingerprint`、`locator` 或结果归属。execution metadata 应记录实际生成目标，至少包括 schema、table、geometry column、SRID 和是否为快显性能优化目标。
...
| `tile_generation_target` | 实际生成目标表、几何列、SRID，以及是否使用快显性能优化目标。 |
