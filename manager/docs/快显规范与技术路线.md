...
3. 没有 ready 快显性能优化目标时，动态 MVT capability 仍可开放，但应标记 `performance_mode=source_transform_path` 或等价慢路径，并提示“建议执行快显性能优化以降低超时风险和改善稳定性”。
4. 有 ready 快显性能优化目标后，动态 MVT 标记为 `performance_mode=ready_3857_target`，仍应保留单瓦片超时、体积和并发保护。
5. 瓦片缓存生成不因数据量大而禁止执行；没有 ready 快显性能优化目标时，应给出更强提示，说明任务可执行但会走源表转换路径，建议先执行快显性能优化。
6. 无 ready 快显性能优化目标时，如果某个动态 MVT 瓦片超时，前端应对该 tile key 做更强抑制，不反复请求同一慢瓦片，并提示执行快显性能优化。
7. 有 ready 快显性能优化目标时，如果某个动态 MVT 瓦片仍超时，前端按既有 TTL 允许后续重试，并提示生成瓦片缓存；此类超时通常意味着单瓦片内容过重、低层级面过复杂或数据库临时压力过高。
