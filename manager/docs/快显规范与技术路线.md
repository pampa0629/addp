...
2. 空间预览中，当动态 MVT 处于 `source_transform_path` 或瓦片返回 `X-ADDP-Tile-Recommendation=quick_view_optimization` / `X-ADDP-Tile-Retry-Policy=suppress_tile` 时，展示“执行快显优化”入口；当瓦片返回 `X-ADDP-Tile-Recommendation=tile_cache_generation` 时，使用节流消息提示生成瓦片缓存，不在预览区域重复展示第二个提示块。
...
5. 有可索引 3857 目标后，动态 MVT 标记为 `performance_mode=ready_3857_target`，仍应保留单瓦片超时、体积和并发保护。
...
9. 第一阶段不为低层级重瓦片单独提供“多尺度快显优化”入口。该类事实只作为后续多尺度快显优化专题的触发条件记录；当前用户闭环统一引导到瓦片缓存生成。
