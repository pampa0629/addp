...
- **向量化平台配置**：记录 Manager owner 的平台普通运行配置及单调版本，不保存 API Key
...
| `embedding_configuration` | 平台向量化普通运行配置单例 | 1 |
| `preview_state` | 预览模式偏好和预览交互状态 | 100-1000 |
| `task_definitions` | 快显管理与空间任务的统一派生任务定义 | 100-10000 |
| `task_resource_bindings` | 派生任务对源/目标资源的规范化绑定 | 100-20000 |
...
erDiagram
    ...
    EMBEDDING_CONFIGURATION ||..o{ EMBEDDING_TASKS : "创建或更新时快照"
    PREVIEW_STATE }o..o{ VECTOR_TILE_CACHE : "同一item指纹"
    ...
    PREVIEW_STATE }o..o{ MODEL3D_TILES : "同一item指纹"
    PREVIEW_STATE }o..o{ GAUSSIAN_SPLAT_KSPLAT : "同一item指纹"
    PREVIEW_STATE }o..o{ POINT_CLOUD_COPC : "同一item指纹"
    PREVIEW_STATE }o..o{ PPTX_PDF : "同一item指纹"
    TASK_DEFINITIONS ||--o{ TASK_RESOURCE_BINDINGS : "绑定源与目标"
    TASK_DEFINITIONS ||--o{ VECTOR_MATERIALIZED_VIEW : "创建或刷新"
    TASK_DEFINITIONS ||--o{ RASTER_COG : "生成"
    TASK_DEFINITIONS ||--o{ MODEL_3D_GLB : "生成"
    TASK_DEFINITIONS ||--o{ MODEL3D_TILES : "生成"
    TASK_DEFINITIONS ||--o{ GAUSSIAN_SPLAT_KSPLAT : "生成"
    TASK_DEFINITIONS ||--o{ POINT_CLOUD_COPC : "生成"
    TASK_DEFINITIONS ||--o{ PPTX_PDF : "生成"
    TASK_DEFINITIONS ||--o{ VECTOR_TILE_CACHE : "生成或刷新"
...
- `vector_materialized_view.task_id` → `task_definitions.id` (软引用，类型为 `vector_materialized_view_generation`)
  - 指向产生或最近刷新该矢量物化视图结果的任务

- `vector_tile_cache.task_id` → `task_definitions.id` (软引用，类型为 `vector_tile_cache_generation`)
  - 指向产生或最近刷新该产物的瓦片缓存任务

- `model_3d_glb.task_id` → `task_definitions.id` (软引用，类型为 `model_3d_glb_generation`)
  - 指向产生或最近刷新该 GLB 快显 artifact 的任务

- `gaussian_splat_ksplat.task_id` → `task_definitions.id` (软引用，类型为 `gaussian_splat_ksplat_generation`)
  - 指向产生或最近刷新该 KSplat 快显 artifact 的任务

- `point_cloud_copc.task_id` → `task_definitions.id` (软引用，类型为 `point_cloud_copc_generation`)
...
- `export_sessions` 是 Manager 用户导出会话表，不是 Manager 任务定义表：
  ...
  - `transfer_execution_id` 只关联 Manager 调用 Transfer 创建的一次性 `sync` execution，不创建 Transfer 任务定义。
...
3. 若 can_use_quick_view=false 且 can_generate_vector_tile_cache=true
   ...
   ↓
4. 用户按引导创建 `manager.task_definitions` 中对应类型的任务
...
6. 更新状态
   ...
   └─ `task_definitions.last_execution_id / last_execution_status`
```
...
- 生成配置属于 `task_definitions.config` 和 execution `execution_config` / `metadata`，不作为瓦片缓存结果身份分叉
...
**task_definitions 表**：
- `idx_manager_task_definitions_tenant_type`（tenant_id, task_type）- 统一任务分类与类型查询
- `idx_manager_task_definitions_tenant_binding_status`（tenant_id, binding_status）- 按租户筛选有效或失效绑定任务
- `idx_manager_task_definitions_last_execution`（last_execution_id）- 最近执行回溯
- `idx_manager_task_definitions_deleted_at`（deleted_at）- 软删除过滤
- `uq_manager_task_definition_semantic`（tenant_id, task_type, semantic_key）- 活跃任务语义身份唯一约束

**task_resource_bindings 表**：
- `idx_manager_task_resource_task`（task_definition_id）- 按任务加载全部源/目标绑定
- `idx_manager_task_resource_engine`（tenant_id, engine_id）- 引擎生命周期和资源回收查询
- `uq_manager_task_resource_binding`（task_definition_id, role, ordinal）- 单任务角色序号唯一
...
**export_sessions 表**：
- `idx_export_sessions_scope_status_created`（tenant_id, user_id, status, created_at）- 按租户与发起用户隔离查询，并支持 cleanup。
- `transfer_execution_id` 建索引，用于 Transfer execution 回溯。
...
Manager execution 的跨模块血缘事实统一写入 `common.task_executions.metadata.lineage_facts`，schema 固定为 `addp.lineage-facts/v1`。输入身份来自 execution 已冻结的 ResourceLocator、item ID 和 fingerprint；业务输出写目标 ResourceLocator，Manager 私有快显 artifact 写 `addp-infra://` Locator；后续 Meta scan execution ID 写入 `meta_scan_refs`。该事实不进入 Manager 私有结果表，也不由 Monitor 或 Meta 反查 Manager 配置补齐。清理 execution 不表达数据派生；通过 Transfer 执行的导入导出由 Transfer 作为唯一血缘 owner。
...
- [task_definitions表](tables/task_definitions表.md) - Manager 统一派生任务定义表
- [task_resource_bindings表](tables/task_resource_bindings表.md) - 派生任务资源绑定表
