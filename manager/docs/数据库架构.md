...
Manager 模块使用 PostgreSQL 的 `manager` schema，负责检索历史、向量化、空间快显和瓦片缓存相关状态。资源树、node 和 item 事实归 Meta 管理，Manager 通过 ResourceLocator 与 Meta facts 做只读探查和预览。
...
| `quick_view` | 快显偏好和预览交互状态 | 100-1000 |
| `vector_quick_view_target_generation` | 快显性能优化结果状态 | 100-10000 |
| `vector_quick_view_target_tasks` | 快显性能优化任务定义 | 100-1000 |
...
erDiagram
    ...
    QUICK_VIEW }o..o{ RASTER_COG : "同一item指纹"
...
- `quick_view.item_fingerprint` ↔ `vector_tile_cache.item_fingerprint` (逻辑关联)
  - `quick_view` 只保存用户偏好和预览交互状态，不保存推荐结果；默认快显结果由状态查询时从 `vector_tile_cache` 动态选择

- `quick_view.item_fingerprint` ↔ `vector_quick_view_target_generation.item_fingerprint` (逻辑关联)
  - `vector_quick_view_target_generation` 只登记 Manager 创建并拥有生命周期的快显优化目标
  - 自动识别的外部 3857 物化视图不写入该表，不出现在结果列表中，也不由 Manager cleanup 删除
  - 后续如支持人工登记外部 3857 目标，应使用独立 artifact state，不复用 `vector_quick_view_target_generation` 表

- `vector_quick_view_target_generation.task_id` → `vector_quick_view_target_tasks.id` (软引用)
...
3. 若 can_use_quick_view=false 且 can_generate_vector_tile_cache=true
   ...
   ↓
4. 用户按引导创建 manager.vector_quick_view_target_tasks 或 manager.vector_tile_cache_tasks
   └─ 快显性能优化任务保存目标、几何和属性策略；瓦片缓存任务保存 target / tile / storage / options
   ↓
5. 执行 vector_quick_view_target_generation 或 vector_tile_cache_generation
   ├─ 写入 common.task_executions
   ├─ vector_quick_view_target_generation 创建或刷新 manager.vector_quick_view_targets 和 3857 快显性能优化目标
   └─ vector_tile_cache_generation 按 tenant_id + item_fingerprint + tile_format 创建或刷新 vector_tile_cache，并写入瓦片对象和 manifest
   ↓
6. 更新状态
   ├─ vector_quick_view_target_generation.status 或 vector_tile_cache.status=ready/failed/cancelled
...
**设计选择**：当前同一 `item_fingerprint + tile_format` 只保留一份当前瓦片缓存结果，瓦片缓存结果通过 `storage_ref` 定位存储位置。
...
- 无可索引 3857 目标且动态 MVT 超时时，引导执行快显性能优化。
...
**quick_view 表**：
- `idx_quick_view_tenant_fingerprint`（tenant_id, item_fingerprint）- item 预览偏好和交互状态唯一身份

**vector_quick_view_target_generation 表**：
...
**vector_quick_view_target_tasks 表**：
- `idx_qvo_tasks_tenant`（tenant_id）- 租户任务查询
- `idx_qvo_tasks_schedule`（enabled, next_run_at）- 任务体系字段索引；当前 `supports_schedule=false`，不启动 Manager 自身定时调度
...
- `idx_vector_tile_cache_tenant_fingerprint_format_unique`（tenant_id, item_fingerprint, tile_format）- 当前结果唯一
...
- `idx_vector_tile_cache_tasks_schedule`（enabled, next_run_at）- 调度扫描
...
2. 删除快显性能优化结果只删除 Manager 创建并登记的 3857 目标，不删除源表或外部只读目标。
3. 删除瓦片缓存结果只删除 Manager 管理的瓦片对象和 `manager.vector_tile_cache` 结果，不删除快显优化目标。
...
- [vector_quick_view_targets表](tables/vector_quick_view_targets表.md) - 快显性能优化结果状态表
- [vector_quick_view_target_tasks表](tables/vector_quick_view_target_tasks表.md) - 快显性能优化任务定义表
- [快显概念说明](快显概念说明.md) - 快显、快显性能优化和瓦片缓存概念边界
- [快显实现规范](快显实现规范.md) - 快显性能优化任务、结果、外部 3857 目标、失效规则和验证记录
