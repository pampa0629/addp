...
- **瓦片缓存结果**：记录瓦片缓存的存储引用、格式、范围、层级、配置指纹和状态
...
2. 若 can_use_quick_view=true
   └─ 预览页展示“切换快显”；render_source=realtime_tile 时可同时展示“生成瓦片缓存”
   ↓
3. 若 can_use_quick_view=false 且 can_generate_tile_cache=true
   └─ 预览页只展示“生成瓦片缓存”，跳转瓦片缓存页面的“任务”tab
   ↓
4. 用户创建 manager.tile_cache_tasks
   └─ config 保存 target / tile / storage / preparation / options
   ↓
5. 执行 tile_cache_generation
   ├─ 写入 common.task_executions
   ├─ 按 tenant_id + item_fingerprint + tile_format + config_hash 创建或刷新 tile_cache
   ├─ 必要且授权时执行准备动作
   └─ 写入瓦片对象和 manifest
   ↓
6. 更新状态
   ├─ tile_cache.status=ready/failed/cancelled
   └─ tile_cache_tasks.last_execution_id / last_execution_status
```
...
**设计选择**：瓦片缓存结果通过 `storage_ref` 定位存储位置，通过 `config_hash` 保证同一生成配置的幂等刷新。

**用途**：
- `storage_ref` 指向瓦片缓存或 manifest，不要求上层硬编码 bucket、prefix 或对象路径；对象前缀包含 `item_fingerprint/config_hash`
- `config_hash` 表达规范化生成配置指纹，只包含瓦片矩阵、层级、SRID、范围、几何列和主键等生成参数，不包含源数据身份、瓦片格式、`locator`、`item_id`、任务名称和调度配置

**好处**：
- 支持用户指定存储位置
- 支持未来 MVT 之外的瓦片格式
- 降低 Manager 上层逻辑对 MinIO 路径规则的耦合
...
- `idx_tile_cache_tenant_fingerprint_config_unique`（tenant_id, item_fingerprint, tile_format, config_hash）- 同配置结果幂等
