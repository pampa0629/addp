...
type Model3DQuickViewHandler struct {
	repo          *repository.Model3DQuickViewRepository
	minioClient   *minio.Client
	defaultBucket string
}

func NewModel3DQuickViewHandler(repo *repository.Model3DQuickViewRepository, minioClient *minio.Client, defaultBucket string) *Model3DQuickViewHandler {
	return &Model3DQuickViewHandler{repo: repo, minioClient: minioClient, defaultBucket: defaultBucket}
}

// GetModel3DQuickViewContent 返回 Manager 三维模型 GLB 快显内容。
...
// @Param id path int true "model 3D quick view ID"
...
// @Router /model_3d_quick_view/{id}/content [get]
// @Security BearerAuth
func (h *Model3DQuickViewHandler) GetModel3DQuickViewContent(c *gin.Context) {
	if h == nil || h.repo == nil || h.minioClient == nil {
		commonAPI.InternalServerError(c, "model 3d quick view service is not available")
		return
	}
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil || id == 0 {
		commonAPI.BadRequestError(c, "invalid model 3d quick view id")
		return
	}
	tenantID := uint(1)
	...
	if result == nil || result.Status != models.Model3DQuickViewStatusReady {
		commonAPI.ErrorResponse(c, http.StatusNotFound, "model 3d quick view is not ready")
		return
	}
	...
	if err != nil {
		commonAPI.ErrorResponse(c, http.StatusNotFound, "model 3d quick view object not found")
		return
	}
	...
	if rangeHeader != "" {
		...
		if err != nil {
			if errors.Is(err, errModel3DQuickViewInvalidRange) {
	...
	if _, err := io.Copy(c.Writer, obj); err != nil {
		logger.L().Error("model 3d quick view stream failed", "error", err, "result_id", result.ID)
	}
}

var errModel3DQuickViewInvalidRange = errors.New("invalid range")

func parseHTTPRange(header string, size int64) (int64, int64, error) {
	if size <= 0 {
		return 0, 0, errModel3DQuickViewInvalidRange
	}
