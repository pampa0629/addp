...
func TestParseKPlatHeader(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("parseKPlatHeader() error = %v", err)
	}
...
func TestInspectGaussianSplatQuickViewResultSummarizesHealthyKPlat(t *testing.T) {
	headerBytes := make([]byte, ksplatHeaderSizeBytes)
	headerBytes[0] = 0
	headerBytes[1] = 1
	binary.LittleEndian.PutUint32(headerBytes[4:8], 1)
	...
	response := inspectGaussianSplatQuickViewResult(result, "manager", "model.ksplat", minio.ObjectInfo{
