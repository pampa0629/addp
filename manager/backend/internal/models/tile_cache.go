...
type TileCacheTask struct {
	...
	TenantID uint `gorm:"not null;index:idx_vector_tile_cache_tasks_tenant" json:"tenant_id"`
...
func (TileCacheTask) TableName() string {
	return "manager.vector_tile_cache_tasks"
}
...
type TileCache struct {
	ID              uint   `gorm:"primaryKey" json:"id"`
	TenantID        uint   `gorm:"not null;index:idx_vector_tile_cache_tenant_item_fingerprint,priority:1;index:idx_vector_tile_cache_tenant_item,priority:1" json:"tenant_id"`
	ItemFingerprint string `gorm:"size:64;not null;index:idx_vector_tile_cache_tenant_item_fingerprint,priority:2" json:"item_fingerprint"`
	ItemID          *uint  `gorm:"index:idx_vector_tile_cache_tenant_item,priority:2" json:"item_id,omitempty"`
...
func (TileCache) TableName() string {
	return "manager.vector_tile_cache"
}
