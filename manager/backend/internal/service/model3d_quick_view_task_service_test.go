...
func TestModel3DQuickViewTaskNormalizesSingleOSGBConfig(t *testing.T) {
...
func TestModel3DQuickViewTaskCreateReusesExistingFingerprintTask(t *testing.T) {
	db := newModel3DQuickViewTaskServiceTestDB(t)
	taskSvc := NewModel3DQuickViewTaskService(repository.NewModel3DQuickViewRepository(db), nil)

	first := &models.Model3DQuickViewTask{
		TenantID: 7,
		Name:     "生成 GLB 快显",
		Enabled:  true,
		Config:   model3DQuickViewTaskTestConfig("fp-osgb", "tile.osgb"),
	}
	if err := taskSvc.Create(context.Background(), first); err != nil {
		t.Fatalf("create first model 3d quick view task: %v", err)
	}
	if first.ID == 0 {
		t.Fatal("first task ID is zero")
	}

	second := &models.Model3DQuickViewTask{
		TenantID:    7,
		Name:        "重新生成 GLB 快显",
	...
	if err := taskSvc.Create(context.Background(), second); err != nil {
		t.Fatalf("create duplicate model 3d quick view task: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("list model 3d quick view tasks: %v", err)
	}
...
func model3DQuickViewTaskTestConfig(fingerprint string, fileName string) commonModels.JSONMap {
...
func newModel3DQuickViewTaskServiceTestDB(t *testing.T) *gorm.DB {
	t.Helper()
