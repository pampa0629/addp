...
type gaussianSplatQuickViewObjectStore interface {
...
type ManagerGaussianSplatQuickViewExecutor struct {
	systemClient    *commonClient.SystemClient
	workflowEngines workflowEngineLister
	objectStore     gaussianSplatQuickViewObjectStore
...
func NewManagerGaussianSplatQuickViewExecutor(
	systemClient *commonClient.SystemClient,
	workflowEngines workflowEngineLister,
	objectStore gaussianSplatQuickViewObjectStore,
	minioEndpoint string,
	...
	defaultBucket string,
	invokeTimeout time.Duration,
) *ManagerGaussianSplatQuickViewExecutor {
	if invokeTimeout <= 0 {
		invokeTimeout = 30 * time.Minute
	}
	return &ManagerGaussianSplatQuickViewExecutor{
...
func (e *ManagerGaussianSplatQuickViewExecutor) BuildGaussianSplatQuickView(ctx context.Context, req GaussianSplatQuickViewExecutionRequest) (*GaussianSplatQuickViewExecutionResult, error) {
	if e == nil || e.systemClient == nil || e.workflowEngines == nil || e.objectStore == nil {
		return nil, errors.New("gaussian splat quick view generation executor is not fully configured")
	}
	if req.Task == nil {
		return nil, errors.New("gaussian splat quick view generation task is required")
	}
	...
	result := &GaussianSplatQuickViewExecutionResult{
...
func (e *ManagerGaussianSplatQuickViewExecutor) prepareSourcePath(ctx context.Context, tenantID uint, source GaussianSplatQuickViewSourceConfig) (string, commonModels.JSONMap, error) {
	...
	if err != nil {
		return "", nil, fmt.Errorf("parse gaussian splat quick view source locator: %w", err)
	}
	...
	if err != nil {
		return "", nil, fmt.Errorf("prepare gaussian splat quick view source path: %w", err)
	}
	return sourcePath, access, nil
}

func (e *ManagerGaussianSplatQuickViewExecutor) ensureTargetBucket(ctx context.Context, bucket string) error {
	exists, err := e.objectStore.BucketExists(ctx, bucket)
	if err != nil {
		return fmt.Errorf("check gaussian splat quick view bucket: %w", err)
	}
	if exists {
		return nil
	}
	if err := e.objectStore.MakeBucket(ctx, bucket, minio.MakeBucketOptions{}); err != nil {
		return fmt.Errorf("create gaussian splat quick view bucket: %w", err)
	}
	return nil
}

func (e *ManagerGaussianSplatQuickViewExecutor) selectDirectWorkflowRuntime(ctx context.Context, tenantID uint, operatorName string) (commonModels.Engine, commonModels.OperatorDescriptor, error) {
	...
	if err != nil {
		return commonModels.Engine{}, commonModels.OperatorDescriptor{}, fmt.Errorf("direct workflow operator %s is unavailable for gaussian splat quick view generation: %w", operatorName, err)
	}
