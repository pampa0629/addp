...
func TestManagerVectorTileCacheWorkflowExecutorUsesVSIS3ForObjectStoreSource(t *testing.T) {
	...
	systemServer := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"id":9,"tenant_id":7,"name":"Source S3","engine_type":"s3","connection_info":{"endpoint":"http://localhost:9002","access_key":"source-ak","secret_key":"source-sk","use_ssl":false},"is_active":true}`))
		}),
	}
