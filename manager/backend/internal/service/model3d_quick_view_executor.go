...
type model3DQuickViewObjectStore interface {
...
type ManagerModel3DQuickViewExecutor struct {
	systemClient    *commonClient.SystemClient
	workflowEngines workflowEngineLister
	objectStore     model3DQuickViewObjectStore
...
func NewManagerModel3DQuickViewExecutor(
	systemClient *commonClient.SystemClient,
	workflowEngines workflowEngineLister,
	objectStore model3DQuickViewObjectStore,
	minioEndpoint string,
	...
	defaultBucket string,
	invokeTimeout time.Duration,
) *ManagerModel3DQuickViewExecutor {
	if invokeTimeout <= 0 {
		invokeTimeout = 30 * time.Minute
	}
	return &ManagerModel3DQuickViewExecutor{
...
func (e *ManagerModel3DQuickViewExecutor) BuildModel3DQuickView(ctx context.Context, req Model3DQuickViewExecutionRequest) (*Model3DQuickViewExecutionResult, error) {
	if e == nil || e.systemClient == nil || e.workflowEngines == nil || e.objectStore == nil {
		return nil, errors.New("model 3d quick view generation executor is not fully configured")
	}
	if req.Task == nil {
		return nil, errors.New("model 3d quick view generation task is required")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("stat model 3d quick view GLB: %w", err)
	}

	facts := operatorInvokeJSONFacts(invokeResult)
	result := &Model3DQuickViewExecutionResult{
...
func (e *ManagerModel3DQuickViewExecutor) prepareSourcePath(ctx context.Context, tenantID uint, source Model3DQuickViewSourceConfig) (string, commonModels.JSONMap, error) {
	loc, err := resourcetree.ParseURI(source.ItemLocator)
	if err != nil {
		return "", nil, fmt.Errorf("parse model 3d quick view source locator: %w", err)
	}
...
func (e *ManagerModel3DQuickViewExecutor) ensureTargetBucket(ctx context.Context, bucket string) error {
	...
	if err != nil {
		return fmt.Errorf("check model 3d quick view bucket: %w", err)
	}
	if exists {
		return nil
	}
	if err := e.objectStore.MakeBucket(ctx, bucket, minio.MakeBucketOptions{}); err != nil {
		return fmt.Errorf("create model 3d quick view bucket: %w", err)
	}
	return nil
}

func (e *ManagerModel3DQuickViewExecutor) selectDirectWorkflowRuntime(ctx context.Context, tenantID uint, operatorName string) (commonModels.Engine, commonModels.OperatorDescriptor, error) {
	...
	if err != nil {
		return commonModels.Engine{}, commonModels.OperatorDescriptor{}, fmt.Errorf("direct workflow operator %s is unavailable for model 3d quick view generation: %w", operatorName, err)
	}
