...
func TestGaussianSplatQuickViewTaskNormalizesSupportedSources(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(cfg.Result.StorageRef, "tenant_7/gaussian-splat-quick-view/"+tc.fingerprint+"/"+tc.wantFile) {
				t.Fatalf("storage_ref = %q, want tenant-scoped KPlat artifact", cfg.Result.StorageRef)
			}
...
func TestGaussianSplatQuickViewTaskEnrichesBoundsFromMetaItem(t *testing.T) {
	...
	svc := NewGaussianSplatQuickViewTaskService(nil, nil)
	svc.SetMetaClient(commonClient.NewMetaClientWithInternalKey(metaServer.URL, "internal-key"))

	if err := svc.enrichGaussianSplatQuickViewTaskSourceFacts(context.Background(), task); err != nil {
		t.Fatalf("enrich gaussian splat quick view source facts: %v", err)
	}
	cfg, err := normalizeGaussianSplatQuickViewTaskConfig(task.Config, "manager", 7)
	if err != nil {
		t.Fatalf("normalize gaussian splat quick view config: %v", err)
	}
