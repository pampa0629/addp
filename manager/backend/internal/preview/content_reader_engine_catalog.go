...
type objectCatalogContentReader struct {
	...
	catalog       plugin.EngineCatalogProvider
	connInfo      plugin.ConnectionInfo
	engineID      uint
	bucket        string
}

func newObjectCatalogContentReader(contentReader plugin.ContentReadableProvider, catalog plugin.EngineCatalogProvider, connInfo plugin.ConnectionInfo, engineID uint, bucket string) *objectCatalogContentReader {
...
func (r *objectCatalogContentReader) Open(ctx context.Context, ref contentio.Ref) (io.ReadCloser, error) {
	return resourceutil.OpenObjectContent(ctx, r.contentReader, r.connInfo, r.engineID, r.bucket, r.objectKey(ref.Path))
}
...
func (r *objectCatalogContentReader) List(ctx context.Context, scope contentio.Ref) ([]contentio.Ref, error) {
	...
	for _, node := range nodes {
		path := strings.TrimPrefix(resourceutil.NodePhysicalPath(node), strings.Trim(r.bucket, "/")+"/")
		if path == "" {
...
func newFileCatalogContentReader(contentReader plugin.ContentReadableProvider, catalog plugin.EngineCatalogProvider, connInfo plugin.ConnectionInfo, engineID uint) *fileCatalogContentReader {
