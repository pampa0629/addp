...
func TestDropLegacyQuickViewTablesRemovesLegacyExecutionsAndTables(t *testing.T) {
	...
	if err := db.Exec(`
		INSERT INTO common.task_executions (module, task_type) VALUES
			...
			('manager', 'model_3d_tiles_generation'),
...
func TestDropLegacyCADPreviewTablesRemovesLegacyExecutionsAndTables(t *testing.T) {
	db, err := gorm.Open(sqlite.Open("file:drop_legacy_cad_preview?mode=memory&cache=shared"), &gorm.Config{})
	if err != nil {
		t.Fatalf("open sqlite: %v", err)
	}
	if err := db.Exec(`ATTACH DATABASE ':memory:' AS common`).Error; err != nil {
	...
	if err := db.Exec(`CREATE TABLE common.task_executions (id INTEGER PRIMARY KEY, module TEXT NOT NULL, task_type TEXT NOT NULL)`).Error; err != nil {
		t.Fatalf("create task_executions: %v", err)
	}
	for _, tableName := range []string{"cad_previews", "cad_preview_tasks"} {
		if err := db.Exec(`CREATE TABLE manager."` + tableName + `" (id INTEGER PRIMARY KEY)`).Error; err != nil {
			t.Fatalf("create legacy table %s: %v", tableName, err)
		}
	}
	if err := db.Exec(`
		INSERT INTO common.task_executions (module, task_type) VALUES
			('manager', 'cad_preview_generation'),
			('manager', 'data_profiling'),
			('monitor', 'cad_preview_generation')
	`).Error; err != nil {
		t.Fatalf("insert executions: %v", err)
	}

	if err := dropLegacyCADPreviewTables(db); err != nil {
		t.Fatalf("dropLegacyCADPreviewTables returned error: %v", err)
	}

	var legacyExecutions int64
	if err := db.Raw(`SELECT COUNT(*) FROM common.task_executions WHERE module = 'manager' AND task_type = 'cad_preview_generation'`).Scan(&legacyExecutions).Error; err != nil {
		t.Fatalf("count legacy executions: %v", err)
	}
	if legacyExecutions != 0 {
		t.Fatalf("legacy CAD preview executions = %d, want 0", legacyExecutions)
	}
	...
	for _, tableName := range []string{"cad_previews", "cad_preview_tasks"} {
		var tableCount int64
		if err := db.Raw(`SELECT COUNT(*) FROM manager.sqlite_master WHERE type = 'table' AND name = ?`, tableName).Scan(&tableCount).Error; err != nil {
			t.Fatalf("count legacy table %s: %v", tableName, err)
		}
		if tableCount != 0 {
			t.Fatalf("legacy table %s still exists", tableName)
