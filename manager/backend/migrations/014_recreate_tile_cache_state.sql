-- 014_recreate_tile_cache_state.sql
...
CREATE TABLE manager.tile_cache_tasks (
...
CREATE INDEX idx_tile_cache_tasks_tenant ON manager.tile_cache_tasks (tenant_id);
CREATE INDEX idx_tile_cache_tasks_schedule ON manager.tile_cache_tasks (enabled, next_run_at);
CREATE INDEX idx_tile_cache_tasks_last_execution ON manager.tile_cache_tasks (last_execution_id);
CREATE INDEX idx_tile_cache_tasks_deleted_at ON manager.tile_cache_tasks (deleted_at);
...
CREATE TABLE manager.quick_view (
    ...
    item_fingerprint            VARCHAR(64) NOT NULL,
    locator                     TEXT,
    preferred_mode              VARCHAR(32) NOT NULL DEFAULT 'table_geojson',
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
COMMENT ON TABLE manager.tile_cache_tasks IS '瓦片缓存生成任务定义表';
COMMENT ON TABLE manager.tile_cache IS '瓦片缓存结果状态表';
