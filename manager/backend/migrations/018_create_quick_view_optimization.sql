-- 018_create_quick_view_optimization.sql
-- Manager quick view optimization task definitions and result state.

CREATE TABLE IF NOT EXISTS manager.quick_view_optimization_tasks (
...
CREATE INDEX IF NOT EXISTS idx_qvo_tasks_tenant
    ON manager.quick_view_optimization_tasks (tenant_id);
CREATE INDEX IF NOT EXISTS idx_qvo_tasks_schedule
    ON manager.quick_view_optimization_tasks (enabled, next_run_at);
CREATE INDEX IF NOT EXISTS idx_qvo_tasks_last_execution
    ON manager.quick_view_optimization_tasks (last_execution_id);
CREATE INDEX IF NOT EXISTS idx_qvo_tasks_deleted_at
    ON manager.quick_view_optimization_tasks (deleted_at);

CREATE TABLE IF NOT EXISTS manager.quick_view_optimization (
    id                              BIGSERIAL PRIMARY KEY,
    tenant_id                       BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_qvo_tenant_item_fingerprint
    ON manager.quick_view_optimization (tenant_id, item_fingerprint);
CREATE INDEX IF NOT EXISTS idx_qvo_tenant_item
    ON manager.quick_view_optimization (tenant_id, item_id);
CREATE INDEX IF NOT EXISTS idx_qvo_task
    ON manager.quick_view_optimization (task_id);
CREATE INDEX IF NOT EXISTS idx_qvo_execution
    ON manager.quick_view_optimization (last_execution_id);
CREATE INDEX IF NOT EXISTS idx_qvo_status
    ON manager.quick_view_optimization (status);
CREATE INDEX IF NOT EXISTS idx_qvo_deleted_at
    ON manager.quick_view_optimization (deleted_at);
...
COMMENT ON TABLE manager.quick_view_optimization_tasks IS '快显性能优化任务定义表';
COMMENT ON TABLE manager.quick_view_optimization IS '快显性能优化结果状态表';
