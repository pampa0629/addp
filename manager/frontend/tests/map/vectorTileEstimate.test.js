...
describe('vectorTileEstimate', () => {
  ...
  it('opens the same create form from the data preview action with its locator', () => {
    ...
    expect(vectorTileSetView).toContain('openCreate(route.query.locator)')
  })

  it('opens TaskProvider create and edit deep links in the business tile set form', () => {
    expect(vectorTileSetView).toContain("const taskID = Number(route.query.task_id || 0)")
    expect(vectorTileSetView).toContain('openEdit(await quickViewAPI.getVectorTileSetTask(taskID))')
    expect(vectorTileSetView).toContain("route.query.create === '1'")
