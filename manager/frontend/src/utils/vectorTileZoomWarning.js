...
export function shouldWarnVectorTileMaxZoom(renderSource) {
  return isCachedTileRenderSource(renderSource) || String(renderSource || '').trim() === 'business_pmtiles'
}
...
export function vectorTileSourceMaxZoom(renderSource, configuredMaxZoom, fallbackMaxZoom = 22) {
  ...
  if (shouldWarnVectorTileMaxZoom(renderSource) && Number.isFinite(maxZoom) && maxZoom > 0) {
    return maxZoom
  }
