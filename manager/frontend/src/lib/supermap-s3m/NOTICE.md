...
`S3ModelOldParser.js` supports externally supplied legacy XML SCP + `.s3m`
datasets. The current SuperMap iObjects C++ builder emits JSON SCP + `.s3mb`.
The remaining parser and renderer sources implement the shared S3M tile
pipeline. Cesium is loaded as an independent Apache-2.0 dependency only when
the S3M preview component is mounted.
