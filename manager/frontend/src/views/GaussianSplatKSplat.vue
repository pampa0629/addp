<template>
  <div class="gaussian-splat-ksplat">
    <el-card>
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane :label="t('manager.gaussianSplatKSplat.tasksTab')" name="tasks">
          <div class="tab-toolbar">
            <div class="toolbar-tip">
              <span class="toolbar-tip-text">{{ t('manager.gaussianSplatKSplat.subtitle') }}</span>
              <el-tooltip
                :content="t('manager.gaussianSplatKSplat.workflowDescription')"
                placement="bottom"
                :show-after="300"
              >
          ...
          <el-table :data="tasks" v-loading="tasksLoading" stripe>
            ...
            <el-table-column :label="t('manager.gaussianSplatKSplat.sourceSize')" width="120">
              <template #default="{ row }">{{ formatBytes(source(row).source_size_bytes) }}</template>
            </el-table-column>
            <el-table-column :label="t('manager.gaussianSplatKSplat.enabled')" width="90">
              <template #default="{ row }">
                <el-tag :type="row.enabled ? 'success' : 'info'">
                  {{ row.enabled ? t('manager.gaussianSplatKSplat.enabledYes') : t('manager.gaussianSplatKSplat.enabledNo') }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column :label="t('manager.gaussianSplatKSplat.lastExecutionStatus')" width="130">
              <template #default="{ row }">
            ...
            <el-table-column :label="t('manager.gaussianSplatKSplat.lastRunAt')" width="170">
              <template #default="{ row }">{{ formatDateTime(row.last_run_at) }}</template>
            </el-table-column>
            <el-table-column :label="t('manager.gaussianSplatKSplat.actions')" width="360" fixed="right">
              <template #default="{ row }">
                <div class="row-actions">
                  ...
                  <el-button size="small" type="danger" @click="deleteTask(row)">{{ t('manager.gaussianSplatKSplat.delete') }}</el-button>
                </div>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-tab-pane :label="t('manager.gaussianSplatKSplat.resultsTab')" name="results">
          <div class="filter-bar">
            <el-tag v-if="resultTaskFilterLabel" type="primary" closable @close="clearResultTaskFilter">
              {{ resultTaskFilterLabel }}
            </el-tag>
            <el-select v-model="resultFilters.status" class="status-filter" clearable :placeholder="t('manager.gaussianSplatKSplat.resultStatus')">
            ...
            <el-input
              ...
              clearable
              :placeholder="t('manager.gaussianSplatKSplat.keywordPlaceholder')"
              @keyup.enter="applyResultFilters"
            />
            <el-button type="primary" @click="applyResultFilters">{{ t('manager.gaussianSplatKSplat.search') }}</el-button>
            <el-button @click="resetResultFilters">{{ t('manager.gaussianSplatKSplat.reset') }}</el-button>
          ...
          <el-table :data="results" v-loading="resultsLoading" stripe>
            ...
            <el-table-column :label="t('manager.gaussianSplatKSplat.diagnostics')" width="170">
              <template #default="{ row }">
            ...
            <el-table-column :label="t('manager.gaussianSplatKSplat.size')" width="120">
              <template #default="{ row }">{{ formatBytes(row.size_bytes) }}</template>
            </el-table-column>
            <el-table-column prop="error_message" :label="t('manager.gaussianSplatKSplat.error')" min-width="180" show-overflow-tooltip />
            <el-table-column :label="t('manager.gaussianSplatKSplat.updatedAt')" width="170">
              ...
              <template #default="{ row }">
                <div class="row-actions">
                  <el-button size="small" :disabled="row.status !== 'ready' || !row.locator" @click="openSourcePreview(row)">
                    {{ t('manager.gaussianSplatKSplat.previewKSplat') }}
                  </el-button>
                  <el-button size="small" :disabled="!hasDiagnostics(row)" @click="openDiagnostics(row)">
                    {{ t('manager.gaussianSplatKSplat.diagnostics') }}
                  </el-button>
                  <el-button size="small" :disabled="!row.last_execution_id" @click="openResultExecution(row)">
                    {{ t('manager.gaussianSplatKSplat.monitor') }}
                  </el-button>
                  <el-button size="small" type="danger" @click="deleteResult(row)">{{ t('manager.gaussianSplatKSplat.delete') }}</el-button>
                </div>
              </template>
            </el-table-column>
          </el-table>
    ...
    >
      <el-form label-position="top" :model="form">
        <div class="form-grid">
          <el-form-item :label="t('manager.gaussianSplatKSplat.name')">
            <el-input v-model="form.name" :placeholder="t('manager.gaussianSplatKSplat.namePlaceholder')" />
          </el-form-item>
          <el-form-item :label="t('manager.gaussianSplatKSplat.enabled')">
            <el-switch
              v-model="form.enabled"
              :active-text="t('manager.gaussianSplatKSplat.enabledYes')"
              :inactive-text="t('manager.gaussianSplatKSplat.enabledNo')"
            />
          </el-form-item>
        </div>

        <el-divider content-position="left">{{ t('manager.gaussianSplatKSplat.sourceScope') }}</el-divider>
        <ResourceTreePicker
          v-model="sourceSelection"
          mode="item"
          :initial-locator="sourceInitialLocator"
          :title="t('manager.gaussianSplatKSplat.sourceTreeTitle')"
          :engine-label="t('manager.gaussianSplatKSplat.engine')"
          :engine-placeholder="t('manager.gaussianSplatKSplat.enginePlaceholder')"
          :search-placeholder="t('manager.gaussianSplatKSplat.searchPlaceholder')"
          :search-all-engines-placeholder="t('manager.gaussianSplatKSplat.searchAllEnginesPlaceholder')"
          :search-empty-text="t('manager.gaussianSplatKSplat.searchEmptyText')"
          tree-height="300px"
        />
        ...
        <el-descriptions class="source-facts" :column="2" border size="small">
          ...
          <el-descriptions-item :label="t('manager.gaussianSplatKSplat.sourceFormat')">
            {{ formatLabel(form.source.format) }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('manager.gaussianSplatKSplat.sourceSize')">
            {{ formatBytes(form.source.source_size_bytes) }}
          </el-descriptions-item>
        </el-descriptions>

        <el-form-item :label="t('manager.gaussianSplatKSplat.fileName')">
          <el-input v-model="form.result.file_name" :placeholder="t('manager.gaussianSplatKSplat.fileNamePlaceholder')" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="taskDialogVisible = false">{{ t('manager.gaussianSplatKSplat.cancel') }}</el-button>
        <el-button type="primary" :loading="saving" @click="saveTask">{{ t('manager.gaussianSplatKSplat.save') }}</el-button>
      </template>
    </el-dialog>

    <el-dialog
      v-model="diagnosticsDialogVisible"
      :title="t('manager.gaussianSplatKSplat.diagnosticsTitle')"
      width="760px"
      destroy-on-close
    >
      <template v-if="selectedDiagnosticResult">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item :label="t('manager.gaussianSplatKSplat.workflowOperator')">
            {{ diagnosticValue(workflowRuntime(selectedDiagnosticResult).operator) }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('manager.gaussianSplatKSplat.workflowExecutionTime')">
            {{ diagnosticExecutionTime(selectedDiagnosticResult) }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('manager.gaussianSplatKSplat.ksplatRef')">
            {{ diagnosticValue(artifactFacts(selectedDiagnosticResult).object || selectedDiagnosticResult.storage_ref) }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('manager.gaussianSplatKSplat.ksplatSize')">
            {{ formatBytes(ksplatFacts(selectedDiagnosticResult).size_bytes || selectedDiagnosticResult.size_bytes) }}
          </el-descriptions-item>
        </el-descriptions>
        <el-divider content-position="left">{{ t('manager.gaussianSplatKSplat.contentInspection') }}</el-divider>
        <div v-if="inspectionLoading" class="inspection-loading">
          {{ t('manager.gaussianSplatKSplat.inspectLoading') }}
        </div>
        <el-alert
        ...
        <template v-else-if="selectedInspection">
          <el-descriptions :column="2" border size="small">
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.inspectStatus')">
              <el-tag :type="inspectionStatusTagType(selectedInspection.summary?.status)">
                {{ diagnosticValue(selectedInspection.summary?.message) }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.splatCount')">
              {{ numberText(selectedInspection.header?.splat_count) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.ksplatVersion')">
              {{ ksplatVersionText(selectedInspection.header) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.compressionLevel')">
              {{ diagnosticValue(selectedInspection.header?.compression_level) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.objectSize')">
              {{ formatBytes(selectedInspection.object_size_bytes) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.bytesInspected')">
              {{ numberText(selectedInspection.bytes_inspected) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.objectContentType')">
              {{ diagnosticValue(selectedInspection.object_content_type || selectedInspection.expected_content_type) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.headerSignature')">
              {{ diagnosticValue(selectedInspection.header_signature_hex) }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('manager.gaussianSplatKSplat.sceneCenter')" :span="2">
          ...
          <el-table class="inspection-checks" :data="selectedInspection.checks || []" size="small" border>
            <el-table-column prop="name" :label="t('manager.gaussianSplatKSplat.inspectCheck')" width="170" />
            <el-table-column :label="t('manager.gaussianSplatKSplat.inspectCheckStatus')" width="100">
              <template #default="{ row }">
                <el-tag :type="inspectionStatusTagType(row.status)" size="small">{{ row.status }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="message" :label="t('manager.gaussianSplatKSplat.inspectCheckMessage')" min-width="260" show-overflow-tooltip />
          </el-table>
        </template>
      </template>
    </el-dialog>
...
const diagnosticSummary = (result) => {
  ...
  if (result?.status === 'ready') return t('manager.gaussianSplatKSplat.contentInspection')
  return t('manager.gaussianSplatKSplat.diagnosticsAvailable')
}
...
const diagnosticExecutionTime = (result) => {
  ...
  return t('manager.gaussianSplatKSplat.executionTimeMs', { ms: numberValue })
}
...
const ksplatVersionText = (header) => {
  if (!header) return '-'
...
const loadTasks = async () => {
  ...
  try {
    const response = await quickViewAPI.listGaussianSplatKSplatTasks({ page: tasksPage.value, page_size: tasksPageSize.value })
    const { items, total } = unwrapList(response)
    tasks.value = items
    tasksTotal.value = total
  } catch (error) {
    ElMessage.error(errorMessage(error, t('manager.gaussianSplatKSplat.loadTasksFailed')))
  } finally {
...
const loadResults = async () => {
  ...
  try {
    ...
    const response = await quickViewAPI.listGaussianSplatKSplats(params)
    const { items, total } = unwrapList(response)
    results.value = items
    resultsTotal.value = total
  } catch (error) {
    ElMessage.error(errorMessage(error, t('manager.gaussianSplatKSplat.loadResultsFailed')))
  } finally {
...
const applySourcePresetFromQuery = () => {
  ...
  form.source.format = gaussianSplatKSplatSourceFormat(route.query.format || form.source.format)
  sourceInitialLocator.value = locator
  const sourceName = String(route.query.name || '').replace(/\s*-\s*.*/, '').trim()
  form.name = t('manager.gaussianSplatKSplat.createFromSourceName', { name: sourceName || sourceFactText(locator) })
}
...
const loadSourceFacts = async (locator) => {
  ...
  try {
    ...
    form.source.format = gaussianSplatKSplatSourceFormat(gaussianSplat.format || form.source.format)
    if (!gaussianSplatKSplatSourceFormat(form.source.format)) {
      ElMessage.warning(t('manager.gaussianSplatKSplat.sourceFormatRequired'))
    }
  } catch (error) {
    ElMessage.warning(errorMessage(error, t('manager.gaussianSplatKSplat.loadSourceFactsFailed')))
  }
}
...
const validateForm = () => {
  ...
  if (!String(form.name || '').trim()) {
    ElMessage.warning(t('manager.gaussianSplatKSplat.nameRequired'))
    return null
  }
  if (!sourceLocator || !sourceEngineID) {
    ElMessage.warning(t('manager.gaussianSplatKSplat.sourceRequired'))
    return null
  }
  if (!String(form.source.item_fingerprint || '').trim()) {
    ElMessage.warning(t('manager.gaussianSplatKSplat.itemFingerprintRequired'))
    return null
  }
  if (!gaussianSplatKSplatSourceFormat(form.source.format)) {
    ElMessage.warning(t('manager.gaussianSplatKSplat.sourceFormatRequired'))
...
const taskPayload = (validated) => ({
  name: String(form.name || '').trim() || t('manager.gaussianSplatKSplat.defaultTaskName'),
  ...
  config: {
    source: {
      ...
      format: gaussianSplatKSplatSourceFormat(form.source.format),
...
const gaussianSplatKSplatSourceFormat = (value) => {
...
const saveTask = async () => {
  ...
  try {
    ...
    if (editingTask.value) {
      await quickViewAPI.updateGaussianSplatKSplatTask(editingTask.value.id, payload)
    } else {
      await quickViewAPI.createGaussianSplatKSplatTask(payload)
    }
    taskDialogVisible.value = false
    await loadTasks()
    ElMessage.success(t('manager.gaussianSplatKSplat.saveSuccess'))
  } catch (error) {
    ElMessage.error(errorMessage(error, t('manager.gaussianSplatKSplat.saveFailed')))
  } finally {
    saving.value = false
  }
}
...
const deleteTask = async (task) => {
  await ElMessageBox.confirm(t('manager.gaussianSplatKSplat.deleteTaskConfirm'), t('manager.gaussianSplatKSplat.delete'), { type: 'warning' })
  await quickViewAPI.deleteGaussianSplatKSplatTask(task.id)
  ElMessage.success(t('manager.gaussianSplatKSplat.deleteSuccess'))
  await loadTasks()
}

const deleteResult = async (result) => {
  await ElMessageBox.confirm(t('manager.gaussianSplatKSplat.deleteResultConfirm'), t('manager.gaussianSplatKSplat.delete'), { type: 'warning' })
  await quickViewAPI.deleteGaussianSplatKSplat(result.id)
  await loadResults()
}
...
const openDiagnostics = async (result) => {
  ...
  try {
    selectedInspection.value = unwrapPayload(await quickViewAPI.inspectGaussianSplatKSplat(result.id))
  } catch (error) {
    inspectionError.value = errorMessage(error, t('manager.gaussianSplatKSplat.inspectFailed'))
  } finally {
...
const openTaskFromQuery = async () => {
  ...
  try {
    const response = await quickViewAPI.getGaussianSplatKSplatTask(taskID)
    editTask(unwrapPayload(response))
  } catch (error) {
    ElMessage.error(errorMessage(error, t('manager.gaussianSplatKSplat.loadTaskFailed')))
  }
}
...
const loadResultTaskFilterFromRoute = async () => {
  ...
  try {
    selectedResultTask.value = unwrapPayload(await quickViewAPI.getGaussianSplatKSplatTask(taskID))
  } catch {
    selectedResultTask.value = null
  }
}
