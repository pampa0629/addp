      ...
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane :label="t('manager.pointCloudCOPC.tasksTab')" name="tasks">
          <div class="tab-toolbar">
            ...
            <el-button type="primary" @click="openCreateDialog">{{ t('manager.pointCloudCOPC.create') }}</el-button>
            <el-button :icon="Refresh" circle @click="loadTasks" />
          </div>

          <el-table :data="tasks" v-loading="tasksLoading" stripe>
            <el-table-column prop="name" :label="t('manager.pointCloudCOPC.name')" min-width="180" show-overflow-tooltip />
            <el-table-column :label="t('manager.pointCloudCOPC.engine')" width="120">
            ...
            <el-table-column :label="t('manager.pointCloudCOPC.actions')" width="360" fixed="right">
              <template #default="{ row }">
                <div class="row-actions">
                  ...
                  <el-button size="small" @click="editTask(row)">{{ t('manager.pointCloudCOPC.edit') }}</el-button>
    ...
    >
      <el-form label-position="top" :model="form">
        ...
        <el-descriptions class="source-facts" :column="2" border size="small">
          <el-descriptions-item :label="t('manager.pointCloudCOPC.sourceLocator')">
            {{ sourceFactText(form.source.item_locator) }}
          </el-descriptions-item>
          <el-descriptions-item :label="t('manager.pointCloudCOPC.sourceEngineID')">
...
const resultTaskFilterLabel = computed(() => {
  ...
  return selectedResultTask.value.name || t('manager.pointCloudCOPC.taskWithId', { id: selectedResultTask.value.id })
})
...
const openSourcePreview = (result) => {
  ...
  router.push({ name: 'DataExplorer', query: { locator: result.locator } })
}
