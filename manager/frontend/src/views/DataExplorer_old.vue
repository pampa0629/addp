      ...
      <div class="preview-wrapper">
        <el-card shadow="never" class="preview-panel">
          ...
          <div v-else class="preview-content">
            ...
            <template v-else>
              <div v-if="objectPreview" class="object-preview" :style="{ gridTemplateRows: objectMetaHeight + 'px 8px 1fr' }">
                ...
                <div class="object-meta-splitter" @mousedown="startObjectMetaResize"></div>
                    ...
                    <template v-else-if="objectContent.kind === 'geojson'">
                      <div class="geojson-preview">
                        <div class="geojson-controls">
                          <div class="toggle-wrapper">
                            <span>地图预览</span>
                            <el-switch v-model="showGeoJSONMap" size="small" />
                          </div>
                          <el-select v-if="showGeoJSONMap" v-model="geoJSONBaseMapType" size="small" class="base-map-select">
                            <el-option
                              v-for="item in baseMapOptions"
                              :key="item.value"
                              :label="item.label"
                        ...
                        <div
                          v-if="showGeoJSONMap"
                          class="geojson-map-container"
                          ref="geoJSONMapContainer"
                        ></div>
                        <pre class="text-preview geojson-text" :class="{ collapsed: showGeoJSONMap }">{{ safeStringify(objectContent.geojson || objectContent.text) }}</pre>
                      </div>
                    </template>
...
const objectMetaHeight = ref(140)
const minObjectMetaHeight = 80
const maxObjectMetaHeight = 300
...
const showGeoJSONMap = ref(true)
const geoJSONBaseMapType = ref('')
const geoJSONMapContainer = ref(null)
let isObjectMetaResizing = false
let objectMetaResizeStartY = 0
let objectMetaResizeStartHeight = 0
let geoJSONGaodeMap = null
let geoJSONGaodeOverlays = []
let geoJSONOlMap = null
let geoJSONOlVectorLayer = null
let geoJSONOlVectorSource = null
let currentGeoJSONBaseType = ''

// 当geoJSONBaseMapType为空时，自动设置为第一个可用底图
watch(
  baseMapOptions,
  (newOptions) => {
    if (newOptions.length === 0) {
      geoJSONBaseMapType.value = ''
      return
    }
    if (!geoJSONBaseMapType.value) {
      geoJSONBaseMapType.value = newOptions[0].value
    }
  },
  { immediate: true }
)
...
const isOlBaseMapType = (value) => ['tiandituVector', 'tiandituImage'].includes(value)
...
const onObjectMetaResize = (event) => {
  if (!isObjectMetaResizing) return
  const delta = event.clientY - objectMetaResizeStartY
  const next = Math.min(maxObjectMetaHeight, Math.max(minObjectMetaHeight, objectMetaResizeStartHeight + delta))
  objectMetaHeight.value = next
}

const stopObjectMetaResize = () => {
  if (!isObjectMetaResizing) return
  isObjectMetaResizing = false
  document.body.classList.remove('is-resizing')
  document.removeEventListener('mousemove', onObjectMetaResize)
  document.removeEventListener('mouseup', stopObjectMetaResize)
}

const startObjectMetaResize = (event) => {
  isObjectMetaResizing = true
  objectMetaResizeStartY = event.clientY
  objectMetaResizeStartHeight = objectMetaHeight.value
  document.body.classList.add('is-resizing')
  document.addEventListener('mousemove', onObjectMetaResize)
  document.addEventListener('mouseup', stopObjectMetaResize)
}
...
const createOlBaseLayers = (baseType, key) => {
...
const ensureOlMap = (baseType) => {
  if (!isOlBaseMapType(baseType)) {
    return null
  }
  ...
  if (currentOlBaseType !== baseType) {
    const { baseLayer, labelLayer } = createOlBaseLayers(baseType, tdtKey)
...
const updateOlOverlays = (map, features, { preserveView = false } = {}) => {
...
const renderMap = async () => {
  ...
  if (isOlBaseMapType(currentType)) {
    if (activeMapType !== currentType) {
      destroyGaodeMap()
    }
    const map = await ensureOlMap(currentType)
    if (!map) {
      return
    }
    applyOlViewState()
    updateOlOverlays(map, geoFeatures.value, { preserveView: switchingBaseMap })
...
const destroyGeoJSONGaodeMap = () => {
  if (geoJSONGaodeOverlays.length > 0) {
    geoJSONGaodeOverlays.forEach((overlay) => {
      if (overlay?.setMap) {
        overlay.setMap(null)
      } else if (overlay?.destroy) {
        overlay.destroy()
    ...
    geoJSONGaodeOverlays = []
  }
  if (geoJSONGaodeMap?.destroy) {
    geoJSONGaodeMap.destroy()
  }
  geoJSONGaodeMap = null
}

const destroyGeoJSONOlMap = () => {
  if (geoJSONOlMap) {
    geoJSONOlMap.setTarget(null)
  }
  geoJSONOlMap = null
  geoJSONOlVectorLayer = null
  geoJSONOlVectorSource = null
  currentGeoJSONBaseType = ''
}

const destroyGeoJSONMap = () => {
  destroyGeoJSONGaodeMap()
  destroyGeoJSONOlMap()
  if (geoJSONMapContainer.value) {
    geoJSONMapContainer.value.innerHTML = ''
  }
}

const renderGeoJSONMap = async () => {
  if (!showGeoJSONMap.value || !objectContent.value?.geojson) {
    destroyGeoJSONMap()
    return
  }
  ...
  if (!geoJSONMapContainer.value) return

  if (!geoJSONBaseMapType.value) {
    if (baseMapOptions.value.length > 0) {
      geoJSONBaseMapType.value = baseMapOptions.value[0].value
    } else {
      destroyGeoJSONMap()
    }
    return
  ...
  const geojsonData = objectContent.value.geojson
  let features = []
  ...
  const currentType = geoJSONBaseMapType.value
  ...
  if (currentType === GAODE_BASE_MAP_VALUE) {
    ...
    if (!geoJSONGaodeMap) {
      geoJSONGaodeMap = new amapLib.Map(geoJSONMapContainer.value, {
        viewMode: '2D',
        zoom: 4,
      ...
      if (amapLib.Scale) {
        geoJSONGaodeMap.addControl(new amapLib.Scale())
      }
      if (amapLib.ToolBar) {
        geoJSONGaodeMap.addControl(new amapLib.ToolBar())
      }
    }

    updateGeoJSONGaodeOverlays(amapLib, geoJSONGaodeMap, features)
    return
  }

  if (isOlBaseMapType(currentType)) {
    destroyGeoJSONGaodeMap()
    const requiresTdtKey = currentType === 'tiandituVector' || currentType === 'tiandituImage'
    const tdtKey = mapConfig.value.tdtKey || DEFAULT_TDT_KEY
    if (requiresTdtKey && !tdtKey) {
      ElMessage.warning('未配置天地图 Key，无法加载天地图底图')
      return
    }

    if (!geoJSONOlMap) {
      geoJSONOlVectorSource = new VectorSource()
      geoJSONOlVectorLayer = new VectorLayer({
        source: geoJSONOlVectorSource,
        style: (feature) => {
          const type = feature.getGeometry()?.getType()
          if (type === 'Point' || type === 'MultiPoint') {
            return tiandituPointStyle
      ...
      geoJSONOlMap = new OlMap({
        target: geoJSONMapContainer.value,
        layers: [],
        view: new OlView({
    ...
    } else if (geoJSONOlMap.getTarget() !== geoJSONMapContainer.value) {
      geoJSONOlMap.setTarget(geoJSONMapContainer.value)
    }

    if (currentGeoJSONBaseType !== currentType) {
      const { baseLayer, labelLayer } = createOlBaseLayers(currentType, tdtKey)
      const layers = geoJSONOlMap.getLayers()
      layers.clear()
      if (baseLayer) layers.push(baseLayer)
      if (labelLayer) layers.push(labelLayer)
      if (geoJSONOlVectorLayer) layers.push(geoJSONOlVectorLayer)
      currentGeoJSONBaseType = currentType
    } else {
      const layers = geoJSONOlMap.getLayers()
      if (geoJSONOlVectorLayer && !layers.getArray().includes(geoJSONOlVectorLayer)) {
        layers.push(geoJSONOlVectorLayer)
      }
    }

    updateGeoJSONOlOverlays(geoJSONOlMap, features)
    return
  }
...
const updateGeoJSONGaodeOverlays = (AMap, map, features) => {
  if (!AMap || !map) return
  ...
  features.forEach((feature) => {
    ...
    switch (geometry.type) {
      ...
      case 'LineString': {
        const path = geometry.coordinates.map(([lng, lat]) => [lng, lat])
        overlays.push(createPolyline(path))
        break
      }
      case 'MultiLineString': {
        geometry.coordinates.forEach((line) => {
      ...
      case 'Polygon': {
        const rings = geometry.coordinates.map((ring) => ring.map(([lng, lat]) => [lng, lat]))
        overlays.push(createPolygon(rings))
        break
      }
      case 'MultiPolygon': {
        geometry.coordinates.forEach((polygonCoords) => {
  ...
  if (overlays.length > 0) {
    map.add(overlays)
    geoJSONGaodeOverlays = overlays
    map.setFitView(overlays, false, [20, 20, 20, 20])
  }
}

const updateGeoJSONOlOverlays = (map, features) => {
  if (!map || !geoJSONOlVectorSource) return

  geoJSONOlVectorSource.clear()
  ...
  if (olFeatures.length > 0) {
    geoJSONOlVectorSource.addFeatures(olFeatures)
    const extent = geoJSONOlVectorSource.getExtent()
    if (extent && isFinite(extent[0])) {
      map.getView().fit(extent, {
        padding: [20, 20, 20, 20],
        maxZoom: 14,
...
watch([showGeoJSONMap, objectContent, geoJSONBaseMapType], () => {
  if (objectContent.value?.kind === 'geojson') {
    nextTick(() => {
      renderGeoJSONMap()
    })
  }
...
.object-meta-splitter:hover::after,
body.is-resizing .object-meta-splitter::after {
  background: var(--el-color-primary);
}
