  ...
  <div class="derived-tasks page-container">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="primary" @click="openDataExplorer"><el-icon><Plus /></el-icon>{{ t('manager.derivedTasks.create') }}</el-button>
      </div>
    </div>
    ...
    <el-table v-loading="loading" :data="tasks" stripe>
      ...
      <el-table-column :label="t('manager.derivedTasks.columns.actions')" width="300" fixed="right">
    ...
    <el-drawer v-model="detailVisible" :title="selectedTask?.name || t('manager.derivedTasks.detail')" size="560px">
...
const { t } = useI18n()
const category = ref(route.query.category === 'spatial_business' ? 'spatial_business' : 'managed_quick_view')
const taskType = ref(typeof route.query.task_type === 'string' ? route.query.task_type : '')
const tasks = ref([])
...
async function syncRoute() {
  ...
  await router.replace({ path: '/derived-tasks', query })
}
...
async function changeCategory() { taskType.value = ''; page.value = 1; await syncRoute(); await loadTasks() }
async function changeFilter() { page.value = 1; await syncRoute(); await loadTasks() }
async function changePageSize() { page.value = 1; await loadTasks() }
async function showDetail(row) {
	try { selectedTask.value = { ...row, ...payload(await getDerivedTask(row.task_type, row.id)) }; detailVisible.value = true }
...
async function openTaskFromRoute() {
  ...
  await showDetail({ task_type: taskType.value, id })
}
...
watch(() => route.query, (query) => {
  const nextCategory = query.category === 'spatial_business' ? 'spatial_business' : 'managed_quick_view'
  const nextType = typeof query.task_type === 'string' ? query.task_type : ''
  if (nextCategory !== category.value || nextType !== taskType.value) {
    category.value = nextCategory; taskType.value = nextType; page.value = 1; loadTasks()
  }
})
onMounted(async () => { await syncRoute(); await loadTasks(); await openTaskFromRoute() })
</script>
