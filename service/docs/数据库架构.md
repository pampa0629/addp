...
| `service_layers` | 服务图层/要素类 | 1000-10000 |
...
```mermaid
erDiagram
    EXTERNAL_SERVICES ||--o{ SERVICE_LAYERS : "1:N"
...
**核心关系**：
- `external_services.id` ← `service_layers.service_id` (1:N)
  - 一个服务可以包含多个图层/要素类
  - 典型场景：WMS 服务包含 10-100 个图层

**外键约束**：
- `service_layers.service_id` → `external_services.id` (ON DELETE CASCADE)
  - 删除服务时，所有关联图层自动删除
  - 保证数据一致性

**租户隔离**：
- `external_services` 表有 `tenant_id` 字段
- `service_layers` 表**没有** `tenant_id`，通过 `service_id` 间接隔离
...
- **图层级**：service_layers 通过 `service_id` 间接隔离
...
SELECT 
  es.*,
  (SELECT COUNT(*) FROM service.service_layers 
...
- [service_layers表](tables/service_layers表.md) - 服务图层表
